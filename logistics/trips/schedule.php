...
$allowed_roles = ['Superadmin', 'admin', 'Transport Manager', 'dispatch-srg', 'dispatch-demra', 'dispatchpos-demra', 'dispatchpos-srg'];
    ...
    if (empty($selected_orders)) {
        ...
        try {
            ...
            foreach ($order_details as $order) {
                ...
                // Accounting — posted via the shared helper (core/functions/helpers.php:
                // postCreditOrderDispatchAccounting()) instead of this file's own hand-rolled
                // copy. Found 8 Aug 2026: the old inline version here (a) picked the revenue
                // account via a plain branch_id match with no "credit"/"not pos" filter, risking
                // a branch's POS revenue account being credited for a credit-sale invoice, (b)
                // never handled Other-Sales/commodity COGS, (c) had no idempotency guard, and
                // (d) ALSO directly overwrote customers.current_balance — the exact double-debit
                // bug already found and fixed in cr/credit_dispatch.php (current_balance is
                // incremented once, atomically, at order CREATION time; writing it again here
                // double-counted it). The shared helper has none of these problems.
                postCreditOrderDispatchAccounting($db, $order->id, $user_id, [
                    'transaction_date' => date('Y-m-d'),
                    'snapshot_trigger' => 'dispatch',
                    'truck_number'     => $vehicle->vehicle_number ?? null,
                    'driver_name'      => $driver->driver_name ?? null,
                    'driver_contact'   => $driver->phone_number ?? null,
                    'shipped_date'     => date('Y-m-d H:i:s'),
                ]);
