    ...
    try {
        ...
        if ($action === 'start_trip') {
            ...
            $db->query("
                ...
                WHERE trip_id = ? AND delivery_status = 'pending'
            ", [$trip_id]);
...
<div class="mb-6 flex justify-between items-start">
    <div>
        ...
        <p class="text-lg text-gray-600">
            ...
            <?php echo $trip->trip_date ? date('l, F j, Y', strtotime($trip->trip_date)) : '—'; ?>
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-green-600 rounded-lg shadow-lg p-6 text-white">
        <p class="text-sm opacity-90">Capacity Used</p>
        <p class="text-3xl font-bold mt-2"><?php echo number_format(($trip->total_weight_kg / $trip->capacity_kg) * 100, 1); ?>%</p>
    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold text-purple-600">
                            <?php echo number_format($trip->total_weight_kg, 2); ?> kg 
                            (<?php echo number_format(($trip->total_weight_kg / $trip->capacity_kg) * 100, 1); ?>%)
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="border-2 rounded-lg p-6 hover:shadow-lg transition-shadow <?php echo $order->delivery_status === 'delivered' ? 'border-green-200 bg-green-50' : 'border-gray-200'; ?>">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <div class="text-right">
                        <p class="text-2xl font-bold text-blue-600">৳<?php echo number_format($order->total_amount ?? 0, 2); ?></p>
