...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_privileges') {
    try {
        ...
        // Run any pending DDL BEFORE the transaction — CREATE TABLE (even
        // IF NOT EXISTS) implicit-commits in MySQL and would make the later
        // commit() throw "There is no active transaction".
        ...
        // Inline per-action ৳ limits from the hierarchical tree (empty/zero = no cap)
        if (isset($_POST['inline_limits']) && is_array($_POST['inline_limits'])) {
            ensureActionLimitsTable();
            $inline_bits = [];
            foreach (getLimitBearingActions() as $ak => $al) {
                if (!array_key_exists($ak, $_POST['inline_limits'])) continue;
                $v = trim((string)$_POST['inline_limits'][$ak]);
                if ($v === '' || (float)$v <= 0) {
                    ...
                    $inline_bits[] = $ak . ' ৳' . number_format((float)$v, 0);
                }
            }
        }
        ...
        try {
            auditLog('user_custom_permissions', 'UPSERT',
                ...
                . (!empty($inline_bits) ? ' | limits: ' . implode(', ', $inline_bits) : ''),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($_POST['action'] ?? '', ['save_approval_limit', 'save_action_limits'])) {
    try {
        ...
        // Only processed when the fields were actually submitted, so a form that
        // carries just the default limit can never silently wipe per-action rows.
        $saved_bits = [];
        foreach (isset($_POST['action_limits']) && is_array($_POST['action_limits']) ? getLimitBearingActions() : [] as $ak => $al) {
...
/* ─── Selected user's current action limits (sidebar + inline tree) ── */
$current_limit = null;
$ual_current   = [];
if ($selected_user) {
...
/* ─── Hierarchical workflow tree (Module → Stage → Pages → Actions) ──
   Pages listed here render grouped under collapsible workflow stages;
   any scanned page not listed falls into "Other pages" at the end.     */
$module_hierarchy = [
    'credit_sales' => [
        ['label' => 'Stage 1 · Order Creation',        'icon' => 'fa-plus-circle',     'pages' => ['sales_hub', 'create_order', 'advance_payment_collection', 'customer_credit_management']],
        ['label' => 'Stage 2 · Approval & Conditions', 'icon' => 'fa-check-circle',    'pages' => ['credit_order_approval', 'payment_watch', 'order_amendment', 'approval_requests']],
        ['label' => 'Stage 3 · Payments & Collection', 'icon' => 'fa-money-bill-wave', 'pages' => ['customer_payment', 'credit_payment_collect', 'payment_history', 'bank_statement']],
        ['label' => 'Stage 4 · Reports & Adjustments', 'icon' => 'fa-chart-bar',       'pages' => ['sales_dashboard', 'all_sales', 'sales_report', 'ageing_report', 'customer_ledger', 'returns', 'over_delivery', 'order_status', 'credit_order_view', 'index']],
    ],
    'production' => [
        ['label' => 'Production & Dispatch Flow',      'icon' => 'fa-industry',        'pages' => ['credit_production', 'credit_dispatch', 'partial_delivery', 'order_status', 'sales_report']],
    ],
];

/* Limit-bearing pages: ৳ input rendered inline next to the page in the tree.
   credit_payment_collect shares the collect_payment ceiling — note only.      */
$inline_limit_primary = [
    'credit_order_approval' => 'approve_order',
    'order_amendment'       => 'amend_order',
    'payment_watch'         => 'early_release',
    'customer_payment'      => 'collect_payment',
    'partial_delivery'      => 'partial_delivery',
];
$inline_limit_shared = [
    'credit_payment_collect' => ['collect_payment', 'Shares the Collect Payment ৳ limit'],
];
$inline_limit_fallback_actions = ['approve_order', 'amend_order', 'early_release']; // these fall back to Default
...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
    ...
    <div class="lg:col-span-1">
        ...
        <div class="bg-white rounded-xl shadow-md p-5 sticky top-4">
            ...
            <!-- ── Default approval limit (fallback ৳) ──────────── -->
            <div class="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                <p class="text-xs font-bold text-amber-800 mb-1">
                    <i class="fas fa-gavel mr-1"></i>Default Approval Limit (৳)
                </p>
                <p class="text-[11px] text-amber-700 mb-2 leading-snug">
                    Backs up Approve / Amend / Early-Release when no per-action limit is set.
                    Per-action ৳ limits are set <strong>inline in the Credit Sales tree</strong> —
                    look for the amber ৳ fields next to limit-bearing pages.
                </p>
                <form method="POST" class="space-y-1.5">
                    <input type="hidden" name="action" value="save_approval_limit">
                    <input type="hidden" name="target_user_id" value="<?php echo $selected_user->id; ?>">
                    <div class="flex items-center gap-1.5">
                        <label class="text-[11px] text-amber-900 font-semibold w-28 shrink-0">Default (fallback)</label>
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-save mr-1"></i>Save Default Limit
                    </button>
                </form>
                <?php if (!empty($ual_current)): ?>
                <div class="mt-2 pt-2 border-t border-amber-200 text-[11px] text-amber-800 space-y-0.5">
                    <p class="font-semibold">Current per-action limits:</p>
                    <?php foreach ($ual_current as $ak => $amt): ?>
                    <p><?php echo htmlspecialchars(str_replace('_', ' ', ucfirst($ak))); ?> — ৳<?php echo number_format($amt, 0); ?></p>
                    <?php endforeach; ?>
                </div>
                <?php endif; ?>
            </div>
                ...
                <div id="panel_<?php echo $module_key; ?>"
                    ...
                    <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <?php
                            // Group pages under workflow stages when this module has a hierarchy
                            $hier   = $module_hierarchy[$module_key] ?? null;
                            $groups = [];
                            if ($hier) {
                                $used = [];
                                foreach ($hier as $stage) {
                                    $stage_pages = [];
                                    foreach ($stage['pages'] as $pk) {
                                        if (isset($all_pages[$pk])) { $stage_pages[$pk] = $all_pages[$pk]; $used[$pk] = true; }
                                    }
                                    if ($stage_pages) $groups[] = ['label' => $stage['label'], 'icon' => $stage['icon'], 'pages' => $stage_pages];
                                }
                                $rest = array_diff_key($all_pages, $used);
                                if ($rest) $groups[] = ['label' => 'Other pages', 'icon' => 'fa-ellipsis-h', 'pages' => $rest];
                            } else {
                                $groups[] = ['label' => null, 'icon' => null, 'pages' => $all_pages];
                            }
                            ?>
                            <div class="space-y-3">
                                <?php foreach ($groups as $grp): ?>
                                <?php if ($grp['label'] !== null): ?>
                                <details open class="border border-<?php echo $col; ?>-100 rounded-lg overflow-hidden">
                                    <summary class="cursor-pointer select-none px-3 py-2 bg-<?php echo $col; ?>-50 text-<?php echo $col; ?>-800 text-xs font-bold flex items-center gap-2">
                                        <i class="fas <?php echo $grp['icon']; ?>"></i><?php echo htmlspecialchars($grp['label']); ?>
                                        <span class="ml-auto font-normal text-<?php echo $col; ?>-500"><?php echo count($grp['pages']); ?> page<?php echo count($grp['pages']) === 1 ? '' : 's'; ?></span>
                                    </summary>
                                    <div class="p-2 space-y-2">
                                <?php endif; ?>

                                <?php foreach ($grp['pages'] as $page_key => $page_meta):
                                ...
                                <div class="border border-gray-100 rounded-lg p-3 bg-gray-50">
                                    ...
                                    <?php if (isset($inline_limit_primary[$page_key])):
                                        $ilk = $inline_limit_primary[$page_key]; ?>
                                    <div class="mt-2 ml-6 flex items-center gap-2 p-1.5 bg-amber-50 border border-amber-200 rounded-lg">
                                        <span class="text-[11px] text-amber-800 font-semibold whitespace-nowrap"><i class="fas fa-gavel mr-1"></i>৳ limit</span>
                                        <input type="number" name="inline_limits[<?php echo $ilk; ?>]" min="0" step="1000"
                                               value="<?php echo isset($ual_current[$ilk]) ? htmlspecialchars($ual_current[$ilk]) : ''; ?>"
                                               placeholder="<?php echo in_array($ilk, $inline_limit_fallback_actions) ? 'default' : 'no cap'; ?>"
                                               class="w-32 px-2 py-1 border border-amber-300 rounded-lg text-xs focus:ring-1 focus:ring-amber-500">
                                        <span class="text-[10px] text-amber-600">
                                            empty = <?php echo in_array($ilk, $inline_limit_fallback_actions) ? 'uses Default fallback' : 'no cap'; ?>;
                                            over the limit <?php echo in_array($ilk, ['collect_payment', 'partial_delivery']) ? 'queues for approval' : 'is blocked'; ?>
                                        </span>
                                    </div>
                                    <?php elseif (isset($inline_limit_shared[$page_key])): ?>
                                    <p class="mt-2 ml-6 text-[10px] text-amber-600 italic">
                                        <i class="fas fa-link mr-1"></i><?php echo htmlspecialchars($inline_limit_shared[$page_key][1]); ?>
                                    </p>
                                    <?php endif; ?>
                                </div>
                                <?php endforeach; ?>
