...
/* ─── Handle POST: save approval limit ─────────────────────── */
ensureApprovalLimitTable();
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_approval_limit') {
    try {
        ...
        if ($limit_raw === '' || (float)$limit_raw <= 0) {
            // Empty or zero = remove the personal cap
            $db->query("DELETE FROM user_approval_limits WHERE user_id = ?", [$target_user_id]);
            $success = "Approval limit removed for <strong>" . htmlspecialchars($tu->display_name) . "</strong>. "
                     . "Only the standard 80% credit-usage escalation rule applies now.";
            auditLog('user_approval_limits', 'deleted',
                "Approval limit removed for '{$tu->display_name}' (id:{$target_user_id})",
            ...
            $success = "Approval limit for <strong>" . htmlspecialchars($tu->display_name) . "</strong> set to "
                     . "৳" . number_format($limit, 0) . ". Orders above this amount will escalate to admin.";
            auditLog('user_approval_limits', 'updated',
                "Approval limit for '{$tu->display_name}' (id:{$target_user_id}) set to ৳" . number_format($limit, 2),
...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
    ...
    <div class="lg:col-span-1">
        ...
        <div class="bg-white rounded-xl shadow-md p-5 sticky top-4">
            ...
            <?php $current_limit = getUserApprovalLimit((int)$selected_user->id); ?>
            <div class="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                <p class="text-xs font-bold text-amber-800 mb-1">
                    <i class="fas fa-gavel mr-1"></i>Order Approval Limit
                </p>
                <p class="text-[11px] text-amber-700 mb-2 leading-snug">
                    Max order value this user may approve on
                    <em>Approve Orders</em>. Anything above escalates to admin.
                    Leave empty for no personal cap (80% credit rule still applies).
                </p>
                <?php if ($current_limit !== null): ?>
                <p class="text-xs text-amber-900 mb-2">Current:
                    <strong>৳<?php echo number_format($current_limit, 0); ?></strong>
                </p>
                <?php else: ?>
                <p class="text-xs text-amber-600 italic mb-2">No personal limit set.</p>
                <?php endif; ?>
                <form method="POST" class="flex gap-1.5">
                    ...
                    <input type="number" name="approval_limit" min="0" step="1000"
                           ...
                           class="flex-1 min-w-0 px-2 py-1.5 border border-amber-300 rounded-lg text-xs focus:ring-1 focus:ring-amber-500">
                    <button type="submit"
                            class="px-3 py-1.5 bg-amber-600 text-white text-xs font-semibold rounded-lg hover:bg-amber-700 transition-colors cursor-pointer">
                        Save
                    </button>
                </form>
            </div>
