...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    // Drive must be configured
    if (!GOOGLE_DRIVE_ENABLED) {
        echo json_encode(['ok' => false, 'msg' => 'Google Drive is not enabled. Check config.php.']);
        exit;
    }

    try {
        $drive = new GoogleDriveService(GOOGLE_OAUTH_CLIENT_ID, GOOGLE_OAUTH_CLIENT_SECRET, GOOGLE_OAUTH_REFRESH_TOKEN);

        // ── Backup now ──────────────────────────────────────────────────────
        if ($action === 'backup_now') {
            $svc    = new DatabaseBackupService($drive, GOOGLE_DRIVE_BACKUP_FOLDER_ID, DB_BACKUP_KEEP_COUNT);
        ...
        if ($action === 'upload_file') {
            ...
            if (empty($folderId)) {
                echo json_encode(['ok' => false, 'msg' => "Drive folder for '$category' is not configured in config.php."]);
                exit;
            }
            ...
            if ($file['size'] > $maxBytes) {
                echo json_encode(['ok' => false, 'msg' => 'File too large (max 50 MB).']);
                exit;
            }
            ...
            $meta     = $drive->uploadFile($file['tmp_name'], $prefix . $safeName, $mime, $folderId);
...
if ($driveEnabled) {
    try {
        ...
        foreach ($folderMap as $cat => $fid) {
            if ($fid) {
                $filesByCategory[$cat] = $drive->listFiles($fid, 50);
            }
        }
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-9 h-9 rounded-lg bg-violet-100 flex items-center justify-center">
                    <i class="fas fa-calendar-clock text-violet-600 text-sm"></i>
                </div>
                <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Schedule</span>
            </div>
            <p class="text-lg font-extrabold text-gray-900">Every 30 min</p>
            <p class="text-xs text-gray-400 mt-0.5">24 hrs rolling window</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="font-bold text-gray-800 flex items-center gap-2">
                ...
                Database Backups
                <span class="text-xs font-normal text-gray-400 ml-1">(newest first &bull; last 50)</span>
            </h2>
            <span class="text-xs text-gray-400">Retention: <?php echo DB_BACKUP_KEEP_COUNT; ?> snapshots = 24 hours</span>
        </div>
