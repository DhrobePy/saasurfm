...
try {
    ...
    $all_roles = [
        'Superadmin', 'admin', 'Accounts', 'accounts-srg',
        ...
        'Transport Manager', 'Expense Initiator', 'Expense Approver',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error) {
    ...
    try {
        ...
        if (isset($_POST['add_user'])) {
            ...
            // Insert user (also stores plain_password for Superadmin visibility)
        ...
        if (isset($_POST['update_user'])) {
            ...
            if (!empty($password)) {
                ...
                $update_fields['plain_password']  = $password;   // keep plain copy in sync
            }
         ...
         <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select id="status" name="status" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors">
                    ...
                    <option value="suspended" <?php echo ($user_to_edit->status ?? '') === 'suspended' ? 'selected' : ''; ?>>Suspended</option>
                </select>
            </div>
        </div>
