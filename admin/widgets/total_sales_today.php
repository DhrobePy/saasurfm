...
try {
    ...
    switch ($date_range) {
        ...
        case 'last_30_days':
            ...
            $end_date = date('Y-m-d 23:59:59'); // End date is today
            break; 
        case 'last_90_days':
    ...
    // RECTIFIED QUERY:
    // This query now ONLY selects from credit_orders, as requested.
    // It checks for completed statuses AND the order_date within the range.
    $query = "
        SELECT 
        ...
        FROM 
            credit_orders 
        WHERE 
            status IN ('Shipped', 'Dispatched', 'Delivered')
            AND order_date BETWEEN ? AND ?
    ";
    
    // We only need to pass the dates once
    $result = $db->query($query, [$start_date, $end_date])->first();
    
    if ($result && $result->total_sales) {
        $stat_value = number_format($result->total_sales, 2);
    }

} catch (Exception $e) {
    // In case of a database error
    $stat_value = "Error";
    error_log("Error in total_sales_today widget: " . $e->getMessage());
}
