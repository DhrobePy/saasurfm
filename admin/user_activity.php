...
/**
 * Where a given audit-log row's reference number should link to, so it can
 * be opened in the record-preview modal. Keyed primarily by (module,
 * record_type) since reference_number itself is unreliable for some modules
 * (e.g. purchase_payment often stores a bank/cheque reference, not a document
 * number) — record_id from the same row is the actual reliable identifier.
 * Returns null when there's no known view page for that combination, in
 * which case the reference number renders as plain text (unchanged from
 * before).
 */
function getAuditRecordViewUrl(?string $module, ?string $recordType, $recordId, ?string $referenceNumber): ?string {
    if (empty($recordId)) return null;
    $id = (int)$recordId;

    // bank rows don't set record_type — the two possible record types
    // (a transaction vs. an inter-account transfer) share this module and
    // must be told apart by the reference-number prefix instead.
    if ($module === 'bank') {
        if ($referenceNumber && str_starts_with($referenceNumber, 'BTX-')) {
            return url("bank/view_transaction.php?id={$id}");
        }
        return null; // transfers (TRF-) have no dedicated view page yet
    }

    $key = $module . '|' . $recordType;
    $map = [
        'expense|expense_voucher'      => 'expense/view_expense_voucher.php?id=',
        'credit_order|credit_order'    => 'cr/credit_order_view.php?id=',
        'customer_payment|customer_payment' => 'cr/credit_payment_receipt.php?id=',
        'purchase|purchase_grn'        => 'purchase/purchase_adnan_grn_receipt.php?id=',
        'purchase|purchase_order'      => 'purchase/purchase_adnan_view_po.php?id=',
        'purchase|adjustment_note'     => 'purchase/purchase_adnan_view_adjustment.php?id=',
        'supplier|supplier'            => 'purchase/view_supplier.php?id=',
    ];
    if (isset($map[$key])) return url($map[$key] . $id);

    // A couple of legacy rows were logged before record_type existed.
    if ($module === 'credit_order' && !$recordType) return url("cr/credit_order_view.php?id={$id}");
    if (($module === 'customer_payments' || $module === 'customer_payment') && !$recordType) {
        return url("cr/credit_payment_receipt.php?id={$id}");
    }
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="module" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        ...
                        <option value="credit_order" <?= $selectedModule === 'credit_order' ? 'selected' : '' ?>>Credit Orders</option>
                        <option value="credit_orders" <?= $selectedModule === 'credit_orders' ? 'selected' : '' ?>>Credit Orders (Delete)</option>
                        <option value="credit_order_returns" <?= $selectedModule === 'credit_order_returns' ? 'selected' : '' ?>>Returns</option>
                        <option value="credit_order_deliveries" <?= $selectedModule === 'credit_order_deliveries' ? 'selected' : '' ?>>Deliveries</option>
                        <option value="customer_payment" <?= $selectedModule === 'customer_payment' ? 'selected' : '' ?>>Customer Payment</option>
                        <option value="customer_ledger" <?= $selectedModule === 'customer_ledger' ? 'selected' : '' ?>>Customer Ledger</option>
                        <option value="expense" <?= $selectedModule === 'expense' ? 'selected' : '' ?>>Expense</option>
                        <option value="bank" <?= $selectedModule === 'bank' ? 'selected' : '' ?>>Bank</option>
                ...
                <div>
                    ...
                    <select name="action" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        ...
                        <optgroup label="General">
                            ...
                            <option value="cancelled" <?= $selectedAction === 'cancelled' ? 'selected' : '' ?>>Cancelled</option>
                            ...
                            <option value="other" <?= $selectedAction === 'other' ? 'selected' : '' ?>>Other</option>
                        </optgroup>
                    </select>
                </div>
                        ...
                        ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs font-medium rounded-full 
                                        ...
                                        $moduleColors = [
                                            ...
                                            'credit_orders'          => 'bg-green-100 text-green-800',
                                            'credit_order_returns'   => 'bg-rose-100 text-rose-800',
                                            'credit_order_deliveries'=> 'bg-teal-100 text-teal-800',
                                            'customer_payment'       => 'bg-purple-100 text-purple-800',
                                            'customer_ledger'        => 'bg-violet-100 text-violet-800',
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <?php
                                    $viewUrl = $activity->reference_number
                                        ? getAuditRecordViewUrl($activity->module, $activity->record_type, $activity->record_id, $activity->reference_number)
                                        : null;
                                    ?>
                                    <?php if ($viewUrl): ?>
                                        <button type="button" onclick="openRecordModal('<?= htmlspecialchars($viewUrl, ENT_QUOTES) ?>')"
                                                class="text-primary-700 font-medium hover:underline hover:text-primary-900 cursor-pointer">
                                            <?= htmlspecialchars($activity->reference_number) ?>
                                        </button>
                                    <?php elseif ($activity->reference_number): ?>
...
// ── Record-preview modal — opens a voucher/order/GRN/etc. view page inline,
// without navigating away from the audit trail ────────────────────────────
function openRecordModal(recordUrl) {
    document.getElementById('recordModalFrame').src = recordUrl;
    document.getElementById('recordModalOpenNew').href = recordUrl;
    document.getElementById('recordModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
}
function closeRecordModal() {
    document.getElementById('recordModal').classList.add('hidden');
    document.getElementById('recordModalFrame').src = 'about:blank';
...
<div id="recordModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
    <div class="bg-white rounded-lg shadow-2xl w-full h-full max-w-6xl max-h-[92vh] flex flex-col overflow-hidden">
        <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
            <span class="text-sm font-semibold text-gray-700"><i class="fas fa-eye mr-2 text-primary-600"></i>Record Preview</span>
            <div class="flex items-center gap-3">
                <a id="recordModalOpenNew" href="#" target="_blank" class="text-xs text-primary-600 hover:underline">
                    <i class="fas fa-up-right-from-square mr-1"></i>Open in new tab
                </a>
                <button type="button" onclick="closeRecordModal()" class="text-gray-500 hover:text-gray-800 text-xl leading-none">&times;</button>
            </div>
        </div>
        <iframe id="recordModalFrame" src="about:blank" class="flex-1 w-full border-0"></iframe>
    </div>
</div>
