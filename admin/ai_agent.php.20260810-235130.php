...
// Order: DeepSeek → OpenRouter (free models) → Groq (5 models) → Cerebras →
//        NVIDIA NIM → GitHub Models → Cloudflare Workers AI →
//        Gemini (3 models) → Anthropic (paid, last resort)
// ═════════════════════════════════════════════════════════════════════════════
function callAIWithFallback(string $system, array $messages, int $max_tokens): array
{
    ...
    // https://openrouter.ai/models?max_price=0 if these start 404ing and swap
    // in whatever's currently listed there, same lesson as the Gemini fix.
    $openrouter_models = [
        ...
        foreach ($openrouter_models as $model) {
            try {
                $res = httpPost('https://openrouter.ai/api/v1/chat/completions', json_encode([
                    ...
                    'temperature' => 0.2,
                    'messages'    => array_merge([['role'=>'system','content'=>$system]], $messages),
                ]), ['Authorization: Bearer '.OPENROUTER_API_KEY, 'Content-Type: application/json', 'HTTP-Referer: https://saas.ujjalfm.com', 'X-Title: Ujjal ERP AI Agent']);
                $d = json_decode($res, true);
                if (!empty($d['choices'][0]['message']['content'])) {
                    return [$d['choices'][0]['message']['content'], "OpenRouter/{$model}"];
                }
            } catch (Exception $e) {
                $msg = $e->getMessage();
                if (strpos($msg,'429')!==false || strpos($msg,'402')!==false || strpos($msg,'404')!==false
                    || strpos($msg,'rate')!==false || strpos($msg,'quota')!==false) {
                    $errors[] = "OpenRouter/{$model}: skipped (".substr($msg,0,60).")"; continue;
                }
    ...
    if (defined('CEREBRAS_API_KEY') && CEREBRAS_API_KEY) {
        foreach ($cerebras_models as $model) {
            try {
                ...
                $d = json_decode($res, true);
                if (!empty($d['choices'][0]['message']['content'])) {
                    return [$d['choices'][0]['message']['content'], "Cerebras/{$model}"];
                }
            } catch (Exception $e) {
                $msg = $e->getMessage();
                if (strpos($msg,'429')!==false || strpos($msg,'quota')!==false || strpos($msg,'404')!==false) {
                    $errors[] = "Cerebras/{$model}: skipped (".substr($msg,0,60).")"; continue;
                }
    ...
    // pool). OpenAI-compatible. Model IDs not live-verified (no working key was
    // available to test) — check build.nvidia.com's catalog if these 404, same
    // lesson as the Gemini model-ID staleness fix below.
    $nvidia_models = [
    ...
    if (defined('NVIDIA_API_KEY') && NVIDIA_API_KEY) {
        foreach ($nvidia_models as $model) {
            try {
                ...
                $d = json_decode($res, true);
                if (!empty($d['choices'][0]['message']['content'])) {
                    return [$d['choices'][0]['message']['content'], "NVIDIA/{$model}"];
                }
            } catch (Exception $e) {
                $msg = $e->getMessage();
                if (strpos($msg,'429')!==false || strpos($msg,'402')!==false || strpos($msg,'404')!==false || strpos($msg,'quota')!==false) {
                    $errors[] = "NVIDIA/{$model}: skipped (".substr($msg,0,60).")"; continue;
                }
    ...
    // ── 6. GitHub Models — free for anyone with a GitHub account. Get a token
    // free at github.com/settings/tokens (plain PAT, no special scopes) after
    // enabling access at github.com/marketplace/models. OpenAI-compatible via
    // Azure AI Inference. Model IDs not live-verified — check
    // github.com/marketplace/models if these 404.
    ...
    if (defined('GITHUB_MODELS_TOKEN') && GITHUB_MODELS_TOKEN) {
        foreach ($github_models as $model) {
            try {
                ...
                $d = json_decode($res, true);
                if (!empty($d['choices'][0]['message']['content'])) {
                    return [$d['choices'][0]['message']['content'], "GitHubModels/{$model}"];
                }
            } catch (Exception $e) {
                $msg = $e->getMessage();
                if (strpos($msg,'429')!==false || strpos($msg,'402')!==false || strpos($msg,'404')!==false || strpos($msg,'RateLimitReached')!==false) {
                    $errors[] = "GitHubModels/{$model}: skipped (".substr($msg,0,60).")"; continue;
                }
    ...
    // "neurons"/day, no card). Needs an Account ID (from the dashboard URL) and
    // an API Token (dash.cloudflare.com/profile/api-tokens, "Workers AI"
    // template). Response shape differs: result.response, not choices[].message.
        ...
        foreach ($cf_models as $model) {
            try {
                $res = httpPost(
                    ...
                    json_encode(['messages' => array_merge([['role'=>'system','content'=>$system]], $messages)]),
                    ['Authorization: Bearer '.CLOUDFLARE_API_TOKEN, 'Content-Type: application/json']
                );
                $d = json_decode($res, true);
                if (!empty($d['result']['response'])) {
                    return [$d['result']['response'], "Cloudflare/{$model}"];
                }
            } catch (Exception $e) {
                $msg = $e->getMessage();
                if (strpos($msg,'429')!==false || strpos($msg,'404')!==false) {
                    $errors[] = "Cloudflare/{$model}: skipped (".substr($msg,0,60).")"; continue;
                }
    ...
    // ── 8. Gemini — 3 live free-tier models ──────────────────────────────────
    ...
    // ── 9. Anthropic Claude Haiku (paid fallback) ─────────────────────────────
