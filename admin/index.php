...
// ── Safe scalar query — always returns a scalar or default ───────────────────
function dq(string $sql, array $p, $default = 0) {
    global $db;
    $res = $db->query($sql, $p);
...
function dql(string $sql, array $p = []): array {
    global $db;
    $res = $db->query($sql, $p);
...
$today_sales       = (float) dq("SELECT IFNULL(SUM(total_amount),0) AS v FROM credit_orders WHERE status IN ('Shipped','Dispatched','Delivered') AND DATE(order_date)=?", [$today]);
$month_sales       = (float) dq("SELECT IFNULL(SUM(total_amount),0) AS v FROM credit_orders WHERE status IN ('Shipped','Dispatched','Delivered') AND order_date BETWEEN ? AND ?", [$month_s, $month_e]);
$pending_count     = (int)   dq("SELECT COUNT(id) AS v FROM credit_orders WHERE status='Pending Approval'", []);
$total_receivables = (float) dq("SELECT IFNULL(SUM(current_balance),0) AS v FROM customers WHERE current_balance > 0.01 AND customer_type='Credit'", []);
$active_customers  = (int)   dq("SELECT COUNT(id) AS v FROM customers WHERE customer_type='Credit'", []);
$over_limit        = (int)   dq("SELECT COUNT(id) AS v FROM customers WHERE customer_type='Credit' AND current_balance > credit_limit AND credit_limit > 0", []);

// ── Lists ─────────────────────────────────────────────────────────────────────
$pending_orders = dql("SELECT co.id, co.order_date, co.total_amount, c.name AS customer_name FROM credit_orders co JOIN customers c ON co.customer_id=c.id WHERE co.status='Pending Approval' ORDER BY co.order_date ASC LIMIT 8");
$recent_orders  = dql("SELECT co.id, co.order_date, co.created_at, co.total_amount, co.status, c.name AS customer_name FROM credit_orders co JOIN customers c ON co.customer_id=c.id ORDER BY co.created_at DESC LIMIT 8");
$top_debtors    = dql("SELECT id, name, business_name, current_balance, credit_limit FROM customers WHERE current_balance > 0.01 AND customer_type='Credit' ORDER BY current_balance DESC LIMIT 6");

// ── 30-day trend ──────────────────────────────────────────────────────────────
$trend_labels = [];
$trend_data   = [];
$_trend_rows  = dql("SELECT DATE(order_date) AS d, IFNULL(SUM(total_amount),0) AS v FROM credit_orders WHERE status IN ('Shipped','Dispatched','Delivered') AND DATE(order_date) BETWEEN ? AND ? GROUP BY DATE(order_date)", [$day30_s, $today]);
$_tmap = [];
foreach ($_trend_rows as $row) $_tmap[$row->d] = (float)$row->v;
for ($i = 29; $i >= 0; $i--) {
    $d = date('Y-m-d', strtotime("-{$i} days"));
    $trend_labels[] = date('d M', strtotime($d));
    $trend_data[]   = $_tmap[$d] ?? 0;
}

// ── User's configured widgets ─────────────────────────────────────────────────
$widgets = dql(
    "SELECT dw.widget_key, dw.widget_name, dw.widget_type, dw.icon, dw.color,
            dw.required_roles, udp.size, udp.date_range, udp.position
...
// Fall back to defaults if user has no preferences saved yet
if (empty($widgets)) {
    $widgets = dql(
        "SELECT widget_key, widget_name, widget_type, icon, color, required_roles,
                'medium' AS size, 'today' AS date_range, sort_order AS position
...
// Daily Owner Digest — fire once/day (after 6am) when an admin opens the
// dashboard, as a fallback if no server cron is configured. Idempotent + safe.
if (function_exists('sendDailyOwnerDigest')) { try { sendDailyOwnerDigest(false); } catch (\Throwable $e) {} }
    ...
    <div class="flex-1 min-w-0">
        <div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
            ...
            <div class="flex flex-wrap justify-between items-start gap-3">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">
                        Good <?php echo (int)date('H') < 12 ? 'morning' : ((int)date('H') < 17 ? 'afternoon' : 'evening'); ?>,
                        <?php echo htmlspecialchars($currentUser['display_name'] ?? 'Admin'); ?>!
                    </h1>
                    <p class="text-sm text-gray-500 mt-0.5">
                        <?php echo date('l, d F Y'); ?> &bull; Live ERP Overview
                    </p>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <a href="../cr/create_order.php" class="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg shadow transition">
            ...
            <div x-show="dailyBriefBanner && !aiOpen"
                 ...
                 class="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-4 flex gap-3 items-start shadow-sm">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between mb-1">
                        <span class="text-xs font-bold text-purple-700 uppercase tracking-wide">
                            <i class="fas fa-sun mr-1 text-yellow-400"></i>AI Daily Brief
                        </span>
                    ...
                    <button @click="openWithTab('insights'); askInsight('daily_brief')"
                            class="mt-2 text-xs text-purple-600 hover:text-purple-800 font-semibold">
                        Open full AI Advisor →
                    </button>
                </div>
            </div>

            <!-- ── EXCEPTION RADAR — what needs a human ────────────────────── -->
            <?php $radar = getExceptionRadar(); $radar_total = array_sum($radar); ?>
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div class="flex items-center justify-between mb-3">
                    <h2 class="text-sm font-bold text-gray-800"><i class="fas fa-triangle-exclamation text-amber-500 mr-1"></i>Exception Radar — what needs a human</h2>
                    <?php if ($radar_total === 0): ?><span class="text-xs text-green-600 font-semibold"><i class="fas fa-check-circle"></i> All clear</span><?php endif; ?>
                </div>
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                    <?php
                    $radar_items = [
                        ['Payments to approve', $radar['pending_approvals'], url('cr/approval_requests.php'),    'fa-money-check-dollar', 'text-orange-600', 'bg-orange-50 border-orange-200'],
                        ['Escalated (over limit)', $radar['escalated_orders'], url('cr/credit_order_approval.php'), 'fa-arrow-trend-up',   'text-red-600',    'bg-red-50 border-red-200'],
                        ['Held for clearance', $radar['held_orders'],        url('cr/payment_watch.php'),         'fa-lock',               'text-amber-600',  'bg-amber-50 border-amber-200'],
                        ['On board / in transit', $radar['in_transit'],      url('cr/credit_dispatch.php'),       'fa-truck',              'text-blue-600',   'bg-blue-50 border-blue-200'],
                        ['Expenses pending', $radar['pending_expenses'],     url('expense/approve_expense.php'),  'fa-receipt',            'text-purple-600', 'bg-purple-50 border-purple-200'],
                        ['QR re-scans (7d)', $radar['qr_reuses'],            url('cr/qr_scan_log.php?reused_only=1'), 'fa-qrcode',         'text-red-600',    'bg-red-50 border-red-200'],
                        ['Negative stock (commodities)', $radar['negative_stock'] ?? 0, url('trading/commodity_inventory.php'), 'fa-warehouse',   'text-rose-600',   'bg-rose-50 border-rose-200'],
                        ['Overdue loans', $radar['overdue_loans'] ?? 0, url('loans/dashboard.php'), 'fa-hand-holding-dollar', 'text-amber-600', 'bg-amber-50 border-amber-200'],
                    ];
                    foreach ($radar_items as [$label, $n, $href, $icon, $hotText, $hotBg]):
                        $hot = $n > 0;
                    ?>
                    <a href="<?php echo $href; ?>" class="flex items-center gap-2 p-2.5 rounded-lg border transition hover:shadow-sm <?php echo $hot ? $hotBg : 'bg-gray-50 border-gray-100'; ?>">
                        <i class="fas <?php echo $icon; ?> <?php echo $hot ? $hotText : 'text-gray-300'; ?>"></i>
                        <div class="min-w-0">
                            <div class="text-lg font-bold leading-none <?php echo $hot ? $hotText : 'text-gray-300'; ?>"><?php echo (int)$n; ?></div>
                            <div class="text-[10px] text-gray-500 leading-tight mt-0.5"><?php echo $label; ?></div>
                        </div>
                    </a>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
                ...
                <!-- Today's Sales -->
                <div class="xl:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 hover:shadow-md transition">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">৳<?php echo number_format($today_sales, 0); ?></p>
                        <p class="text-xs text-gray-400 mt-0.5">This month: ৳<?php echo number_format($month_sales, 0); ?></p>
                    </div>
                </div>

                <!-- Pending Orders -->
                <div class="xl:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 hover:shadow-md transition <?php echo $pending_count > 0 ? 'border-l-4 border-l-orange-400' : ''; ?>">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900"><?php echo number_format($pending_count); ?></p>
                        <p class="text-xs text-gray-400 mt-0.5">orders waiting</p>
                    </div>
                </div>

                <!-- Receivables -->
                <div class="xl:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 hover:shadow-md transition border-l-4 border-l-red-400">
                    <div class="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center flex-shrink-0">
                        <i class="fas fa-hand-holding-usd text-red-500 text-lg"></i>
                    </div>
                    <div>
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Total Receivables</p>
                        <p class="text-2xl font-bold text-gray-900">৳<?php echo number_format($total_receivables, 0); ?></p>
                        <p class="text-xs text-gray-400 mt-0.5">outstanding dues</p>
                    </div>
                </div>

                <!-- Customers -->
                <div class="xl:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 hover:shadow-md transition">
                    <div class="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                        <i class="fas fa-users text-blue-500 text-lg"></i>
                    </div>
                    <div>
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Credit Customers</p>
                        <p class="text-2xl font-bold text-gray-900"><?php echo number_format($active_customers); ?></p>
                        <p class="text-xs text-gray-400 mt-0.5">total accounts</p>
                    </div>
                </div>

                <!-- Over-limit -->
                <div class="xl:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center gap-4 hover:shadow-md transition <?php echo $over_limit > 0 ? 'border-l-4 border-l-red-500' : ''; ?>">
                    <div class="w-12 h-12 rounded-full <?php echo $over_limit > 0 ? 'bg-red-100' : 'bg-gray-100'; ?> flex items-center justify-center flex-shrink-0">
                        <i class="fas fa-exclamation-triangle <?php echo $over_limit > 0 ? 'text-red-600' : 'text-gray-400'; ?> text-lg"></i>
                    </div>
                    <div>
                        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Over Credit Limit</p>
                        <p class="text-2xl font-bold <?php echo $over_limit > 0 ? 'text-red-600' : 'text-gray-900'; ?>"><?php echo $over_limit; ?></p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-bold text-gray-800 text-sm">
                            <i class="fas fa-chart-bar text-blue-500 mr-2"></i>30-Day Sales Trend
                        </h3>
                        <span class="text-xs text-gray-400">Credit orders (completed)</span>
                    </div>
                    ...
                    <div class="h-48 flex items-center justify-center text-gray-400 text-sm">
                        <div class="text-center"><i class="fas fa-chart-bar text-3xl mb-2 opacity-30"></i><br>No sales data yet</div>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="lg:col-span-1 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col">
                    <div class="p-4 border-b border-gray-100 flex items-center justify-between">
                        <h3 class="font-bold text-gray-800 text-sm">
                            <i class="fas fa-clock text-orange-500 mr-2"></i>Pending Approvals
                        </h3>
                        <?php if ($pending_count > 0): ?>
                        <span class="bg-orange-500 text-white text-xs font-bold px-2 py-0.5 rounded-full"><?php echo $pending_count; ?></span>
                        <?php endif; ?>
                    </div>
                    <div class="flex-1 overflow-y-auto divide-y divide-gray-50" style="max-height:260px;">
                        <?php if (empty($pending_orders)): ?>
                        <div class="py-10 text-center text-gray-400 text-sm">
                            <i class="fas fa-check-circle text-green-500 text-2xl mb-2"></i><br>All caught up!
                        </div>
                        <?php else: ?>
                        <?php foreach ($pending_orders as $o): ?>
                        <a href="../cr/credit_order_approval.php?id=<?php echo $o->id; ?>"
                           class="flex items-center justify-between px-4 py-3 hover:bg-orange-50 transition">
                            <div>
                                <p class="text-sm font-semibold text-gray-800 leading-tight"><?php echo htmlspecialchars($o->customer_name); ?></p>
                                <p class="text-xs text-gray-400">#<?php echo $o->id; ?> &bull; <?php echo date('d M', strtotime($o->order_date)); ?></p>
                            </div>
                            <div class="text-right flex-shrink-0 ml-3">
                                <span class="text-sm font-bold text-orange-600">৳<?php echo number_format($o->total_amount, 0); ?></span>
                            </div>
                        </a>
                        <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                    <?php if ($pending_count > 8): ?>
                    <div class="p-3 border-t border-gray-100 text-center">
                        <a href="../cr/credit_order_approval.php" class="text-xs text-blue-600 hover:text-blue-800 font-semibold">
                            View all <?php echo $pending_count; ?> orders →
                        </a>
                    </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-5">
                ...
                <div class="lg:col-span-3 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-4 border-b border-gray-100 flex items-center justify-between">
                        <h3 class="font-bold text-gray-800 text-sm">
                            <i class="fas fa-list-alt text-blue-500 mr-2"></i>Recent Credit Orders
                        </h3>
                        <a href="../cr/all_sales.php" class="text-xs text-blue-600 hover:text-blue-800 font-semibold">View all →</a>
                    </div>
                    <?php if (empty($recent_orders)): ?>
                    <div class="py-10 text-center text-gray-400 text-sm">
                        <i class="fas fa-inbox text-3xl mb-2 opacity-30"></i><br>No orders yet
                    </div>
                    <?php else: ?>
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="px-4 py-3 text-gray-500 font-mono text-xs">#<?php echo $o->id; ?></td>
                                    <td class="px-4 py-3 font-semibold text-gray-800"><?php echo htmlspecialchars($o->customer_name); ?></td>
                                    <td class="px-4 py-3 text-gray-500 text-xs"><?php echo date('d M Y', strtotime($o->created_at)); ?></td>
                                    <td class="px-4 py-3 text-right font-bold text-gray-900">৳<?php echo number_format($o->total_amount, 0); ?></td>
                                    <td class="px-4 py-3">
                                        <span class="px-2 py-0.5 rounded-full text-xs font-semibold <?php echo $sc; ?>">
                ...
                <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col">
                    <div class="p-4 border-b border-gray-100 flex items-center justify-between">
                        <h3 class="font-bold text-gray-800 text-sm">
                            <i class="fas fa-exclamation-circle text-red-500 mr-2"></i>Top Debtors
                        </h3>
                        <?php if ($total_receivables > 0): ?>
                        <span class="text-xs font-bold text-red-600">৳<?php echo number_format($total_receivables, 0); ?> total</span>
                        <?php endif; ?>
                    </div>
                    <?php if (empty($top_debtors)): ?>
                    <div class="flex-1 flex items-center justify-center py-10 text-gray-400 text-sm">
                        <div class="text-center"><i class="fas fa-smile text-green-400 text-3xl mb-2"></i><br>No outstanding dues!</div>
                    </div>
                    <?php else: ?>
                    <div class="flex-1 divide-y divide-gray-50 overflow-y-auto" style="max-height:340px;">
                        <?php foreach ($top_debtors as $c):
                            $pct = $c->credit_limit > 0 ? min(100, ($c->current_balance / $c->credit_limit) * 100) : 0;
                            $bar_color = $pct >= 100 ? 'bg-red-500' : ($pct >= 80 ? 'bg-orange-400' : 'bg-blue-400');
                        ?>
                        <div class="px-4 py-3">
                            <div class="flex items-center justify-between mb-1">
                                <div>
                                    <p class="text-sm font-semibold text-gray-800 leading-tight"><?php echo htmlspecialchars($c->name); ?></p>
                                    <?php if (!empty($c->business_name)): ?>
                                    <p class="text-xs text-gray-400"><?php echo htmlspecialchars($c->business_name); ?></p>
                                    <?php endif; ?>
                                </div>
                                <span class="text-sm font-bold text-red-600 ml-3 flex-shrink-0">
                                    ৳<?php echo number_format($c->current_balance, 0); ?>
                                </span>
                            </div>
                            <?php if ($c->credit_limit > 0): ?>
                            <div class="w-full bg-gray-100 rounded-full h-1.5 mt-1.5">
                                <div class="<?php echo $bar_color; ?> h-1.5 rounded-full" style="width:<?php echo $pct; ?>%"></div>
                            </div>
                            <p class="text-xs text-gray-400 mt-0.5">
                                <?php echo number_format($pct, 0); ?>% of ৳<?php echo number_format($c->credit_limit, 0); ?> limit
                                <?php if ($pct >= 100): ?><span class="text-red-500 font-bold ml-1">OVER LIMIT</span><?php endif; ?>
                            </p>
                            <?php endif; ?>
                        </div>
                        <?php endforeach; ?>
                    ...
                    <div class="p-3 border-t border-gray-100 text-center">
                        <a href="../customers/index.php" class="text-xs text-blue-600 hover:text-blue-800 font-semibold">View all customers →</a>
                    </div>
                    <?php endif; ?>
                </div>

            </div><!-- /recent orders + debtors -->
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    <?php
                    $actions = [
                        ['../cr/create_order.php',              'fa-plus-circle',     'blue',   'New Order'],
                        ['../cr/credit_order_approval.php',     'fa-check-circle',    'green',  'Approve Orders'],
                        ['../cr/credit_dispatch.php',           'fa-truck',           'indigo', 'Dispatch'],
                        ['../customers/index.php',              'fa-users',           'purple', 'Customers'],
                        ['../product/pricing_engine.php',       'fa-bolt',            'yellow', 'Pricing'],
                        ['../cr/ageing_report.php',             'fa-calendar-alt',    'red',    'Ageing Report'],
                        ['../cr/all_sales.php',                 'fa-list',            'gray',   'All Orders'],
                        ['db_viewer.php',                       'fa-database',        'slate',  'DB Viewer'],
                        ['../cr/credit_history_export.php',     'fa-file-excel',      'emerald','Export'],
                        ['settings.php',                        'fa-cog',             'blue',   'Customize'],
                    ];
                    $bg_map = ['blue'=>'bg-blue-50 hover:bg-blue-100 text-blue-600','green'=>'bg-green-50 hover:bg-green-100 text-green-600','indigo'=>'bg-indigo-50 hover:bg-indigo-100 text-indigo-600','purple'=>'bg-purple-50 hover:bg-purple-100 text-purple-600','yellow'=>'bg-yellow-50 hover:bg-yellow-100 text-yellow-600','red'=>'bg-red-50 hover:bg-red-100 text-red-600','gray'=>'bg-gray-50 hover:bg-gray-100 text-gray-600','slate'=>'bg-slate-50 hover:bg-slate-100 text-slate-600','emerald'=>'bg-emerald-50 hover:bg-emerald-100 text-emerald-600'];
                    foreach ($actions as [$href, $icon, $color, $label]):
                        $cls = $bg_map[$color] ?? 'bg-gray-50 hover:bg-gray-100 text-gray-600';
                    ?>
                    <a href="<?php echo htmlspecialchars($href); ?>"
                       class="flex flex-col items-center gap-2 p-3 rounded-xl <?php echo $cls; ?> transition text-center">
                        <i class="fas <?php echo $icon; ?> text-xl"></i>
                        <span class="text-xs font-semibold leading-tight"><?php echo $label; ?></span>
                    </a>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-base font-bold text-gray-700">
                        <i class="fas fa-th-large text-gray-400 mr-2"></i>My Dashboard Widgets
                    </h2>
                    <a href="settings.php" class="text-xs text-blue-600 hover:text-blue-800 font-semibold">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
                    ...
                    <div class="<?php echo $col; ?>">
                        ...
                        <div class="bg-red-50 border border-red-200 text-red-600 p-4 rounded-xl text-sm">
                            ...
                            Widget file missing: <code><?php echo htmlspecialchars(basename($wp)); ?></code>
                        </div>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
    ...
    <div x-show="aiOpen"
                ...
                <div x-show="insightResponse && !insightLoading">
                    ...
                    <div class="flex gap-2 mt-2 items-center">
                        ...
                        <span class="ml-auto text-xs text-gray-400 flex items-center gap-1">
                            <template x-if="insightSource === 'ai'"><span><i class="fas fa-robot text-purple-400"></i> AI Analysis</span></template>
                            <template x-if="insightSource === 'local'"><span><i class="fas fa-calculator text-indigo-400"></i> Smart Local Analysis</span></template>
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div x-show="dbResponse&&!dbLoading" class="p-3 space-y-3">
                    <div class="bg-blue-50 border border-blue-200 rounded-xl p-3">
                        <div class="flex items-center gap-2 mb-1.5">
                            <i class="fas text-blue-600 text-xs" :class="dbSource === 'local' ? 'fa-calculator' : 'fa-robot'"></i>
                            <span class="text-xs font-bold text-blue-700 uppercase tracking-wide" x-text="dbSource === 'local' ? 'Summary (local match)' : 'Summary'"></span>
        ...
        <div x-show="activeTab==='agent'" class="flex flex-col flex-1 overflow-hidden">
            ...
            <div class="border-t border-gray-200 p-3 flex-shrink-0 bg-white">
                ...
                <p class="text-xs text-gray-400 mt-1.5"><i class="fas fa-lock text-green-500 mr-1"></i>Safe pre-defined actions · Always confirms before writing</p>
            </div>
        </div>
...
// ── Alpine Dashboard App
function dashboardApp() {
    return {
        aiOpen: false, activeTab: 'insights', unreadBadge: false, copied: false,

        insightLoading: false, insightResponse: '', insightError: '', insightSource: 'ai',
        insightAction: '', insightLabel: '', insightQ: '',
        insightLoadingMsg: 'Analyzing ERP data…',
        dailyBriefBanner: false, dailyBriefHtml: '',
        ...
        dbLoading: false, dbQ: '', dbResponse: '', dbError: '', dbSource: 'ai',
        dbRows: [], dbColumns: [], dbSql: '', dbRowCount: 0, dbTime: '',
        showSql: false, dbLoadingMsg: 'Translating to SQL…',
        ...
        agentLoading: false, agentInput: '', agentMessages: [],
        agentLoadingMsg: 'Thinking…', agentProvider: '',
        ...
        agentCapabilities: [
            ...
            { label:'<i class="fas fa-exchange-alt text-orange-500 mr-1"></i>Update Status',   msg:'Update the status of a credit order' },
            { label:'<i class="fas fa-user-circle text-teal-500 mr-1"></i>Create Customer',    msg:'Add a new customer to the system' },
        ],

        init() { setTimeout(() => this.fetchBanner(), 900); },
        ...
        openWithTab(tab) { this.aiOpen = true; this.activeTab = tab; this.unreadBadge = false; },
        ...
        async askInsight(action) {
            ...
            try {
                ...
                clearInterval(iv);
                if (d.success) { this.insightResponse = this.md(d.response); this.insightSource = d.source || 'ai'; }
        ...
        async askInsightCustom() {
            ...
            try {
                ...
                if (d.success) { this.insightResponse = this.md(d.response); this.insightSource = d.source || 'ai'; this.insightQ = ''; }
        ...
        async fetchBanner() {
            try {
                ...
                if (d.success) {
                    ...
                    this.dailyBriefBanner = true; this.unreadBadge = true;
                }
            } catch {}
        },
        ...
        async runDbQuery() {
            ...
            try {
                ...
                if (d.success) {
                    ...
                    this.dbSql=d.sql||''; this.dbRowCount=d.row_count||0; this.dbSource=d.source||'ai';
        ...
        startAgentAction(msg) { this.activeTab='agent'; this.agentInput=msg; this.sendAgentMessage(); },

        async sendAgentMessage() {
            const msg=this.agentInput.trim(); if (!msg) return;
            this.agentInput='';
            this.agentMessages.push({ role:'user', content:msg, time:this.timeNow() });
            this.agentLoading=true; this.scrollChat();
