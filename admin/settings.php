...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    } elseif ($_POST['action'] === 'save_invoice_outstanding_display') {
        // Show/hide "Previous Due" and "Total Due" (total outstanding) on the printed credit invoice
        try {
            $db->getPdo()->exec("CREATE TABLE IF NOT EXISTS `settings` (
                `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
                `name` varchar(100) NOT NULL,
            ...
            $val = !empty($_POST['show_invoice_outstanding']) ? '1' : '0';
            $db->query(
                "INSERT INTO settings (name, value) VALUES ('show_invoice_outstanding', ?)
                 ON DUPLICATE KEY UPDATE value = VALUES(value)",
                [$val]
            );
            auditLog('company_settings', 'invoice_outstanding_display_toggled',
                'Credit invoice Previous Due / Total Due display ' . ($val === '1' ? 'SHOWN' : 'HIDDEN') . ' by ' . ($currentUser['display_name'] ?? 'admin'), []);
            $success = 'Invoice display policy saved: Previous Due and Total Due are now '
                     . ($val === '1' ? 'shown on the printed invoice.' : 'hidden from the printed invoice.');
        } catch (\Throwable $e) {
            $error = 'Failed to save invoice display setting: ' . $e->getMessage();
        }
    } elseif ($_POST['action'] === 'save_credit_limit_auto_release') {
        // New rule: auto-release over-limit orders to ship once paid back within limit
        try {
            $db->getPdo()->exec("CREATE TABLE IF NOT EXISTS `settings` (
                `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
                `name` varchar(100) NOT NULL,
            ...
            $val = !empty($_POST['credit_limit_auto_release']) ? '1' : '0';
            $db->query(
                "INSERT INTO settings (name, value) VALUES ('credit_limit_auto_release', ?)
                 ON DUPLICATE KEY UPDATE value = VALUES(value)",
                [$val]
            );
            auditLog('company_settings', 'credit_limit_auto_release_toggled',
                'Credit-limit auto-release ' . ($val === '1' ? 'ENABLED' : 'DISABLED') . ' by ' . ($currentUser['display_name'] ?? 'admin'), []);
            $success = 'Credit-limit auto-release policy saved: over-limit orders '
                     . ($val === '1'
                        ? 'now auto-hold at ship (not approval) and release automatically once paid within limit.'
                        : 'go back to the standard approval-gate behavior (no automatic ship hold).');
        } catch (\Throwable $e) {
            $error = 'Failed to save credit-limit auto-release setting: ' . $e->getMessage();
        }
    } elseif ($_POST['action'] === 'save_commodity_sale_approval') {
        // Commodity Trading: require approval for every commodity sale posted
        // by a non-admin, on top of the per-user delegated ৳ limit.
        try {
            $db->getPdo()->exec("CREATE TABLE IF NOT EXISTS `settings` (
                `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
                `name` varchar(100) NOT NULL,
            ...
            $val = !empty($_POST['commodity_sale_require_approval']) ? '1' : '0';
            $db->query(
                "INSERT INTO settings (name, value) VALUES ('commodity_sale_require_approval', ?)
                 ON DUPLICATE KEY UPDATE value = VALUES(value)",
                [$val]
            );
            auditLog('settings', 'updated',
                'Commodity sale approval policy ' . ($val === '1' ? 'ENABLED' : 'DISABLED') . ' by ' . ($currentUser['display_name'] ?? 'admin'), []);
            $success = 'Commodity sale approval policy saved: '
                     . ($val === '1'
                        ? 'every non-admin commodity sale now needs approval, regardless of any delegated limit.'
                        : 'non-admin sales post directly if their delegated ৳ limit covers the amount.');
        } catch (\Throwable $e) {
            $error = 'Failed to save commodity sale approval setting: ' . $e->getMessage();
        }
    } elseif ($_POST['action'] === 'save_loan_disbursement_approval') {
        // Loans: require approval for every loan disbursement posted by a
        // non-admin, on top of the per-user delegated ৳ limit.
        try {
            $db->getPdo()->exec("CREATE TABLE IF NOT EXISTS `settings` (
                `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
                `name` varchar(100) NOT NULL,
            ...
            $val = !empty($_POST['loan_disbursement_require_approval']) ? '1' : '0';
            $db->query(
                "INSERT INTO settings (name, value) VALUES ('loan_disbursement_require_approval', ?)
                 ON DUPLICATE KEY UPDATE value = VALUES(value)",
                [$val]
            );
            auditLog('settings', 'updated',
                'Loan disbursement approval policy ' . ($val === '1' ? 'ENABLED' : 'DISABLED') . ' by ' . ($currentUser['display_name'] ?? 'admin'), []);
            $success = 'Loan disbursement approval policy saved: '
                     . ($val === '1'
                        ? 'every non-admin loan disbursement now needs approval, regardless of any delegated limit.'
                        : 'non-admin disbursements post directly if their delegated ৳ limit covers the amount.');
        } catch (\Throwable $e) {
            $error = 'Failed to save loan disbursement approval setting: ' . $e->getMessage();
        }
    }
...
// Current invoice outstanding-display policy for the toggle UI
$invoice_outstanding_on = showInvoiceOutstandingEnabled();
// Current payment-approval policy for the toggle UI
$payment_approval_on = paymentApprovalRequiredForAll();
// Current credit-limit auto-release policy for the toggle UI
$credit_limit_auto_release_on = creditLimitAutoReleaseEnabled();
// Current commodity-sale approval policy for the toggle UI
$commodity_sale_approval_on = commoditySaleApprovalRequiredForAll();
$loan_disbursement_approval_on = loanDisbursementApprovalRequiredForAll();
...
<!-- ═══ Credit Invoice — Outstanding Display ═══════════════════════════════════ -->
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
    <h2 class="text-lg font-bold text-gray-800 mb-1 flex items-center gap-2">
        <i class="fas fa-file-invoice text-teal-600"></i> Credit Invoice — Outstanding Display
    </h2>
    <p class="text-sm text-gray-500 mb-4">
        When ON, the printed credit invoice shows the customer's <strong>Previous Due</strong> and
        <strong>Total Due</strong> (total outstanding across all orders) below the invoice amount.
        Turn OFF to print only this invoice's own amount and balance, without revealing the customer's
        wider outstanding position.
    </p>
    <form method="POST" class="flex items-center gap-4">
        <input type="hidden" name="action" value="save_invoice_outstanding_display">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
        <input type="hidden" name="show_invoice_outstanding" value="0">
        <label class="inline-flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="show_invoice_outstanding" value="1"
                   <?php echo $invoice_outstanding_on ? 'checked' : ''; ?>
                   class="w-5 h-5 accent-teal-600">
            <span class="text-sm font-medium text-gray-700">
                Show Previous Due / Total Due on printed invoice
                <span class="ml-1 text-xs px-2 py-0.5 rounded-full <?php echo $invoice_outstanding_on ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'; ?>">
                    <?php echo $invoice_outstanding_on ? 'ON' : 'OFF'; ?>
                </span>
            </span>
        </label>
        <button type="submit" class="px-4 py-2 bg-teal-600 text-white text-sm font-semibold rounded-lg hover:bg-teal-700">
            <i class="fas fa-save mr-1"></i>Save Policy
        </button>
    </form>
</div>

<!-- ═══ Credit-Limit Auto-Release Policy ═══════════════════════════════════════ -->
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
    <h2 class="text-lg font-bold text-gray-800 mb-1 flex items-center gap-2">
        <i class="fas fa-truck-ramp-box text-blue-600"></i> Credit-Limit Auto-Release Policy
    </h2>
    <p class="text-sm text-gray-500 mb-4">
        When ON, an order that crosses the customer's <strong>credit limit</strong> does not change who may
        approve it — approval, production and "ready to ship" proceed exactly as normal. Instead, the order
        automatically holds at the <strong>shipping</strong> step and releases itself the moment the customer's
        total due (including this invoice) is brought back within their credit limit through an authorized
        payment — no manual clearance needed. When OFF, the process is exactly as it is today.
    </p>
    <form method="POST" class="flex items-center gap-4">
        <input type="hidden" name="action" value="save_credit_limit_auto_release">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
        <input type="hidden" name="credit_limit_auto_release" value="0">
        <label class="inline-flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="credit_limit_auto_release" value="1"
                   <?php echo $credit_limit_auto_release_on ? 'checked' : ''; ?>
                   class="w-5 h-5 accent-blue-600">
            <span class="text-sm font-medium text-gray-700">
                Auto-release over-limit orders once paid within limit
                <span class="ml-1 text-xs px-2 py-0.5 rounded-full <?php echo $credit_limit_auto_release_on ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'; ?>">
                    <?php echo $credit_limit_auto_release_on ? 'ON' : 'OFF'; ?>
                </span>
            </span>
        </label>
...
<!-- ═══ Commodity Sale Approval Policy ══════════════════════════════════════════ -->
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
    <h2 class="text-lg font-bold text-gray-800 mb-1 flex items-center gap-2">
        <i class="fas fa-money-bill-transfer text-rose-600"></i> Commodity Sale Approval Policy
    </h2>
    <p class="text-sm text-gray-500 mb-4">
        Applies to <strong>Trading → Commodity Sale</strong> (direct resale of purchased commodities, e.g. surplus
        wheat). When ON, <strong>every sale posted by a non-admin</strong> is parked for approval regardless of any
        delegated ৳ limit. When OFF, a non-admin with a delegated commodity-sale limit covering the amount posts
        directly; one with <strong>no limit configured</strong> still always queues — a limit is never assumed.
        Superadmin/admin always post directly either way.
    </p>
    <form method="POST" class="flex items-center gap-4">
        <input type="hidden" name="action" value="save_commodity_sale_approval">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
        <input type="hidden" name="commodity_sale_require_approval" value="0">
        <label class="inline-flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="commodity_sale_require_approval" value="1"
                   <?php echo $commodity_sale_approval_on ? 'checked' : ''; ?>
                   class="w-5 h-5 accent-rose-600">
            <span class="text-sm font-medium text-gray-700">
                Require approval for every commodity sale
                <span class="ml-1 text-xs px-2 py-0.5 rounded-full <?php echo $commodity_sale_approval_on ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'; ?>">
                    <?php echo $commodity_sale_approval_on ? 'ON' : 'OFF'; ?>
                </span>
            </span>
        </label>
        <button type="submit" class="px-4 py-2 bg-rose-600 text-white text-sm font-semibold rounded-lg hover:bg-rose-700">
            <i class="fas fa-save mr-1"></i>Save Policy
        </button>
    </form>
</div>

<!-- ═══ Loan Disbursement Approval Policy ══════════════════════════════════════ -->
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
    <h2 class="text-lg font-bold text-gray-800 mb-1 flex items-center gap-2">
        <i class="fas fa-hand-holding-dollar text-amber-600"></i> Loan Disbursement Approval Policy
    </h2>
    <p class="text-sm text-gray-500 mb-4">
        Applies to <strong>Loans → New Loan</strong> (cash advances to customers/suppliers/related parties). When
        ON, <strong>every loan posted by a non-admin</strong> is parked for approval regardless of any delegated ৳
        limit. When OFF, a non-admin with a delegated loan-disbursement limit covering the amount posts directly;
        one with <strong>no limit configured</strong> still always queues — a limit is never assumed.
        Superadmin/admin always post directly either way.
    </p>
    <form method="POST" class="flex items-center gap-4">
        <input type="hidden" name="action" value="save_loan_disbursement_approval">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
        <input type="hidden" name="loan_disbursement_require_approval" value="0">
        <label class="inline-flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="loan_disbursement_require_approval" value="1"
                   <?php echo $loan_disbursement_approval_on ? 'checked' : ''; ?>
                   class="w-5 h-5 accent-amber-600">
            <span class="text-sm font-medium text-gray-700">
                Require approval for every loan disbursement
                <span class="ml-1 text-xs px-2 py-0.5 rounded-full <?php echo $loan_disbursement_approval_on ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'; ?>">
                    <?php echo $loan_disbursement_approval_on ? 'ON' : 'OFF'; ?>
                </span>
            </span>
        </label>
        <button type="submit" class="px-4 py-2 bg-amber-600 text-white text-sm font-semibold rounded-lg hover:bg-amber-700">
