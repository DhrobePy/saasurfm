...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $payment_type = $_POST['payment_type'] ?? null; // Get payment type
        $user_notes = trim($_POST['notes']) ?: ''; // Get user notes
        
        // Combine payment type and notes for storage
        $notes = $payment_type;
        if (!empty($user_notes)) {
            $notes = $payment_type . " | " . $user_notes;
        }
        ...
        if ($amount > $amount_due + 0.001) { // Add a small tolerance for float comparison
             throw new Exception("Payment amount (৳" . number_format($amount, 2) . ") cannot be greater than the amount due (৳" . number_format($amount_due, 2) . ").");
        }
        if ($account_id <= 0) throw new Exception("Please select a valid payment account.");
        if (empty($payment_type)) throw new Exception("Please select a payment type."); // Add validation
        ...
        $journal_desc = $payment_type . " received for Rental #" . $rental->id . 
        ...
        $db->insert('transaction_lines', [
            ...
            'entry_type' => 'debit',
            'amount' => $amount,
            'description' => $payment_type . " from " . $rental->customer_name
        ]);
        ...
        $db->insert('transaction_lines', [
            ...
            'entry_type' => 'credit',
            'amount' => $amount,
            'description' => $payment_type . " applied to Rental #" . $rental->id
        ]);
    ...
    <div class="bg-gray-50 p-4 rounded-lg border mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="text-md font-medium text-gray-900">৳<?php echo number_format($rental->total_amount, 2); ?></p>
            </div>
            <div>
                <p class="text-sm text-gray-500">Previously Paid</p>
                <p class="text-md font-medium text-gray-900">৳<?php echo number_format($total_paid, 2); ?></p>
            </div>
            <div class="bg-blue-100 border border-blue-300 rounded-lg p-3">
                <p class="text-sm font-bold text-blue-800">Amount Due</p>
                <p class="text-2xl font-bold text-blue-600">৳<?php echo number_format($amount_due, 2); ?></p>
            </div>
        </div>
    </div>
                    ...
                    <select name="payment_type" required 
                            ...
                            x-init="$watch('amount', value => { 
                                if (value >= maxAmount - 0.001) { 
                                    $el.value = 'Full Balance Payment'; 
                                } else if ($el.value === 'Full Balance Payment') {
                                    $el.value = 'Partial Payment';
                                }
                            })">
                        <option value="Installment Payment">Installment Payment</option>
                        <option value="Partial Payment">Partial Payment</option>
                        <option value="Monthly Rental Payment">Monthly Rental Payment</option>
                        <option value="Full Balance Payment" <?php echo ($amount_due == $amount_due) ? '' : ''; // Logic to pre-select if amount matches? Might be better to let Alpine handle it. ?>>Full Balance Payment</option>
                    </select>
                </div>
        ...
        <div class="bg-yellow-50 border-2 border-yellow-300 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-green-50 border-2 border-green-300 rounded-lg p-4">
                    ...
                    <p class="text-2xl font-bold text-green-600 mt-2" x-text="'৳' + parseFloat(amount).toLocaleString('en-BD', {minimumFractionDigits: 2})">৳0.00</p>
                </div>
                <div class="bg-red-50 border-2 border-red-300 rounded-lg p-4">
                    ...
                    <p class="text-2xl font-bold text-red-600 mt-2" x-text="'৳' + parseFloat(amount).toLocaleString('en-BD', {minimumFractionDigits: 2})">৳0.00</p>
                </div>
            </div>
        </div>
