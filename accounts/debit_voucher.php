...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_voucher') {
    ...
    if (empty($voucher_date)) {
        ...
        try {
            ...
            // Generate voucher number — sequence-based (MAX-existing+1), not
            // rand(1,9999). voucher_number is UNIQUE; a random 4-digit value
            // with no collision retry can clash on any busy day.
            $dv_date_prefix = date('Ymd');
            $last_dv = $db->query(
                "SELECT voucher_number FROM debit_vouchers WHERE voucher_number LIKE ? ORDER BY id DESC LIMIT 1",
                ["DV-{$dv_date_prefix}-%"]
            )->first();
            $dv_seq = $last_dv ? ((int)substr($last_dv->voucher_number, -4) + 1) : 1;
            $voucher_number = sprintf("DV-%s-%04d", $dv_date_prefix, $dv_seq);
