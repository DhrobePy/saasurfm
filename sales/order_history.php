...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    foreach ($export_results as $row) {
        fputcsv($output, [
            ...
            number_format($row->discount_amount ?? 0, 2),
            number_format($row->total_amount ?? 0, 2),
            number_format($row->advance_paid ?? 0, 2),
            number_format($row->amount_paid ?? 0, 2),
            number_format($row->balance_due ?? 0, 2),
            ucwords(str_replace('_', ' ', $row->status ?? '')),
            number_format($row->total_weight_kg ?? 0, 2),
            $row->assigned_branch ?? '',
            $row->created_by,
                    ...
                    <?php foreach ($orders as $order): ?>
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">
                                    <div>
                                        ...
                                        <span class="font-medium"><?php echo $order->order_date ? date('d M Y', strtotime($order->order_date)) : '—'; ?></span>
                                    </div>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-gray-500">
                                    <?php echo $order->created_at ? date('d M Y h:i A', strtotime($order->created_at)) : '—'; ?>
                                </div>
                            </td>
