...
if ($customer->current_balance < 0) {
    $advice[] = "Customer in Advance: This customer has a positive balance (BDT "
        . number_format(abs($customer->current_balance), 2) . "). Ensure new orders are applied against this advance.";
}
    ...
    <div class="flex flex-col md:flex-row">
        <div class="md:w-1/3 p-6 flex flex-col items-center justify-center bg-gray-50 border-b md:border-b-0 md:border-r border-gray-200">
            <?php if ($customer->photo_url) : ?>
                <img class="h-40 w-40 rounded-full object-cover shadow-xl"
                     src="<?php echo url($customer->photo_url); ?>"
                     alt="Profile Photo"
                     id="customer_photo_img"
                     onerror="this.style.display='none'; document.getElementById('customer_photo_fallback').style.display='flex';">
                <span id="customer_photo_fallback"
                      class="h-40 w-40 rounded-full bg-primary-100 text-primary-700 font-bold text-6xl items-center justify-center shadow-lg"
                      style="display:none;">
                    <?php echo strtoupper(substr(trim($customer->name), 0, 1)); ?>
                </span>
            <?php else : ?>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-600"><?php echo number_format($stats->total_paid_last_90_days ?? 0, 0); ?></p>
                </div>
                <div class="col-span-2">
                    <label class="block text-xs text-gray-500">Avg. Invoice Value</label>
                    <p class="text-3xl font-bold text-gray-900"><?php echo number_format($stats->avg_invoice_value ?? 0, 2); ?></p>
                </div>
            </div>
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <?php if ($customer->customer_type == 'Credit' && ($customer->initial_due ?? 0) > 0) : ?>
                    <tr class="bg-amber-50 border-b border-amber-200">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-amber-700">—</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm">
                            <span class="px-2 py-0.5 bg-amber-100 text-amber-700 rounded text-xs font-semibold">Opening Balance</span>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-amber-700">Initial Due</td>
                        <td class="px-4 py-3 text-sm text-amber-700">Previous balance brought forward at account creation</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-right font-bold text-amber-700"><?php echo number_format($customer->initial_due, 2); ?></td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-right">—</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-right font-bold text-amber-700"><?php echo number_format($customer->initial_due, 2); ?></td>
                    </tr>
                <?php endif; ?>
