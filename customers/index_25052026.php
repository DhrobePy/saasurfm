...
// Bug 18 fix: Delete was triggered by a simple GET request — CSRF-able via a crafted
// link. Also had no role restriction despite the page being accessible to sales/collector.
// Fixed: require POST + CSRF token + Superadmin/admin only.
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_customer'])) {
        // Role check
        $deleter_role = $_SESSION['user_role'] ?? '';
        if (!in_array($deleter_role, ['Superadmin', 'admin'])) {
            throw new Exception('Permission denied. Only Superadmin or admin can delete customers.');
        }
                    ...
                    <?php foreach ($customers as $customer): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                ...
                                <!-- Bug 18 fix: POST form with CSRF — not a GET link -->
                                <form method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this customer? This action cannot be undone.');">
                                    ...
                                    <button type="submit" class="text-gray-500 hover:text-red-600" title="Delete Customer">
