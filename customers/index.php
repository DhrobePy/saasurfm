...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_customer'])) {
    try {
        ...
        // Feature #13: a customer with any outstanding balance (due OR advance)
        // cannot be deleted — settle/clear it first.
        ...
        // Feature #3/#13: soft-delete to the Recycle Bin, cascade-archiving the
        // customer's ENTIRE footprint (orders, ledger, payments, deliveries…) so
        // nothing is orphaned and the whole account restores in one click.
        // Photo file is intentionally NOT removed so a restore is complete.
        ensureRecycleBinTables();
        $pdo = $db->getPdo();
        $pdo->beginTransaction();
        $batch = recycleBegin('customer', 'Customer — ' . ($cust->name ?? '') . " (#{$del_id})", $del_id);
        recycleArchiveCustomerCascade($batch, $del_id);      // related records first
        recycleArchiveDelete($batch, 'customers', 'id', $del_id); // then the customer row
        recycleFinalize($batch);
        $pdo->commit();

        // Audit trail (was missing) — records who deleted whom, and the batch to restore.
        ...
        $_SESSION['success_flash'] = 'Customer and all related records moved to Recycle Bin (batch #' . $batch . ') — restorable from Admin → Recycle Bin.';
    } catch (Exception $e) {
...
// ── Customers with Zero Due (fully settled) ───────────────────────────────
$customers_zero = $db->query(
    "SELECT c.id, c.name, c.business_name, c.phone_number, c.credit_limit,
            COALESCE(tb.true_balance, c.initial_due, 0) AS current_balance,
            lo.last_order_date, COALESCE(oo.open_orders, 0) AS open_orders
     $base_enriched_sql
     WHERE ABS(COALESCE(tb.true_balance, c.initial_due, 0)) <= 0.01 $where_sql
...
<!-- Sub-tabs: Directory | Balances (Feature #4) -->
<div class="mb-4 flex items-center gap-2 border-b border-gray-200">
    <a href="directory.php" class="px-4 py-2 text-sm font-semibold text-gray-500 hover:text-gray-800 border-b-2 border-transparent">
        <i class="fas fa-address-book mr-1"></i>Directory
    </a>
    <a href="index.php" class="px-4 py-2 text-sm font-semibold text-green-600 border-b-2 border-green-500">
          ...
          <td class="px-2 py-1 text-center whitespace-nowrap">
            <div class="flex items-center justify-center gap-1.5">
              ...
              <?php if (in_array($userRole, ['Superadmin','admin'])): ?>
                <?php if (abs((float)($c->current_balance ?? 0)) <= 0.01): ?>
                ...
                <?php else: ?>
                <span class="text-gray-200 cursor-not-allowed" title="Cannot delete — customer has an outstanding balance"><i class="fas fa-lock text-[11px]"></i></span>
                <?php endif; ?>
              <?php endif; ?>
            </div>
          </td>
...
<!-- ══ Zero Due — Settled Customers (compact) ══════════════════════════════ -->
<div class="mt-6 bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
  <div class="px-3 py-2 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
    <span class="text-xs font-bold text-gray-700"><i class="fas fa-check-circle text-gray-400 mr-1"></i>Zero Due — Settled Customers</span>
    <span class="text-[10px] text-gray-400">(<?php echo count($customers_zero); ?>)</span>
    <span class="ml-auto text-[10px] text-gray-400">Nothing owed &amp; no advance held</span>
  </div>
  <?php if (empty($customers_zero)): ?>
    <p class="px-3 py-5 text-center text-[11px] text-gray-400">No fully-settled customers<?php echo $search !== '' ? ' match this search' : ''; ?>.</p>
  <?php else: ?>
  <div class="overflow-x-auto">
    <table class="min-w-full text-sm">
      <thead class="bg-gray-50">
        <tr>
          <th class="px-2 py-1.5 text-left  text-[10px] font-semibold text-gray-500 uppercase">Customer</th>
          <th class="px-2 py-1.5 text-left  text-[10px] font-semibold text-gray-500 uppercase">Phone</th>
          <th class="px-2 py-1.5 text-right text-[10px] font-semibold text-gray-500 uppercase">Credit Limit</th>
          <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-500 uppercase">Open Orders</th>
          <th class="px-2 py-1.5 text-center text-[10px] font-semibold text-gray-500 uppercase">Actions</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-gray-50">
        <?php foreach ($customers_zero as $c): ?>
        <tr class="hover:bg-gray-50">
          <td class="px-2 py-1">
            <a href="view.php?id=<?php echo (int)$c->id; ?>" class="font-medium text-gray-700 hover:text-blue-600"><?php echo htmlspecialchars($c->name); ?></a>
            <?php if (!empty($c->business_name)): ?><span class="block text-[10px] text-gray-400"><?php echo htmlspecialchars($c->business_name); ?></span><?php endif; ?>
          </td>
          <td class="px-2 py-1 text-xs text-gray-500"><?php echo htmlspecialchars($c->phone_number ?? '—'); ?></td>
          <td class="px-2 py-1 text-right text-xs text-gray-600"><?php echo fmt_money((float)$c->credit_limit); ?></td>
          <td class="px-2 py-1 text-center">
            <?php $oo = (int)$c->open_orders; ?>
            <?php if ($oo > 0): ?><span class="text-[10px] text-blue-600 font-semibold"><?php echo $oo; ?></span><?php else: ?><span class="text-gray-200 text-[10px]">—</span><?php endif; ?>
          </td>
          <td class="px-2 py-1 text-center whitespace-nowrap">
            <div class="flex items-center justify-center gap-1.5">
              <a href="<?php echo url('cr/customer_ledger.php'); ?>?customer_id=<?php echo $c->id; ?>" class="text-blue-400 hover:text-blue-600" title="Ledger"><i class="fas fa-book text-[11px]"></i></a>
              <a href="manage.php?id=<?php echo $c->id; ?>" class="text-gray-300 hover:text-blue-500" title="Edit"><i class="fas fa-pencil-alt text-[11px]"></i></a>
              <?php if (in_array($userRole, ['Superadmin','admin'])): ?>
              <form method="POST" class="inline"
                    onsubmit="return confirm('Delete <?php echo htmlspecialchars(addslashes($c->name)); ?>?\n\nThe customer AND all their records (orders, ledger, payments) move to the Recycle Bin. Restorable by Superadmin.');">
                <input type="hidden" name="delete_customer" value="1">
                <input type="hidden" name="delete_id" value="<?php echo $c->id; ?>">
                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrfToken); ?>">
                <button type="submit" class="text-gray-300 hover:text-red-500" title="Delete → Recycle Bin"><i class="fas fa-trash text-[11px]"></i></button>
              </form>
              <?php endif; ?>
            </div>
          </td>
  ...
  <?php if (count($customers_zero) >= 500): ?>
  <p class="text-center text-[10px] text-amber-600 py-1.5 border-t border-amber-100 bg-amber-50">Showing first 500 — use search to narrow</p>
  <?php endif; ?>
  <?php endif; ?>
</div>
