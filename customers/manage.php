...
// Feature #13: only Superadmin may set a customer's opening balance (balance field).
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $status = $_POST['status'];
        // Only Superadmin may set an opening balance; others always create with 0.
        $initial_due = $is_superadmin ? (float)($_POST['initial_due'] ?? 0.00) : 0.00;
        ...
        if ($edit_mode) {
            ...
            // Create invoice and ledger entry for initial due
            if ($customer_type === 'Credit' && $initial_due > 0) {
                ...
                // Create the matching customer_ledger opening entry
                $db->insert('customer_ledger', [
                    'customer_id'      => $new_customer_id,
                    'transaction_date' => date('Y-m-d'),
                    'transaction_type' => 'opening_balance',
                    'reference_type'   => 'initial_due',
                    'reference_id'     => $new_customer_id,
                    'invoice_number'   => $order_number,
                    'description'      => 'Opening balance – Previous due carried forward',
                    ...
                    <div class="space-y-4">
                        ...
                        <div x-show="customerType === 'Credit'" x-transition>
                            <label for="initial_due" class="block text-sm font-medium text-gray-700 mb-1">
                                ...
                                <span class="text-[10px] px-1.5 py-0.5 bg-purple-100 text-purple-700 rounded ml-1">Superadmin</span>
                            </label>
                        ...
                        <?php else: ?>
                        <div x-show="customerType === 'Credit'" x-transition class="p-2 bg-gray-50 border border-gray-200 rounded text-xs text-gray-500">
                            <i class="fas fa-lock mr-1"></i>Only a Superadmin can set an opening balance. This customer will start at ৳0.
                        </div>
                        <?php endif; ?>
