...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_return') {
    try {
        ...
        $success = "Return #{$return_number} recorded successfully" . ($auto_approve ? " and approved." : ". Awaiting admin approval.");
        $selected_order_id = 0;
        $selected_order    = null;
        $order_items       = [];
    ...
    if (!$is_admin) {
        ...
        try {
            ...
            if ($ret->status === 'approved') {
                ...
                // Remove the ledger credit note entry
                $db->query(
                    "DELETE FROM customer_ledger WHERE reference_type = 'credit_order_returns' AND reference_id = ?",
                    [$ret_id]
                );
            ...
            $db->query("DELETE FROM credit_order_return_items WHERE return_id = ?", [$ret_id]);
            $db->query("DELETE FROM credit_order_returns WHERE id = ?", [$ret_id]);

            $pdo->commit();

            auditLog('credit_order_returns', 'deleted',
                "Return {$ret->return_number} deleted by admin" . ($ret->status === 'approved' ? " (financials reversed)" : ""),
                ['return_id' => $ret_id, 'was_approved' => $ret->status === 'approved']
            );

            $success = "Return #{$ret->return_number} deleted." . ($ret->status === 'approved' ? " Invoice and ledger have been restored." : "");
