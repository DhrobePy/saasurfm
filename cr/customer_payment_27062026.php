...
// --- DATA: Get Customers — true ledger balance, never the stale cache column ---
$customers = $db->query(
    "SELECT c.id, c.name, c.business_name, c.phone_number, c.credit_limit, c.initial_due,
            COALESCE(tb.true_balance, c.initial_due, 0) AS outstanding_balance,
            (c.credit_limit - COALESCE(tb.true_balance, c.initial_due, 0)) AS available_credit
     FROM customers c
     LEFT JOIN (
