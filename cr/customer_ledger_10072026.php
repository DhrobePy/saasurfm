        ...
        try {
            ...
            if ($ref_id) {
                if (in_array($ref_type, ['credit_order','credit_orders'])) {
                    ...
                    // Reverse each order's paid columns before deleting allocations, otherwise the
                    // order appears "more paid than it is" after the ledger row is gone.
                    // Advance payments incremented advance_paid; invoice payments incremented amount_paid.
                    $pay_row = null; $allocs = [];
                    try {
