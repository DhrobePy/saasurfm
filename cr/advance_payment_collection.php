...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'record_advance_payment' && !$error) {
    try {
        ...
        // ── Maker/checker (Feature #6): advances follow the same policy as invoice
        // receipts. When the global approval policy is ON, EVERY non-admin advance
        // queues; otherwise only advances over the user's collect_payment limit do.
        // Admins always post directly. The checker reviews it back on THIS page.
        $exec_pending_req_id = (int)($_POST['pending_req_id'] ?? 0);
        if (!$is_admin) {
            $collect_cap = getUserActionLimit((int)($currentUser['id'] ?? 0), 'collect_payment');
            $over_limit  = $collect_cap !== null && $total_payment_amount > $collect_cap;

            if ($exec_pending_req_id) {
                if ($over_limit) {
                    throw new Exception('Your collection limit (৳' . number_format($collect_cap, 0)
                        . ') does not cover this ৳' . number_format($total_payment_amount, 0)
                        . ' advance — a more senior officer must post it.');
                }
            } else {
                $needs_approval = paymentApprovalRequiredForAll() || $over_limit;
                if ($needs_approval) {
                    $pdo->rollBack();
                    $req_id = submitPendingRequest('advance', $total_payment_amount, [
                        ...
                        'customer_id' => $customer_id,
                        'summary'     => 'Advance ৳' . number_format($total_payment_amount, 0) . ' from ' . $customer_name
                                       . ' (' . $payment_method . ($reference_number !== '' ? ', ref ' . $reference_number : '') . ')',
                        'maker_limit' => $collect_cap,
                    ]);
                    if (!$req_id) throw new Exception('Could not queue this advance for approval. Please try again.');
                    $reason = $over_limit ? 'over ৳' . number_format($collect_cap, 0) . ' limit' : 'payment approval policy';
                    auditLogPayment('queued', $req_id, 'REQ-' . $req_id,
                        'Advance of ৳' . number_format($total_payment_amount, 2) . ' from ' . $customer_name
                        . ' queued for approval (' . $reason . ') by ' . ($currentUser['display_name'] ?? 'user'));
                    $_SESSION['success_flash'] = 'This advance (৳' . number_format($total_payment_amount, 0)
                        . ') was sent for approval as request #' . $req_id
                        . '. It will post once a senior officer approves it.';
        ...
        // --- 2. Generate Payment Number (Bug 15 fix: sequence-based, no rand()) ---
        $adv_date_prefix = date('Ymd', strtotime($payment_date));
        $last_adv        = $db->query(
            "SELECT payment_number FROM customer_payments WHERE payment_number LIKE ? ORDER BY id DESC LIMIT 1",
            ["ADVPAY-{$adv_date_prefix}-%"]
        )->first();
        $adv_seq        = $last_adv ? ((int)substr($last_adv->payment_number, -4) + 1) : 1;
        $payment_number = sprintf("ADVPAY-%s-%04d", $adv_date_prefix, $adv_seq);
        ...
        // customers.current_balance is a cache, but it's still read directly by
        // credit-limit checks — a silently swallowed failure here (Database::update()
        // returns false, never throws) could let a customer's cached balance drift.
        ...
        $dr_line_id = $db->insert('transaction_lines', [
        ...
        if (!$dr_line_id) throw new Exception("Failed to post the debit line of the advance payment journal entry.");

        // Credit: Accounts Receivable
        $cr_line_id = $db->insert('transaction_lines', [
            'journal_entry_id' => $journal_id,
        ...
        if (!$cr_line_id) throw new Exception("Failed to post the credit line of the advance payment journal entry — would leave the journal unbalanced.");
        ...
        foreach ($allocations as $order_id => $amount) {
            ...
            if ($alloc_amount > 0) {
                ...
                if (!$alloc_id) throw new Exception("Failed to record the advance-payment allocation for order #{$order_id}.");

                // Update credit_orders: increase advance_paid, decrease balance_due.
                // Inside a loop over multiple orders, so a mid-loop write failure is
                // genuinely plausible — previously unchecked, it would leave
                // payment_allocations/customer_ledger saying the advance was applied
                // while credit_orders.advance_paid/balance_due never actually moved.
                $db->query(
                ...
                if ($db->error()) throw new Exception("Failed to update order #{$order_id}'s advance/balance for this allocation.");
            }
        }
        ...
        if ($unallocated_amount > 0.01) {
            ...
            if (!$db->update('customer_payments', $payment_id, ['notes' => $notes_update])) {
                throw new Exception("Failed to record the unallocated amount note.");
            }
        }
        ...
        if ($exec_pending_req_id) {
            if (decidePendingRequest($exec_pending_req_id, 'approved', null, $payment_number)) {
                auditLogPayment('request_approved', $payment_id ?? 0, $payment_number,
                    'Pending advance request #' . $exec_pending_req_id . ' approved & posted as ' . $payment_number
...
// --- Feature #6: pending advance-request review (?pending_req prefills the form) ---
$preq        = null;
$preq_error  = null;
$preq_maker  = null;
$preq_cust   = null;
...
if ($preq_id_get) {
    $preq = getPendingRequest($preq_id_get, 'advance');
    if (!$preq) {
        $preq_error = "Request #{$preq_id_get} is not open — it may already be approved, rejected, or cancelled.";
    } else {
        $my_cap = $is_admin ? null : getUserActionLimit((int)($currentUser['id'] ?? 0), 'collect_payment');
        if (!$is_admin && $my_cap !== null && (float)$preq->amount > $my_cap) {
            $preq_error = "Request #{$preq_id_get} (৳" . number_format((float)$preq->amount, 0)
                        . ") is above your own collection limit of ৳" . number_format($my_cap, 0) . ".";
            $preq = null;
        } else {
            $pp = $preq->payload_arr;
            $preq_cust  = $db->query("SELECT id, name, phone_number FROM customers WHERE id = ?", [(int)($pp['customer_id'] ?? 0)])->first();
            $preq_maker = $db->query("SELECT display_name FROM users WHERE id = ?", [(int)$preq->maker_user_id])->first();
            if (!$preq_cust) { $preq_error = 'The customer on this request no longer exists.'; $preq = null; }
        }
    }
}
...
<?php if ($preq_error): ?>
<div class="mb-4 px-4 py-2.5 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
...
<?php elseif ($preq): ?>
<div class="mb-4 px-4 py-3 bg-purple-50 border-2 border-purple-300 rounded-lg text-purple-900 text-sm">
    <p class="font-bold mb-1"><i class="fas fa-user-check mr-1"></i>Reviewing pending advance request #<?php echo (int)$preq->id; ?></p>
    <p class="text-xs">
        ...
        Verify below and submit to post it, or
        <a href="approval_requests.php" class="underline font-semibold">reject it from the requests page</a>.
    </p>
</div>
<?php endif; ?>
        ...
        fetchPendingOrders() {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    // Feature #6: apply a queued advance-request's amount + allocations (checker review)
                    if (window.__advPrefill) {
                        const pf = window.__advPrefill;
                        this.totalPaymentAmount = parseFloat(pf.amount) || 0;
                        this.pendingOrders.forEach(o => {
                            if (pf.allocations && pf.allocations[o.id] !== undefined) {
                                o.advanceAmount = parseFloat(pf.allocations[o.id]) || 0;
                            }
                        });
                        this.calculateTotal();
                        window.__advPrefill = null;
                    }
                } else {
...
document.addEventListener('DOMContentLoaded', () => {
    $(document).ready(function() {
        ...
        <?php if ($preq && $preq_cust): $pp = $preq->payload_arr; ?>
        // Feature #6: prefill the whole form from advance request #<?php echo (int)$preq->id; ?>
        window.__advPrefill = {
            amount:      <?php echo (float)($pp['totalPaymentAmount'] ?? 0); ?>,
        ...
        setV('input[name="payment_date"]',    <?php echo json_encode($pp['payment_date'] ?? date('Y-m-d')); ?>);
        setV('input[name="reference_number"]',<?php echo json_encode($pp['reference_number'] ?? ''); ?>);
        setV('textarea[name="notes"]',        <?php echo json_encode($pp['notes'] ?? ''); ?>);
        const mEl = document.querySelector('select[name="payment_method"]');
        if (mEl) { mEl.value = <?php echo json_encode($pp['payment_method'] ?? 'Cash'); ?>; toggleBankAccount(mEl); }
        <?php if (!empty($pp['bank_account_id'])): ?>
        const bEl = document.getElementById('bank_account_select');
        if (bEl) { bEl.value = '<?php echo (int)$pp['bank_account_id']; ?>'; }
        <?php endif; ?>
        // Select the customer in Select2 → fires customer-selected → loads orders → prefill applies
        const opt = document.querySelector('#customer_id option[value="<?php echo (int)$preq_cust->id; ?>"]');
        if (opt) {
            $('#customer_id').val('<?php echo (int)$preq_cust->id; ?>').trigger('change');
        } else {
            // Customer not in the dropdown list — dispatch directly
            window.dispatchEvent(new CustomEvent('customer-selected', { detail: {
                id: '<?php echo (int)$preq_cust->id; ?>',
                name: <?php echo json_encode($preq_cust->name); ?>
            }}));
        }
        <?php endif; ?>
    });
