...
// Get customers with credit limits — Credit-type customers only (Bug 2 fix)
$customers = $db->query(
...
function searchCustomers(query) {
    ...
    if (matches.length === 0) {
        ...
        dropdown.innerHTML = matches.map(c => {
            const avail = c.creditLimit - (c.balance - c.initialDue);
            const limitStr = c.creditLimit > 0 ? ` — Avail: ৳${avail.toFixed(0)}` : '';
            return `<div class="px-4 py-2 hover:bg-blue-50 cursor-pointer text-sm border-b border-gray-100"
                ...
                <span class="text-blue-600 text-xs">${limitStr}</span>
            </div>`;
        }).join('');
    }
...
function updateCustomerInfo(c) {
    ...
    const initialDue  = c.initialDue  || 0;
    const balance     = c.balance     || 0;
    const available   = creditLimit - (balance - initialDue);
...
function updateCreditUsage(orderTotal) {
    ...
    const creditLimit = c.creditLimit || 0;
    const available   = creditLimit - (c.balance - c.initialDue);
