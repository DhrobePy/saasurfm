...
// Feature #2 — this page now lives inside the returns_center.php hub as a tab.
// A direct hit redirects into the hub (preserving any deep-link params like
// ?order_id=). When embedded (CR_EMBED), the hub already gated + rendered chrome.
if (!defined('CR_EMBED')) {
    $qs = $_GET; $qs['tab'] = 'returns';
    ...
    if (!$is_admin) {
        ...
        try {
            ...
            if ($action === 'approve_return') {
                ...
                if ($db->error()) throw new Exception('Failed to mark the return as approved.');
                $pdo->commit();
    ...
    if (!$is_admin) {
        ...
        try {
            ...
            if ($ret->status === 'approved') {
                ...
                if ($db->error()) throw new Exception('Failed to restore the order total/balance while undoing this return.');
                ...
                if ($db->error()) throw new Exception('Failed to restore the customer balance while undoing this return.');
                ...
                if ($ret->return_type === 'full') {
                    ...
                    if ($db->error()) throw new Exception('Failed to restore the order status while undoing this return.');
                }
            }
...
// Every write below was previously unchecked — Database::query()/::insert() never
// throw on failure (silently swallowed PDOException), so the caller's transaction
// would still commit even if, say, the customer_ledger credit note failed to post:
// the return would end up marked 'approved' with the order's total/balance reduced
// but no ledger trail justifying it, or vice versa. Now throws on any failure so the
// caller's existing catch-block rollback actually has something to catch.
function _applyReturnFinancials($db, $order, $return_id, $return_number, $return_type, $amount, $user_id): void {
    ...
    if ($db->error()) throw new Exception('Failed to update the order total/balance for this return.');

    $db->query(
        "UPDATE customers SET current_balance = GREATEST(0, current_balance - ?) WHERE id = ?",
        [$amount, $customer_id]
    );
    if ($db->error()) throw new Exception('Failed to update the customer balance for this return.');
    ...
    if (!$ledger_id) throw new Exception('Failed to post the customer ledger credit note for this return.');

    if ($return_type === 'full') {
        $db->query("UPDATE credit_orders SET status = 'cancelled', updated_at = NOW() WHERE id = ?", [$order_id]);
        if ($db->error()) throw new Exception('Failed to cancel the order for this full return.');
    }
}
