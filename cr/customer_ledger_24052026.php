        ...
        try {
            ...
            if ($ref_id) {
                if (in_array($ref_type, ['credit_order','credit_orders'])) {
                    // Delete order workflow, shipping, items, deliveries, returns, then order
                    $db->getPdo()->exec("DELETE FROM credit_order_workflow WHERE order_id = {$ref_id}");
                    $db->getPdo()->exec("DELETE FROM credit_order_shipping WHERE order_id = {$ref_id}");
                    $db->getPdo()->exec("DELETE FROM production_schedule WHERE order_id = {$ref_id}");
                    try { $db->getPdo()->exec("DELETE FROM credit_order_delivery_items WHERE delivery_id IN (SELECT id FROM credit_order_deliveries WHERE order_id = {$ref_id})"); } catch(Exception $e2){}
                    try { $db->getPdo()->exec("DELETE FROM credit_order_deliveries WHERE order_id = {$ref_id}"); } catch(Exception $e2){}
                    try { $db->getPdo()->exec("DELETE FROM credit_order_return_items WHERE return_id IN (SELECT id FROM credit_order_returns WHERE order_id = {$ref_id})"); } catch(Exception $e2){}
                    try { $db->getPdo()->exec("DELETE FROM credit_order_returns WHERE order_id = {$ref_id}"); } catch(Exception $e2){}
                    try { $db->getPdo()->exec("DELETE FROM payment_allocations WHERE order_id = {$ref_id}"); } catch(Exception $e2){}
                    $db->getPdo()->exec("DELETE FROM credit_order_items WHERE order_id = {$ref_id}");
                    $db->getPdo()->exec("DELETE FROM credit_orders WHERE id = {$ref_id}");
                } elseif (in_array($ref_type, ['customer_payments'])) {
                    // Delete payment allocations and the payment
                    try { $db->getPdo()->exec("DELETE FROM payment_allocations WHERE payment_id = {$ref_id}"); } catch(Exception $e2){}
                    try { $db->getPdo()->exec("DELETE FROM customer_payments WHERE id = {$ref_id}"); } catch(Exception $e2){}
                } elseif (in_array($ref_type, ['credit_order_returns'])) {
                    // Delete return items + return
                    try { $db->getPdo()->exec("DELETE FROM credit_order_return_items WHERE return_id = {$ref_id}"); } catch(Exception $e2){}
                    try { $db->getPdo()->exec("DELETE FROM credit_order_returns WHERE id = {$ref_id}"); } catch(Exception $e2){}
                } elseif (in_array($ref_type, ['credit_order_deliveries'])) {
                    // Delete delivery items + delivery
                    try { $db->getPdo()->exec("DELETE FROM credit_order_delivery_items WHERE delivery_id = {$ref_id}"); } catch(Exception $e2){}
                    try { $db->getPdo()->exec("DELETE FROM credit_order_deliveries WHERE id = {$ref_id}"); } catch(Exception $e2){}
                }
            }

            // Delete the ledger entry itself
            $db->getPdo()->exec("DELETE FROM customer_ledger WHERE id = {$entry_id}");
    ...
    if ($customer_info) {
      ...
      $opening = $db->query(
        ...
        if (!empty($ledger_entries)) {
            $last_entry = $ledger_entries[count($ledger_entries) - 1];
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="space-y-2 text-sm">
            ...
            <?php
            $available_credit = ($customer_info->credit_limit ?? 0) - ($customer_info->current_balance ?? 0);
            ?>
            <div class="flex justify-between">
