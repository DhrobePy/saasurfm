          ...
          <tr class="hover:bg-blue-50/30 transition-colors cursor-pointer"
            ...
            <td class="px-3 py-3 text-center whitespace-nowrap" onclick="event.stopPropagation()">
              <div class="flex items-center justify-center gap-1">
                <button type="button"
                        onclick="editPayment(<?php echo $pay->id; ?>, '<?php echo htmlspecialchars($pay->payment_number, ENT_QUOTES); ?>', <?php echo (int)$pay->customer_id; ?>)"
                        title="Correct payment (reverse to Recycle Bin, then re-enter)"
...
// Feature #1: "Edit" a posted payment = REVERSE it (archived to Recycle Bin,
// restorable) then re-enter the corrected one on the collect page. No silent
// in-place overwrite of financial data.
function editPayment(id, receiptNo, customerId) {
    if (!confirm('Correct payment ' + receiptNo + '?\n\nThis REVERSES the original (moved to the Recycle Bin — restorable) and takes you to re-enter the corrected payment.')) return;
    const body = new URLSearchParams({ payment_id: id, reason: 'Correction — reversed to re-enter (' + receiptNo + ')', csrf_token: CSRF_TOKEN });
    ...
    .then(data => {
        if (data.success) {
            window.location = '<?php echo url('cr/customer_payment.php'); ?>?customer_id=' + customerId;
        } else {
            alert(data.message || 'Could not reverse the payment.');
        }
    })
