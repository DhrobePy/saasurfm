...
// Backdated order_date (for reconciliation) is Superadmin/admin only — matches
// the rest of the app's convention for reconciliation-type controls (opening
// balances, ledger adjustments, purchase reconciliation are all admin-gated).
$is_admin  = in_array(($currentUser['role'] ?? ''), ['Superadmin', 'admin'], true);
$pageTitle = 'Create Credit Order';
$error = null;
$success = null;

// "Other Sales" — direct resale of Trading commodities through the Credit Sales
// pipeline (real credit_orders row, shortcut lifecycle skipping production).
ensureOtherSalesColumns();
$os_commodities = $db->query(
    "SELECT id, name, unit FROM purchase_commodities WHERE is_sellable = 1 AND status = 'active' ORDER BY name ASC"
)->results();
$os_branches = $db->query("SELECT id, name FROM branches WHERE status = 'active' ORDER BY name ASC")->results();
$os_origins_by_commodity = [];
foreach ($db->query("SELECT commodity_id, origin_name FROM purchase_commodity_origins WHERE status = 'active' ORDER BY origin_name ASC")->results() as $o) {
    $os_origins_by_commodity[(int)$o->commodity_id][] = $o->origin_name;
}
$os_pos_by_commodity = [];
foreach ($db->query(
    ...
    $os_pos_by_commodity[(int)$po->commodity_id][] = [
        'id' => (int)$po->id, 'label' => "{$po->po_number} — {$po->supplier_name} ({$po->wheat_origin}, " . date('d M Y', strtotime($po->po_date)) . ')',
    ];
}
$os_inventory_rows = $db->query("SELECT commodity_id, branch_id, origin, quantity_on_hand FROM commodity_inventory")->results();
...
$customers = $db->query(
    "SELECT c.id, c.name, c.business_name, c.phone_number, c.business_address, c.credit_limit, c.initial_due, c.status,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_order') {
    try {
        ...
        // Other Sales: direct resale of a Trading commodity through this same
        // order pipeline. Requires a source Branch/Warehouse/Dock at creation
        // time since the order skips the production step (which is normally
        // where assigned_branch_id gets set) entirely.
        $is_other_sales = !empty($_POST['is_other_sales']) ? 1 : 0;
        $os_branch_id = null;
        if ($is_other_sales) {
            $os_branch_id = (int)($_POST['os_branch_id'] ?? 0);
            if ($os_branch_id <= 0) throw new Exception('Please select the source Branch / Warehouse / Dock for this Other Sales order.');
        }
        ...
        // Feature #9: an order that breaches the customer's credit limit is BLOCKED
        // from the normal queue — it routes straight to 'escalated' (Superadmin
        // approval required) and pings Superadmin, regardless of who created it.
        // ($is_admin computed at page-top, reused here.)
        $is_over_credit_limit = $credit_limit > 0 && $balance_due > $available_credit;
        $credit_over_by       = $is_over_credit_limit ? ($balance_due - $available_credit) : 0;
        $credit_limit_note    = '';
        if ($is_over_credit_limit) {
            $credit_limit_note = sprintf(
                ' | ⚠ CREDIT LIMIT EXCEEDED — Limit: ৳%s | Current Outstanding: ৳%s | This Order: ৳%s | Over By: ৳%s | SUPERADMIN APPROVAL REQUIRED',
                number_format($credit_limit, 0),
                number_format($current_due, 0),
                number_format($balance_due, 0),
                number_format($credit_over_by, 0)
            );
        }
        $initial_status = $is_over_credit_limit ? 'escalated' : 'pending_approval';

        // Backdated order_date for reconciliation (Superadmin/admin only). Never
        // trust the POST value for non-admins — always today, regardless of what
        // was submitted (defense in depth against a tampered request). Even for
        // admins, the date must be a real calendar date and not in the future —
        // "backdated" means past, not pre-dated.
        $today = date('Y-m-d');
        $order_date = $today;
        $is_backdated = false;
        if ($is_admin && !empty($_POST['order_date'])) {
            $posted_date = trim($_POST['order_date']);
            if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $posted_date) && strtotime($posted_date) !== false) {
                if ($posted_date <= $today) {
                    $order_date   = $posted_date;
                    $is_backdated = ($order_date !== $today);
                } else {
        ...
        // Scoped to the order's OWN date (not always today), so a backdated order
        // gets a number consistent with its actual date for reconciliation.
        ...
        if ($is_backdated) {
            $credit_limit_note .= sprintf(
                ' | 📅 BACKDATED — order_date set to %s (reconciliation entry) by %s',
                $order_date, ($currentUser['display_name'] ?? 'admin')
            );
        }
        ...
        $order_insert = [
            ...
            'is_other_sales'       => $is_other_sales,
        ];
        if ($is_other_sales) {
            // Set now (not at the usual production-assignment step) since Other
            // Sales orders skip production entirely.
            $order_insert['assigned_branch_id'] = $os_branch_id;
        }
        $order_id = $db->insert('credit_orders', $order_insert);
        ...
        foreach ($items as $item) {
            if ($is_other_sales) {
                $commodity_id = (int)($item['commodity_id'] ?? 0);
                if ($commodity_id <= 0) throw new Exception('An Other Sales item is missing its commodity.');
                // Fail fast at creation, not at dispatch — mirrors trading/commodity_sale.php's check.
                $os_check_commodity = $db->query("SELECT name, inventory_account_id FROM purchase_commodities WHERE id = ? AND is_sellable = 1", [$commodity_id])->first();
                if (!$os_check_commodity) throw new Exception('Commodity not found or not marked sellable.');
                if (empty($os_check_commodity->inventory_account_id)) {
                    throw new Exception('"' . $os_check_commodity->name . '" has no Inventory account configured — set one in Purchase → Procurement Catalog before selling it.');
                }
                $shadow = getShadowProductForCommodity($commodity_id);
                $db->insert('credit_order_items', [
                    'order_id'                  => $order_id,
                    'product_id'                => $shadow['product_id'],
                    'variant_id'                => $shadow['variant_id'],
                    'commodity_id'               => $commodity_id,
                    'origin'                     => !empty($item['origin']) ? $item['origin'] : null,
                    'source_purchase_order_id'   => !empty($item['source_purchase_order_id']) ? (int)$item['source_purchase_order_id'] : null,
        ...
        $db->insert('credit_order_workflow', [
            ...
            'action' => $is_over_credit_limit ? 'escalate' : 'submit',
            'performed_by_user_id' => $user_id,
            'comments' => ($is_over_credit_limit
                ? 'Order created — BLOCKED over credit limit, escalated for Superadmin approval'
                : 'Order created and submitted for approval') . $credit_limit_note
        ]);

        $db->getPdo()->commit();

        // Feature #9: alert Superadmin when an order is blocked over the credit limit.
        if ($is_over_credit_limit) {
            try {
                if (defined('TELEGRAM_NOTIFICATIONS_ENABLED') && TELEGRAM_NOTIFICATIONS_ENABLED
                    ...
                    $tg = new TelegramNotifier(TELEGRAM_BOT_TOKEN, getTelegramChatId('orders'));
                    $tg->sendMessage(
                        "<b>🚫 ORDER OVER CREDIT LIMIT — SUPERADMIN APPROVAL NEEDED</b>\n"
                        . "───────────────────────────────\n\n"
                        . "• Order: <code>{$order_number}</code>\n"
                        . "• Customer: <b>" . htmlspecialchars($cust_rec->name) . "</b>\n"
                        . "• This order: ৳" . number_format($balance_due, 0) . "\n"
                        . "• Credit limit: ৳" . number_format($credit_limit, 0)
                        . " | Outstanding: ৳" . number_format($current_due, 0) . "\n"
                        . "• Over by: <b>৳" . number_format($credit_over_by, 0) . "</b>\n"
                        . "• Created by: " . ($currentUser['display_name'] ?? 'user') . "\n\n"
                        . "<i>Approve in Credit Sales → Approve Orders (Superadmin only).</i>"
                    );
                }
            } catch (\Throwable $te) { error_log('credit-limit Telegram: ' . $te->getMessage()); }
        }
        
        // ============================================
        // TELEGRAM NOTIFICATION (regular product orders — Other Sales uses its
        // own notification below since its items aren't in products/product_variants)
        // ============================================
        if (!$is_other_sales && defined('TELEGRAM_NOTIFICATIONS_ENABLED') && TELEGRAM_NOTIFICATIONS_ENABLED) {
            try {
                require_once '../core/classes/TelegramNotifier.php';

                $telegram = new TelegramNotifier(TELEGRAM_BOT_TOKEN, getTelegramChatId('orders'));
        ...
        // ============================================
        // OTHER SALES NOTIFICATION — distinct message, routed to the dispatch
        // group since these orders skip production and go straight toward
        // ready-to-ship (no separate "production" notification will ever fire).
        // ============================================
        if ($is_other_sales && defined('TELEGRAM_NOTIFICATIONS_ENABLED') && TELEGRAM_NOTIFICATIONS_ENABLED
            && defined('TELEGRAM_BOT_TOKEN') && defined('TELEGRAM_CHAT_ID')) {
            try {
                require_once '../core/classes/TelegramNotifier.php';
                $tg_os = new TelegramNotifier(TELEGRAM_BOT_TOKEN, getTelegramChatId('dispatch'));

                $os_branch_row = $db->query("SELECT name FROM branches WHERE id = ?", [$os_branch_id])->first();
                $os_customer = $db->query("SELECT name, phone_number FROM customers WHERE id = ?", [$customer_id])->first();

                $os_commodity_ids = array_values(array_unique(array_filter(array_map(fn($it) => (int)($it['commodity_id'] ?? 0), $items))));
                $os_commodity_names = [];
                if (!empty($os_commodity_ids)) {
                    $ph = implode(',', array_fill(0, count($os_commodity_ids), '?'));
                    foreach ($db->query("SELECT id, name, unit FROM purchase_commodities WHERE id IN ($ph)", $os_commodity_ids)->results() as $pc) {
                        $os_commodity_names[(int)$pc->id] = ['name' => $pc->name, 'unit' => $pc->unit];
                    }
                }

                $os_lines = [];
                foreach ($items as $it) {
                    $cid = (int)($it['commodity_id'] ?? 0);
                    $cinfo = $os_commodity_names[$cid] ?? ['name' => 'Commodity', 'unit' => ''];
                    $os_lines[] = "• " . htmlspecialchars($cinfo['name'])
                        . (!empty($it['origin']) ? ' (' . htmlspecialchars($it['origin']) . ')' : '')
                        . " — " . number_format((float)$it['quantity'], 3) . ' ' . htmlspecialchars($cinfo['unit'])
                        . " @ ৳" . number_format((float)$it['unit_price'], 2);
                }

                $tg_os->sendMessage(
                    "<b>🔁 OTHER SALES ORDER — Direct Commodity Resale</b>\n"
                    . "───────────────────────────────\n\n"
                    . "• Order: <code>{$order_number}</code>\n"
                    . "• Customer: <b>" . htmlspecialchars($os_customer ? $os_customer->name : 'Unknown') . "</b>"
                    . ($os_customer && $os_customer->phone_number ? " (" . htmlspecialchars($os_customer->phone_number) . ")" : "") . "\n"
                    . "• Source: <b>" . htmlspecialchars($os_branch_row ? $os_branch_row->name : 'N/A') . "</b>\n"
                    . "• Items:\n" . implode("\n", $os_lines) . "\n\n"
                    . "• Total: ৳" . number_format($total, 0)
                    . ($special_instructions !== '' ? "\n• Remarks: " . htmlspecialchars($special_instructions) : "") . "\n\n"
                    . "<i>⚡ Skips production — moves straight to Ready-to-Ship once approved.</i>"
                );
            } catch (\Throwable $te) { error_log('other-sales Telegram: ' . $te->getMessage()); }
        }

        auditLog('credit_sales', 'created',
            ($is_other_sales ? "Other Sales order {$order_number} created" : "Credit order {$order_number} created") . " — Total: ৳" . number_format($total, 2),
            ['record_id' => $order_id, 'customer_id' => $customer_id, 'total' => $total, 'items_count' => count($items), 'is_other_sales' => $is_other_sales]
        );
...
<div class="bg-white rounded-lg shadow-md p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
                Order Date
                <span class="ml-1 text-[10px] font-semibold uppercase tracking-wide text-amber-600 bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5 align-middle">Reconciliation</span>
            </label>
            <input type="date" name="order_date" id="order_date_field" class="w-full px-4 py-2 border rounded-lg"
                value="<?php echo date('Y-m-d'); ?>" max="<?php echo date('Y-m-d'); ?>">
            <p class="mt-1 text-xs text-gray-500">
                <i class="fas fa-clock-rotate-left mr-1"></i>Defaults to today. Set a past date to post a backdated order for reconciliation — the order number and reports will reflect that date.
            </p>
        </div>
        <?php endif; ?>
        ...
        <div>
            ...
            <p class="mt-1 text-xs text-gray-500">
                <i class="fas fa-info-circle mr-1"></i>Can be set freely — including a past date, to record a backdated delivery.
            </p>
        </div>
    ...
    <!-- Other Sales toggle — direct resale of a Trading commodity through this
         same order pipeline, skipping production and going straight toward
         Ready-to-Ship once approved. -->
    <div class="mt-4">
        <label class="inline-flex items-center gap-2 cursor-pointer select-none bg-purple-50 border border-purple-200 rounded-lg px-4 py-2">
            <input type="checkbox" id="os_toggle" onchange="onOtherSalesToggle()" class="accent-purple-600 w-4 h-4">
            <span class="text-sm font-semibold text-purple-800"><i class="fas fa-right-left mr-1"></i>Other Sales — direct resale of a Trading commodity (skips production, ships directly)</span>
        </label>
        <input type="hidden" name="is_other_sales" id="is_other_sales" value="0">

        <div id="os_source_block" class="hidden mt-3">
            <label class="block text-sm font-medium text-gray-700 mb-2">Branch / Warehouse / Dock (source) *</label>
            <select name="os_branch_id" id="os_branch_id" class="w-full px-4 py-2 border rounded-lg" onchange="osUpdateStockHint()">
                <option value="">Select source</option>
                <?php foreach ($os_branches as $b): ?>
                <option value="<?php echo (int)$b->id; ?>"><?php echo htmlspecialchars($b->name); ?></option>
                <?php endforeach; ?>
            </select>
            <p class="mt-1 text-xs text-gray-500">Which factory, warehouse, or unloading dock this sale ships from.</p>
        </div>
    </div>
    ...
    <div class="mt-4">
        <label class="block text-sm font-medium text-gray-700 mb-2" id="specialInstructionsLabel">Special Instructions</label>
...
<!-- Order Items -->
<div class="bg-white rounded-lg shadow-md p-6">
    <h2 class="text-xl font-bold text-gray-800 mb-4" id="orderItemsHeading">Order Items</h2>
    ...
    <div id="otherSalesItemsSection" class="hidden">
        <!-- Add Commodity Row -->
        <div class="grid grid-cols-12 gap-3 mb-4 p-4 bg-purple-50 rounded-lg border border-purple-100">
            <div class="col-span-3">
                <label class="block text-sm font-medium text-gray-700 mb-2">Commodity *</label>
                <select id="os_add_commodity" class="w-full px-3 py-2 border rounded-lg text-sm" onchange="osCommodityChanged()">
                    <option value="">Select commodity</option>
                    <?php foreach ($os_commodities as $c): ?>
                ...
                <p class="mt-1 text-[11px] text-gray-500">Manage sellable goods in <a href="../purchase/procurement_catalog.php" class="text-purple-600 hover:underline" target="_blank">Procurement Catalog</a></p>
            </div>
            <div class="col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Origin</label>
                <select id="os_add_origin" class="w-full px-3 py-2 border rounded-lg text-sm" onchange="osUpdateStockHint()">
                    <option value="">Not tracked / mixed</option>
                </select>
            </div>
            <div class="col-span-3">
                <label class="block text-sm font-medium text-gray-700 mb-2">Link to Purchase Order <span class="text-gray-400 font-normal">(optional)</span></label>
                <select id="os_add_po" class="w-full px-3 py-2 border rounded-lg text-xs">
                    <option value="">Not linked</option>
                </select>
            </div>
            <div class="col-span-1">
                <label class="block text-sm font-medium text-gray-700 mb-2">Unit</label>
                <input type="text" id="os_add_unit" class="w-full px-2 py-2 border rounded-lg bg-gray-100 text-center text-sm" readonly>
            </div>
            <div class="col-span-1">
                <label class="block text-sm font-medium text-gray-700 mb-2">Qty *</label>
                <input type="number" id="os_add_quantity" step="0.001" min="0.001" class="w-full px-2 py-2 border rounded-lg text-sm" value="1">
            </div>
            <div class="col-span-1">
                <label class="block text-sm font-medium text-gray-700 mb-2">Price *</label>
                <input type="number" id="os_add_price" step="0.01" min="0" class="w-full px-2 py-2 border rounded-lg text-sm">
            </div>
            <div class="col-span-1">
                <label class="block text-sm font-medium text-gray-700 mb-2">Disc</label>
                <div class="flex gap-1">
                    <select id="os_add_discount_type" class="w-10 px-1 py-2 border rounded-l-lg text-xs">
                        <option value="percent">%</option>
                        <option value="fixed">৳</option>
                    </select>
                    <input type="number" id="os_add_discount" step="0.01" min="0" class="w-14 px-1 py-2 border rounded-r-lg text-xs" value="0">
                </div>
            </div>
            <div class="col-span-12">
                <p id="os_stock_hint" class="text-xs mt-1 text-gray-500"></p>
            </div>
            <div class="col-span-12 flex justify-end">
                <button type="button" onclick="addOsItem()" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 font-medium text-sm">
                    <i class="fas fa-cart-plus"></i> Add Commodity
                </button>
            </div>
        </div>

        <!-- Other Sales Items Table -->
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Commodity</th>
                        <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Linked PO</th>
                        <th class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase">Qty</th>
                        <th class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase">Price</th>
                        <th class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase">Discount</th>
                        <th class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase">Total</th>
                ...
                <tbody id="osItemsBody" class="bg-white divide-y divide-gray-200">
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-gray-400">
                            <i class="fas fa-boxes-stacked text-5xl mb-3 opacity-50"></i>
                            <p class="text-lg">No commodity items yet</p>
                        </td>
                    </tr>
                </tbody>
            </table>
...
// ── Other Sales (Trading commodity resale) ──────────────────────────────────
const osOriginsByCommodity = <?php echo json_encode($os_origins_by_commodity); ?>;
const osPosByCommodity     = <?php echo json_encode($os_pos_by_commodity); ?>;
const osInventory          = <?php echo json_encode(array_map(function($r) {
    return ['commodity_id' => (int)$r->commodity_id, 'branch_id' => (int)$r->branch_id, 'origin' => $r->origin, 'qty' => (float)$r->quantity_on_hand];
}, $os_inventory_rows)); ?>;

let otherSaleItems = [];

function onOtherSalesToggle() {
    const on = document.getElementById('os_toggle').checked;
    document.getElementById('is_other_sales').value = on ? '1' : '0';
    document.getElementById('os_source_block').classList.toggle('hidden', !on);
    document.getElementById('regularItemsSection').classList.toggle('hidden', on);
    document.getElementById('otherSalesItemsSection').classList.toggle('hidden', !on);
    document.getElementById('orderItemsHeading').textContent = on ? 'Order Items — Other Sales (Trading Commodities)' : 'Order Items';
    document.getElementById('specialInstructionsLabel').textContent = on ? 'Remarks (Other Sales)' : 'Special Instructions';
    calculateTotals();
}

function osCommodityChanged() {
    const commodityId = parseInt(document.getElementById('os_add_commodity').value) || 0;
    const opt = document.getElementById('os_add_commodity').selectedOptions[0];
    document.getElementById('os_add_unit').value = opt ? (opt.dataset.unit || '') : '';

    const originSel = document.getElementById('os_add_origin');
    const origins = osOriginsByCommodity[commodityId] || [];
    originSel.innerHTML = '<option value="">Not tracked / mixed</option>' + origins.map(o => `<option value="${o}">${o}</option>`).join('');

    const poSel = document.getElementById('os_add_po');
    const pos = osPosByCommodity[commodityId] || [];
    poSel.innerHTML = '<option value="">Not linked</option>' + pos.map(p => `<option value="${p.id}">${p.label}</option>`).join('');

    osUpdateStockHint();
}

function osUpdateStockHint() {
    const commodityId = parseInt(document.getElementById('os_add_commodity').value) || 0;
    const branchId = parseInt(document.getElementById('os_branch_id').value) || 0;
    const origin = document.getElementById('os_add_origin').value;
    const unit = document.getElementById('os_add_unit').value;
    const hintEl = document.getElementById('os_stock_hint');

    if (!commodityId || !branchId) {
        hintEl.textContent = branchId ? '' : 'Select the source Branch / Warehouse / Dock above to see stock on hand.';
        hintEl.className = 'text-xs mt-1 text-gray-400';
        return;
    }
    const inv = osInventory.find(i => i.commodity_id === commodityId && i.branch_id === branchId && i.origin === origin);
    const qty = inv ? inv.qty : 0;
    hintEl.textContent = `On hand at this source${origin ? ' (' + origin + ')' : ''}: ${qty.toFixed(3)} ${unit}`;
    hintEl.className = 'text-xs mt-1 font-medium ' + (qty > 0 ? 'text-blue-600' : 'text-gray-400');
}

function addOsItem() {
    const cSel = document.getElementById('os_add_commodity');
    const branchId = parseInt(document.getElementById('os_branch_id').value) || 0;
    if (!branchId) { alert('Select the source Branch / Warehouse / Dock first.'); return; }
    if (!cSel.value) { alert('Please select a commodity.'); return; }

    const qty = parseFloat(document.getElementById('os_add_quantity').value) || 0;
    const price = parseFloat(document.getElementById('os_add_price').value) || 0;
    if (qty <= 0) { alert('Please enter a valid quantity.'); return; }
    if (price <= 0) { alert('Please enter a valid price.'); return; }

    const origin = document.getElementById('os_add_origin').value;
    const commodityId = parseInt(cSel.value);
    const cOpt = cSel.selectedOptions[0];
    const unit = document.getElementById('os_add_unit').value;
    const poId = document.getElementById('os_add_po').value;

    const inv = osInventory.find(i => i.commodity_id === commodityId && i.branch_id === branchId && i.origin === origin);
    const onHand = inv ? inv.qty : 0;
    if (qty > onHand) {
        if (!confirm(`Warning: requested quantity (${qty}) exceeds stock on hand (${onHand.toFixed(3)} ${unit}) at this source${origin ? ' (' + origin + ')' : ''}. Continue anyway?`)) return;
    }

    const discType = document.getElementById('os_add_discount_type').value;
    const discValue = parseFloat(document.getElementById('os_add_discount').value) || 0;
    const discPerUnit = discType === 'percent' ? (price * discValue / 100) : discValue;
    const discAmount = discPerUnit * qty;
    const lineTotal = qty * (price - discPerUnit);

    otherSaleItems.push({
        commodity_id: commodityId,
        commodity_name: cOpt.textContent,
        origin: origin,
        unit: unit,
        source_purchase_order_id: poId || null,
        po_label: poId ? poSelLabel(poId, commodityId) : '',
        quantity: qty,
        unit_price: price,
    ...
    document.getElementById('os_add_commodity').value = '';
    document.getElementById('os_add_origin').innerHTML = '<option value="">Not tracked / mixed</option>';
    document.getElementById('os_add_po').innerHTML = '<option value="">Not linked</option>';
    document.getElementById('os_add_unit').value = '';
    document.getElementById('os_add_quantity').value = '1';
    document.getElementById('os_add_price').value = '';
    document.getElementById('os_add_discount').value = '0';
    document.getElementById('os_stock_hint').textContent = '';
}

function poSelLabel(poId, commodityId) {
    const pos = osPosByCommodity[commodityId] || [];
    const p = pos.find(x => String(x.id) === String(poId));
    return p ? p.label : '';
}

function renderOsItems() {
    const tbody = document.getElementById('osItemsBody');
    if (otherSaleItems.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="7" class="px-4 py-8 text-center text-gray-400">
                    <i class="fas fa-boxes-stacked text-5xl mb-3 opacity-50"></i>
    ...
    tbody.innerHTML = otherSaleItems.map((item, index) => `
        <tr class="hover:bg-gray-50">
            <td class="px-3 py-3 text-sm">${item.commodity_name}${item.origin ? ` <span class="text-gray-400 text-xs">(${item.origin})</span>` : ''}</td>
            <td class="px-3 py-3 text-xs text-gray-500">${item.po_label || '—'}</td>
            <td class="px-3 py-3 text-sm text-right">
                <input type="number" value="${item.quantity}" min="0.001" step="0.001"
                       onchange="updateOsQuantity(${index}, this.value)"
            ...
            <td class="px-3 py-3 text-sm text-right text-red-600">৳${item.discount.toFixed(2)}</td>
            <td class="px-3 py-3 text-sm text-right font-bold text-green-600">৳${item.line_total.toFixed(2)}</td>
            <td class="px-3 py-3 text-center">
                <button type="button" onclick="removeOsItem(${index})" class="text-red-600 hover:text-red-900">
...
function updateOsQuantity(index, newQty) {
    const qty = parseFloat(newQty) || 0;
    const item = otherSaleItems[index];
    const discPerUnit = item.discount_type === 'percent' ? (item.unit_price * item.discount_value / 100) : item.discount_value;
    item.quantity = qty;
    item.discount = discPerUnit * qty;
    item.line_total = qty * (item.unit_price - discPerUnit);
    renderOsItems();
    calculateTotals();
}

function removeOsItem(index) {
    otherSaleItems.splice(index, 1);
    renderOsItems();
    calculateTotals();
}
...
function calculateTotals() {
    const isOtherSales = document.getElementById('os_toggle').checked;
    const activeItems = isOtherSales ? otherSaleItems : orderItems;

    // Calculate subtotal (before any discounts). Regular items use effective_price
    // (includes mini-truck surcharge); Other Sales items use their manual unit_price.
    const subtotal = activeItems.reduce((sum, item) => sum + ((isOtherSales ? item.unit_price : item.effective_price) * item.quantity), 0);

    // Calculate item-level discounts
    const itemDiscount = activeItems.reduce((sum, item) => sum + item.discount, 0);
    ...
    document.getElementById('items_json').value = JSON.stringify(activeItems);
    ...
    if (matches.length === 0) {
        ...
        dropdown.innerHTML = matches.map(c => {
            ...
            return `<div class="px-4 py-2 hover:bg-blue-50 cursor-pointer text-sm border-b border-gray-100"
                ...
                ${c.address ? `<div class="text-gray-400 text-[11px] mt-0.5"><i class="fas fa-map-marker-alt mr-1"></i>${c.address}</div>` : ''}
            </div>`;
        }).join('') + newCustBtn;
    }
...
document.getElementById('creditOrderForm').addEventListener('submit', function(e) {
    const isOtherSales = document.getElementById('os_toggle').checked;
    if (isOtherSales) {
        if (!document.getElementById('os_branch_id').value) {
            e.preventDefault();
            alert('Please select the source Branch / Warehouse / Dock for this Other Sales order.');
            return false;
        }
        if (otherSaleItems.length === 0) {
            e.preventDefault();
            alert('Please add at least one commodity item to the order');
            return false;
        }
