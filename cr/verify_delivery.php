...
$delivery_id = (int)($_GET['delivery_id'] ?? ($_POST['delivery_id'] ?? 0));
$error = null; $flash = null; $order = null; $delivery = null;
...
if ($inv === '' || $sig === '') {
    ...
    } elseif (!hash_equals(deliveryQrSignature($order->order_number, $delivery_id ?: null), $sig)) {
        // The signature covers delivery_id too, so a tampered URL (swapped or
        // stripped delivery_id) fails here even with an otherwise-valid sig.
        $error = 'Invalid or altered QR code — this is not a genuine dispatch slip.';
        $order = null;
    } elseif ($delivery_id > 0) {
        $delivery = $db->query("SELECT * FROM credit_order_deliveries WHERE id = ? AND order_id = ?", [$delivery_id, $order->id])->first();
        if (!$delivery) { $error = 'Delivery record not found for this order.'; $order = null; }
    }
}
$is_delivery_scan = $order && $delivery !== null;

$conf = null; $items = []; $assigned_driver = ''; $assigned_vehicle = ''; $dispatch_ok = false;
// NULL-safe lookup (order_id <=> ? AND delivery_id <=> ?) — a whole-order scan
// (delivery_id NULL) and each partial delivery's own scan are separate rows,
// so one order can carry many confirmations, one per trip.
$loadState = function () use ($db, &$order, &$conf, $is_delivery_scan, $delivery_id) {
    $conf = $order ? $db->query(
        "SELECT * FROM cr_delivery_confirmations WHERE order_id = ? AND delivery_id <=> ?",
        [$order->id, $is_delivery_scan ? $delivery_id : null]
    )->first() : null;
};
if ($order) {
    $loadState();
    if ($is_delivery_scan) {
        $assigned_driver  = $delivery->driver_name  ?? '';
        $assigned_vehicle = $delivery->truck_number ?? '';
        $items = $db->query(
    ...
    $dispatch_ok = orderDispatchAllowed((int)$order->id);   // dispatch hold cleared? (order-level — applies to every trip)
}
...
if ($order && !$gate_done && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'confirm_gate') {
    ...
    if ($blocked_status) {
        ...
        if ($gate_driver === '' || $gate_vehicle === '') {
            ...
            try {
                // Explicit check-then-insert-or-update — no longer relying on
                // ON DUPLICATE KEY UPDATE, since the table's uniqueness is now
                // (order_id, delivery_id) enforced here in application logic
                // rather than a DB constraint (see ensureDeliveryConfirmTable()).
                $existing = $db->query(
                    "SELECT id FROM cr_delivery_confirmations WHERE order_id = ? AND delivery_id <=> ?",
                    [$order->id, $is_delivery_scan ? $delivery_id : null]
                )->first();
                if ($existing) {
                    $db->query(
                        "UPDATE cr_delivery_confirmations
                         SET gate_out_at = ?, gate_out_by_user_id = ?, gate_out_by_name = ?,
                             driver_name = ?, vehicle_number = ?, gate_note = ?
                         WHERE id = ?",
                        [date('Y-m-d H:i:s'), $user_id, $user_name, $gate_driver, $gate_vehicle, $gate_note ?: null, $existing->id]
                    );
                } else {
                    $db->insert('cr_delivery_confirmations', [
                        'order_id'            => $order->id,
                        'delivery_id'         => $is_delivery_scan ? $delivery_id : null,
                        'order_number'        => $order->order_number,
                        'gate_out_at'         => date('Y-m-d H:i:s'),
                        'gate_out_by_user_id' => $user_id,
                        'gate_out_by_name'    => $user_name,
                        'driver_name'         => $gate_driver,
                        'vehicle_number'      => $gate_vehicle,
                        'gate_note'           => $gate_note ?: null,
                    ]);
                }
                ...
                // — scoped to THIS delivery's own row for a partial trip, since an
                // order can have a different truck per trip; the order-wide shipping
                // record only applies to the legacy whole-order flow.
                if ($gate_driver !== $assigned_driver || $gate_vehicle !== $assigned_vehicle) {
                    try {
                        if ($is_delivery_scan) {
                            $db->query(
                                "UPDATE credit_order_deliveries SET driver_name = ?, truck_number = ? WHERE id = ?",
                                [$gate_driver, $gate_vehicle, $delivery_id]
                            );
                        } else {
                            $db->query(
                    ...
                    } catch (Exception $e) { /* shipping/delivery row may not exist yet — non-fatal */ }
                }
                // Physically left the gate → mark shipped if it hasn't been already.
                // Feature #8: the gate scan is what marks the order SHIPPED
                // (goods_on_board → shipped). Fallback covers pre-migration orders.
                if (in_array($order->status, ['goods_on_board', 'approved', 'in_production', 'produced', 'ready_to_ship'])) {
                ...
                auditLogOrder('gate_out', (int)$order->id, $order->order_number,
                    ...
                    . ($is_delivery_scan ? " for delivery {$delivery->delivery_number}" : ''),
                    ['driver' => $gate_driver, 'vehicle' => $gate_vehicle, 'delivery_id' => $is_delivery_scan ? $delivery_id : null]);
    ...
    try {
        $db->query(
            "UPDATE cr_delivery_confirmations
             ...
             WHERE order_id = ? AND delivery_id <=> ? AND confirmed_at IS NULL",
            [$user_id, $user_name, $received_by ?: null, $note ?: null, $order->id, $is_delivery_scan ? $delivery_id : null]
        );
        if ($is_delivery_scan) {
            // Whether the ORDER is now fully delivered depends on cumulative
            // partial deliveries, not just this one trip — same decision
            // partial_delivery.php makes, via the same shared helper, so both
            // entry points agree (see updateOrderDeliveryStatus() in helpers.php).
            $order->status = updateOrderDeliveryStatus($db, (int)$order->id);
        ...
        auditLogOrder('delivery_confirmed', (int)$order->id, $order->order_number,
            ...
            . ($is_delivery_scan ? " for delivery {$delivery->delivery_number} (order now: {$order->status})" : ''),
            ['driver' => $assigned_driver, 'vehicle' => $assigned_vehicle, 'delivery_id' => $is_delivery_scan ? $delivery_id : null]);
        $flash = $is_delivery_scan
            ? 'Delivery confirmed for this trip.' . ($order->status === 'delivered' ? ' All goods now delivered — order closed.' : ' Order remains open for further deliveries.')
            : 'Delivery confirmed. This order is now locked.';
...
/* ─── POST 3: VOID a mistaken gate pass (Superadmin/admin only, before
     delivery is confirmed) — e.g. wrong driver/vehicle entered, or scanned
     prematurely. Archives via the recycle bin (same pattern used everywhere
     else in this app) rather than a hard delete, so it stays recoverable, and
     clears the row so the gate stage can be re-scanned cleanly. Deliberately
     NOT offered once delivery is confirmed — undoing a completed delivery
     touches accounting/status and is a separate, bigger action outside this
     feature's scope. ────────────────────────────────────────────────────── */
if ($order && $conf && $gate_done && !$delivered && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'void_gate') {
    if (!in_array($_SESSION['user_role'] ?? '', ['Superadmin', 'admin'], true)) {
        $error = 'Only Superadmin/Admin can void a gate pass.';
    } else {
        try {
            $batch = recycleBegin(
                'gate_pass_confirmation',
                'Voided gate pass — ' . $order->order_number . ($is_delivery_scan ? " (delivery {$delivery->delivery_number})" : ''),
                null
            );
            $n = recycleArchiveDelete($batch, 'cr_delivery_confirmations', 'id', $conf->id);
            if ($n < 1) throw new Exception('Nothing to void.');
            auditLogOrder('gate_pass_voided', (int)$order->id, $order->order_number,
                'Gate pass voided by ' . $user_name . ($is_delivery_scan ? " for delivery {$delivery->delivery_number}" : ''),
                ['delivery_id' => $is_delivery_scan ? $delivery_id : null]);
            $flash = 'Gate pass voided — this can be re-scanned from the gate stage.';
            $loadState();
            $gate_done = $conf && !empty($conf->gate_out_at);
        } catch (Exception $e) {
            $error = 'Could not void: ' . $e->getMessage();
        }
    }
}
...
// Feature #9: log this scan (GET = an actual scan). A scan on an already-delivered
// order is flagged as reuse and pings admins via Telegram (inside recordQrScan).
$scan_total = ($order && $_SERVER['REQUEST_METHOD'] === 'GET')
    ? recordQrScan((int)$order->id, $order->order_number, $stage, $user_name)
    : 0;
$is_reuse_scan = ($order && $_SERVER['REQUEST_METHOD'] === 'GET' && $stage === 'done');
...
<div class="bg-white rounded-2xl shadow-lg overflow-hidden">
    ...
    <?php /* ── Order facts ── */ ?>
    <?php if ($is_delivery_scan): ?>
    <div class="px-6 pt-3 -mb-1">
        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold">
            <i class="fas fa-truck"></i>Partial Delivery <?php echo htmlspecialchars($delivery->delivery_number); ?><?php echo $delivery->is_final ? ' (FINAL)' : ''; ?>
        </span>
    </div>
    <?php endif; ?>
    <div class="px-6 py-4 border-t border-gray-100 space-y-2 text-sm">
        <?php if ($is_delivery_scan): ?>
        <div class="flex justify-between"><span class="text-gray-500">Gate Pass No.</span><span class="font-mono font-bold"><?php echo htmlspecialchars($delivery->delivery_number); ?></span></div>
        <?php endif; ?>
        <div class="flex justify-between"><span class="text-gray-500"><?php echo $is_delivery_scan ? 'Order No.' : 'Gate Pass No. (Order)'; ?></span><span class="font-mono font-bold"><?php echo htmlspecialchars($order->order_number); ?></span></div>
    ...
    <?php if ($stage === 'gate'): ?>
        ...
        <form method="POST" class="px-6 py-4 border-t border-gray-100 space-y-3"
            ...
            <?php if ($is_delivery_scan): ?><input type="hidden" name="delivery_id" value="<?php echo (int)$delivery_id; ?>"><?php endif; ?>
            <div>
    ...
    <?php elseif ($stage === 'delivery'): ?>
        ...
        <?php if (in_array($_SESSION['user_role'] ?? '', ['Superadmin', 'admin'], true)): ?>
        <form method="POST" class="px-6 pb-4"
              onsubmit="return confirm('Void this gate pass? The goods have NOT been marked delivered yet, so this just undoes the gate-out — e.g. wrong driver/vehicle entered. It can be re-scanned afterward.');">
            <input type="hidden" name="action" value="void_gate">
            <input type="hidden" name="inv" value="<?php echo htmlspecialchars($order->order_number); ?>">
            <input type="hidden" name="sig" value="<?php echo htmlspecialchars($sig); ?>">
            <?php if ($is_delivery_scan): ?><input type="hidden" name="delivery_id" value="<?php echo (int)$delivery_id; ?>"><?php endif; ?>
            <button type="submit" class="w-full py-2 border border-red-300 text-red-600 text-xs font-semibold rounded-lg hover:bg-red-50"><i class="fas fa-rotate-left mr-1"></i>Void Gate Pass (undo — Superadmin/Admin)</button>
        </form>
        <?php endif; ?>
