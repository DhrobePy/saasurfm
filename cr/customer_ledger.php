        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <?php foreach ($ledger_entries as $entry): 
                    ...
                    // Route to the designated transaction view page — only for
                    // reference types that actually have one. Adjustments/opening
                    // balances/amendments are ledger-internal notes with no
                    // separate detail page to route to.
                    $tx_link = null;
                    if ((int)($entry->reference_id ?? 0) > 0) {
                        if ($entry->reference_type === 'credit_orders') {
                            $tx_link = url('cr/credit_invoice_print.php?id=' . (int)$entry->reference_id);
                        } elseif ($entry->reference_type === 'customer_payments') {
                            $tx_link = url('cr/credit_payment_receipt.php?id=' . (int)$entry->reference_id);
                        }
                    }
                ?>
                <tr class="hover:bg-gray-50<?php echo $tx_link ? ' cursor-pointer' : ''; ?>"
                    <?php if ($tx_link): ?>onclick="if(!event.target.closest('form,button,a')) window.open('<?php echo $tx_link; ?>','_blank')"<?php endif; ?>>
                    ...
                    <td class="px-4 py-3 text-sm whitespace-nowrap font-medium">
                        <?php if ($tx_link): ?>
                        <a href="<?php echo $tx_link; ?>" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline" title="View transaction">
                            <?php echo htmlspecialchars($entry->invoice_number ?? '-'); ?>
                            <i class="fas fa-external-link-alt text-[9px] ml-0.5"></i>
                        </a>
                        <?php else: ?>
                        <span class="text-gray-700"><?php echo htmlspecialchars($entry->invoice_number ?? '-'); ?></span>
                        <?php endif; ?>
                    </td>
