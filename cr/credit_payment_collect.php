...
$allowed_roles = ['Superadmin', 'admin', 'Accounts', 'collector'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'collect_payment') {
    ...
    // Same 'collect_payment' key as customer_payment.php; no legacy fallback.
    $collect_cap          = $is_admin ? null : getUserActionLimit((int)($currentUser['id'] ?? 0), 'collect_payment');
    $no_limit_configured  = !$is_admin && $collect_cap === null;
    $over_collect_limit   = $collect_cap !== null && $payment_amount > $collect_cap;
    ...
    if ($customer_id <= 0) {
    ...
    } elseif (!$is_admin && (paymentApprovalRequiredForAll() || $over_collect_limit || $no_limit_configured)) {
        // Feature #6 / maker-checker: park as a pending request instead of posting.
        // When the global "approval for all payments" policy is ON every non-admin
        // receipt queues; otherwise receipts over the user's collect_payment limit
        // queue. "No limit configured" is NOT unlimited trust — nobody has explicitly
        // delegated this user a ceiling yet, so their receipts always queue until an
        // admin sets one in Privileges → Action Limits.
        // Payload uses customer_payment.php field names so the checker reviews it there.
        $q_cust = $db->query("SELECT name FROM customers WHERE id = ?", [$customer_id])->first();
        $req_id = submitPendingRequest('payment', $payment_amount, [
            'customer_id'      => $customer_id,
            'customer_name'    => $q_cust->name ?? '',
            'payment_date'     => $payment_date,
            'payment_method'   => $payment_method,
            'bank_account_id'  => $bank_account_id,
            'reference_number' => $reference_number !== '' ? $reference_number : $cheque_number,
            'notes'            => $notes,
            ...
            'customer_id' => $customer_id,
            'summary'     => 'Field receipt ৳' . number_format($payment_amount, 0) . ' from ' . ($q_cust->name ?? 'customer')
                           . ' (' . $payment_method . ')',
            'maker_limit' => $collect_cap,
        ]);
        if ($req_id) {
            $reason = $over_collect_limit
            ...
            auditLogPayment('queued', $req_id, 'REQ-' . $req_id,
                'Field payment of ৳' . number_format($payment_amount, 2) . ' queued for approval (' . $reason . ') by '
                . ($currentUser['display_name'] ?? 'user'));
            header('Location: credit_payment_collect.php?queued=' . $req_id . '&amt=' . urlencode(number_format($payment_amount, 0)));
            exit();
        }
        $error = "Could not queue this receipt for approval. Please try again.";
        ...
        try {
            ...
            // Generate payment receipt number — sequence-based (MAX-existing+1),
            // not rand(1,9999). Both columns are UNIQUE; a random 4-digit value
            // with no collision retry could clash on any busy day even without
            // any row ever being deleted. Same fix as cr/customer_payment.php's
            // already-safe payment_number generation.
            $pay_date_prefix = date('Ymd');
            $last_receipt = $db->query(
                "SELECT receipt_number FROM customer_payments WHERE receipt_number LIKE ? ORDER BY id DESC LIMIT 1",
                ["RCP-{$pay_date_prefix}-%"]
            )->first();
            $receipt_seq = $last_receipt ? ((int)substr($last_receipt->receipt_number, -4) + 1) : 1;
            $receipt_number = sprintf("RCP-%s-%04d", $pay_date_prefix, $receipt_seq);
            ...
            // Compute running balance from aggregate — immune to balance_after chain drift.
            ...
            $agg_cpc_td = (float)($agg_cpc->td ?? 0);
            $agg_cpc_tc = (float)($agg_cpc->tc ?? 0);
            $prev_balance = ($agg_cpc_td > 0 || $agg_cpc_tc > 0)
                ? $agg_cpc_td - $agg_cpc_tc
                : (float)($customer->initial_due ?? 0);
            ...
            foreach ($invoice_allocations as $order_id => $alloc_amount) {
                ...
                if ($alloc_amount > 0) {
                    ...
                    // Use balance_due = balance_due - ? (not total_amount - (amount_paid + ?))
                    // because MySQL evaluates SET left-to-right: the second expression would
                    // see the already-incremented amount_paid, causing a double-subtract.
                    $db->query(
            ...
            // ── Bridge: mirror to bank_transactions so this payment appears in
            //    the Bank Statement module.  Matched by account_number between
            //    chart_of_accounts and bank_tx_accounts.  Fails silently.
            if (in_array($payment_method, ['Bank Transfer', 'Cheque']) && $bank_account_id) {
                try {
                    require_once dirname(__DIR__) . '/bank/BankManager.php';
                    $bm_bridge = $db->query(
                        "SELECT bta.id AS bta_id
                         FROM bank_tx_accounts bta
                         INNER JOIN chart_of_accounts coa ON coa.account_number = bta.account_number
                         WHERE coa.id = ? AND bta.status = 'active'
                         LIMIT 1",
                        [$bank_account_id]
                    )->first();
                    if ($bm_bridge) {
                        ...
                        $bankMgr->createTransaction([
                            ...
                            'cheque_number'      => $cheque_number ?: null,
                            'payee_payer_name'   => $customer->name ?? null,
                            'description'        => "Credit sale payment — {$customer->name} — Receipt #{$receipt_number}",
                        ], $user_id, $currentUser['display_name'] ?? ($currentUser['username'] ?? 'System'));
                    }
                } catch (\Throwable $bm_ex) {
                    error_log("BankManager bridge (credit payment collect): " . $bm_ex->getMessage());
                }
            }
            ...
            try {
                if (defined('TELEGRAM_NOTIFICATIONS_ENABLED') && TELEGRAM_NOTIFICATIONS_ENABLED) {
                    ...
                    if ($user_branch) {
                        $branch_info = $db->query("SELECT name FROM branches WHERE id = ?", [$user_branch])->first();
                    ...
                    $paymentData = [
                        ...
                        'payment_type' => empty($allocated_invoices) ? 'Advance Payment' : 'Invoice Payment',
                    ...
                    if ($result['success']) {
                        error_log("✓ Telegram payment notification sent for receipt: $receipt_number");
                    } else {
                        error_log("✗ Telegram notification failed: " . json_encode($result['response']));
                    }
                }
            } catch (Exception $e) {
                error_log("✗ Telegram payment notification error: " . $e->getMessage());
            }
...
<div class="bg-amber-50 border-l-4 border-amber-500 text-amber-800 p-4 mb-6 rounded-r-lg">
    <p class="font-bold"><i class="fas fa-hourglass-half mr-1"></i>Sent for Approval</p>
    <p class="text-sm mt-1">
        This receipt (৳<?php echo htmlspecialchars($_GET['amt'] ?? ''); ?>) is above your collection limit,
        so it was queued as request #<?php echo (int)$_GET['queued']; ?>.
        It will post to the ledger once a senior officer approves it on the
        <a href="approval_requests.php" class="underline font-semibold">Approval Requests</a> page.
    </p>
</div>
<?php endif; ?>
