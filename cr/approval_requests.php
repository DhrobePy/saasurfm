...
$pending = $db->query(
    "SELECT r.*, c.name AS customer_name, co.order_number,
     ...
     WHERE r.status = 'pending' AND r.request_type NOT IN ('pos_credit_sale', 'pos_exit_release')
...
$decided = $db->query(
    "SELECT r.*, c.name AS customer_name, co.order_number,
     ...
     WHERE r.status != 'pending' AND r.request_type NOT IN ('pos_credit_sale', 'pos_exit_release') AND r.created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)
