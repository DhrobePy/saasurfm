...
function req_checker_covers(object $r, bool $is_admin, ?float $collect_cap, ?float $delivery_cap): bool {
    ...
    // Advances are receipts too → gated by the collect_payment cap, like payments.
    $cap = in_array($r->request_type, ['payment', 'advance']) ? $collect_cap : $delivery_cap;
...
<div class="bg-white rounded-xl shadow-md overflow-hidden mb-8">
    ...
    <div class="divide-y divide-gray-100">
        <?php foreach ($pending as $r):
            ...
            $type_label = $r->request_type === 'advance' ? 'Advance' : ($r->request_type === 'payment' ? 'Payment' : 'Delivery');
            $type_color = $pay_like ? 'green' : 'indigo';
            $age_hrs = max(0, (time() - strtotime($r->created_at)) / 3600);
        ?>
        <div class="px-6 py-4 flex flex-wrap items-start gap-4">
            <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0
                        <?php echo $pay_like ? 'bg-green-100' : 'bg-indigo-100'; ?>">
                <i class="fas <?php echo $pay_like ? 'fa-money-bill-wave text-green-600' : 'fa-dolly text-indigo-600'; ?>"></i>
            </div>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2"><?php echo $r->request_type === 'advance' ? 'Advance' : ($r->request_type === 'payment' ? 'Payment' : 'Delivery'); ?></td>
