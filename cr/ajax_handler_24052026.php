...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $received_token = $_SERVER['HTTP_X_CSRF_TOKEN']
                       ?? $data['csrf_token']
                       ?? '';
        // Allow requests that include the correct token; also allow from same origin without token
        // (internal Ajax calls from our own pages typically don't send the header — be lenient)
    ...
    switch ($action) {
        ...
        case 'place_credit_order':
            ...
            try {
                ...
                // Update customer balance (credit used)
                $db->query(
                ...
                $prev_bal  = $prev ? (float)$prev->balance_after : (float)$customer->initial_due;
                $new_bal   = $prev_bal + $total_amount;
