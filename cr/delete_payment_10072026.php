...
try {
    ...
    foreach ($allocations as $alloc) {
        ...
        // MySQL evaluates SET left-to-right, so `amount_paid` in the balance_due
        // expression is already the restored value — giving the correct identity:
        //   balance_due = total_amount − GREATEST(0, old_amount_paid − alloc)
        $db->query(
    ...
    $db->query("DELETE FROM payment_allocations WHERE payment_id = ?", [$payment_id]);
    ...
    if (!empty($ledger_entries)) {
        ...
        foreach ($ledger_entries as $le) {
            $db->query("DELETE FROM customer_ledger WHERE id = ?", [$le->id]);
        }
    ...
    foreach ($journals as $je) {
        $db->query("DELETE FROM transaction_lines WHERE journal_entry_id = ?", [$je->id]);
        $db->query("DELETE FROM journal_entries WHERE id = ?", [$je->id]);
    }

    // ── 6. Void pending bank_transactions entry created by bridge ─────────────
    //    Match by reference_number = payment_number (set by both payment pages).
    $db->query(
    ...
    $db->query("DELETE FROM customer_payments WHERE id = ?", [$payment_id]);
    ...
    auditLog(
        'customer_payments',
        'deleted',
        "Payment {$payment_number} (৳" . number_format($pay_amount, 2) . ") for {$pay->customer_name} deleted by {$deletedBy}{$reasonNote}",
        [
            'severity'          => 'critical',
    ...
    exit(json_encode([
        'success'        => true,
        'message'        => "Payment {$payment_number} deleted and all related records reversed.",
        'payment_number' => $payment_number,
    ]));
