...
// Delivery-scoped gate pass: a specific partial-delivery trip, not the whole
// order. Items/driver/vehicle/date come from THAT delivery, never the full
// order — the same principle already applied to credit_invoice_print.php.
$delivery = null;
if ($delivery_id > 0) {
    $delivery = $db->query("SELECT * FROM credit_order_deliveries WHERE id = ? AND order_id = ?", [$delivery_id, $order_id])->first();
    if (!$delivery) { die('Delivery record not found for this order.'); }
}
$is_delivery_slip = $delivery !== null;
...
$conf = $db->query(
    "SELECT gate_out_at, gate_out_by_name, confirmed_at, confirmed_by_name FROM cr_delivery_confirmations WHERE order_id = ? AND delivery_id <=> ?",
    [$order_id, $is_delivery_slip ? $delivery_id : null]
)->first();

// Signed QR → the login-gated confirm page. Omitting delivery_id for a
// whole-order slip reproduces the exact legacy URL/signature unchanged.
$qr_sig = deliveryQrSignature((string)$order->order_number, $is_delivery_slip ? $delivery_id : null);
$qr_url = (defined('APP_URL') ? rtrim(APP_URL, '/') : '')
        . '/cr/verify_delivery.php?inv=' . urlencode((string)$order->order_number) . '&sig=' . $qr_sig
        . ($is_delivery_slip ? '&delivery_id=' . $delivery_id : '');
...
<div class="slip">
  <div class="top">
    ...
    <div class="doc-title">
      <div class="t">GATE PASS <span style="font-weight:600;font-size:12px;color:#6b7280;">/ <?php echo $is_delivery_slip ? 'PARTIAL DELIVERY' : 'DISPATCH SLIP'; ?></span></div>
      <div style="font-size:10px;color:#9ca3af;text-transform:uppercase;letter-spacing:.5px;margin-top:4px;">Gate Pass No.</div>
      <div class="n"><?php echo htmlspecialchars($is_delivery_slip ? $delivery->delivery_number : $order->order_number); ?></div>
      <?php if ($is_delivery_slip): ?>
      <div style="font-size:11px;color:#6b7280;margin-top:2px;">Order: <?php echo htmlspecialchars($order->order_number); ?><?php echo $delivery->is_final ? ' · FINAL DELIVERY' : ''; ?></div>
      <?php endif; ?>
