...
restrict_access($allowed_roles, 'credit_sales', 'over_delivery');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_od') {
    if (!$can_record) {
        ...
        try {
            ...
            $order = $db->query(
                "SELECT co.* FROM credit_orders co WHERE co.id = ? AND co.status IN ('shipped','delivered')",
                [$order_id]
            )->first();
            if (!$order) throw new Exception('Order not found or not eligible (must be shipped/delivered).');
            ...
            $success = "Over-delivery #{$od_number} recorded" . ($auto_approve ? ' and approved.' : '. Awaiting approval.');
            $selected_order_id = 0;
            $selected_order    = null;
            $order_items       = [];
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
    ...
    <p class="text-sm text-gray-500 mb-5">No shipped/delivered orders matched your search.</p>
    <?php endif; ?>
