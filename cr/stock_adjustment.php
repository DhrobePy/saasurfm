...
// Feature #2 — this page is the "Stock Adjustments" tab of returns_center.php.
// Direct hits redirect into the hub; when embedded (CR_EMBED) the hub gated it.
if (!defined('CR_EMBED')) {
    $qs = $_GET; $qs['tab'] = 'adjust';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_adjustment') {
    try {
        ...
        if (!$adj_id) throw new Exception('Failed to record the adjustment.');
        ...
        try {
            ...
            if (($_POST['action']) === 'reject_adjustment') {
                $db->query("UPDATE cr_stock_adjustments SET status='rejected', approved_by_user_id=?, approved_at=NOW() WHERE id=?",
                           [$user_id, $adj_id]);
                if ($db->error()) throw new Exception('Failed to mark the adjustment as rejected.');
                ...
                // Every write below was previously unchecked — a silently swallowed
                // failure (Database::query() never throws) let this transaction still
                // commit with the adjustment marked 'approved' and "inventory updated
                // and journal posted" reported, even if the inventory write, the journal,
                // or the status flip itself silently failed — a book-vs-warehouse desync
                // with no trail pointing back to this adjustment.
                ...
                if ($db->error()) throw new Exception('Failed to update the inventory quantity for this adjustment.');
                ...
                if ($inv_acct && $off_acct && $val > 0) {
                    ...
                    if ($db->error()) throw new Exception('Failed to create the journal entry for this adjustment.');
                    ...
                    if ($db->error()) throw new Exception('Failed to post the debit line of the adjustment journal entry.');
                    ...
                    if ($db->error()) throw new Exception('Failed to post the credit line of the adjustment journal entry.');
                }
                ...
                if ($db->error()) throw new Exception('Failed to mark the adjustment as approved.');
                $pdo->commit();
...
<div class="<?php echo defined('CR_EMBED') ? '' : 'max-w-screen-xl mx-auto px-4 sm:px-6 py-6'; ?>">
