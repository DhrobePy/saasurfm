...
function dsConditions(
    ...
    // 'shipped' = partially delivered (a trip went out via partial_delivery.php
    // but the order isn't fully complete yet — updateOrderDeliveryStatus() only
    // moves it to 'delivered' once everything's out). Without it here, an
    // overdue partial shipment would silently vanish from both tabs the moment
    // the first truck left — exactly the gap a partial ACI order surfaced live.
    $where  = ["co.status IN ('approved','in_production','produced','ready_to_ship','shipped')", 'co.assigned_branch_id IS NOT NULL', "co.required_date {$boundary_op} ?"];
    $params = [$today];
...
// Payment/dispatch hold — so production managers can see at a glance that an
// order due today is stuck at Accounts regardless of production progress.
// Per-order (not a bulk query) since the underlying condition types need
// per-customer outstanding/payment sums — same trusted logic Payment Watch
// and the gate-pass scan already use, just surfaced here too.
$hold_reason_by_order = [];
foreach (array_merge($schedule_orders, $overdue_orders) as $o) {
...
renderDeliveryBoard($schedule_orders, 'schedule', false, $is_admin, $items_by_order, $status_cls, $status_labels, $day_palette, $vehicle_badge, $day_label, $today, $hold_reason_by_order);
echo '</div>';

echo '<div class="' . ($active_tab !== 'overdue' ? 'hidden' : '') . '">';
renderDeliveryBoard($overdue_orders, 'overdue', true, $is_admin, $items_by_order, $status_cls, $status_labels, $day_palette, $vehicle_badge, $day_label, $today, $hold_reason_by_order);
echo '</div>';
?>
