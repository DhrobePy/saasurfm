...
// Feature #1: a posted payment may be reversed by Superadmin/Admin, OR by anyone
// granted Payment History → Delete / Reverse Payment in Privileges. Every reversal
// is archived to the Recycle Bin (restorable).
$currentUser = getCurrentUser();
$__is_admin_dp = in_array(($currentUser['role'] ?? ''), ['Superadmin', 'admin'], true);
if (!$__is_admin_dp && !userCanPageAction('credit_sales', 'payment_history', 'can_delete')) {
    restrict_access(['Superadmin', 'admin']); // denies + redirects (no delete grant)
}
...
try {
    ...
    if (!empty($ledger_entries)) {
        ...
        // Previously unchecked — a mid-loop failure (plausible on a busy ledger with
        // many subsequent rows) would leave every balance_after from that point
        // forward permanently stale, with no other code path that re-derives and
        // fixes this column, while the delete still reports success.
    ...
    if ($db->error()) throw new Exception('Failed to sync the customer cached balance after this delete.');
    ...
    if (defined('TELEGRAM_NOTIFICATIONS_ENABLED') && TELEGRAM_NOTIFICATIONS_ENABLED) {
        try {
            ...
            $notifier = new TelegramNotifier(TELEGRAM_BOT_TOKEN, getTelegramChatId('payment_received'));
