...
function req_checker_covers(object $r, bool $is_admin, ?float $collect_cap, ?float $delivery_cap): bool {
    if ($is_admin) return true;
    $cap = $r->request_type === 'payment' ? $collect_cap : $delivery_cap;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['req_action'])) {
    ...
    try {
        ...
        if ($_POST['req_action'] === 'reject') {
            ...
            if (!$can_reject && !req_checker_covers($req, $is_admin, $my_collect_cap, $my_delivery_cap)) {
...
<div class="bg-white rounded-xl shadow-md overflow-hidden mb-8">
    ...
    <div class="divide-y divide-gray-100">
        <?php foreach ($pending as $r):
            $covers = req_checker_covers($r, $is_admin, $my_collect_cap, $my_delivery_cap);
            $is_mine = (int)$r->maker_user_id === $user_id;
            $review_url = $r->request_type === 'payment'
                ? 'customer_payment.php?pending_req=' . (int)$r->id
                : 'partial_delivery.php?order_id=' . (int)$r->order_id . '&pending_req=' . (int)$r->id;
            $age_hrs = max(0, (time() - strtotime($r->created_at)) / 3600);
        ?>
        <div class="px-6 py-4 flex flex-wrap items-start gap-4">
            <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0
                        <?php echo $r->request_type === 'payment' ? 'bg-green-100' : 'bg-indigo-100'; ?>">
                <i class="fas <?php echo $r->request_type === 'payment' ? 'fa-money-bill-wave text-green-600' : 'fa-dolly text-indigo-600'; ?>"></i>
            </div>

            <div class="flex-1 min-w-0">
                <p class="font-semibold text-gray-900 text-sm">
                    #<?php echo (int)$r->id; ?> —
                    <?php echo $r->request_type === 'payment' ? 'Payment' : 'Delivery'; ?>
                    <span class="font-bold text-<?php echo $r->request_type === 'payment' ? 'green' : 'indigo'; ?>-700">
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2"><?php echo $r->request_type === 'payment' ? 'Payment' : 'Delivery'; ?></td>
