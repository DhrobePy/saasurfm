...
// Self-migrating: price revision history, shown on credit_order_view.php.
// DDL before any transaction (implicit-commit rule).
try { $db->getPdo()->exec("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'update_order') {
        try {
            ...
            // Detect price revisions against the current DB state — matched by
            // product+variant since items are wholesale deleted/reinserted below
            // (no stable item id to diff against). A reason is mandatory whenever
            // any line's price is actually being changed.
            $old_items_for_price_check = $db->query("SELECT product_id, variant_id, unit_price FROM credit_order_items WHERE order_id = ?", [$order_id])->results();
            $old_price_by_key = [];
            foreach ($old_items_for_price_check as $oi) {
                $old_price_by_key[$oi->product_id . ':' . ($oi->variant_id ?? 0)] = (float)$oi->unit_price;
            }
            $price_changes = [];
            foreach ($items as $item) {
                $key = $item['product_id'] . ':' . (!empty($item['variant_id']) ? $item['variant_id'] : 0);
                if (isset($old_price_by_key[$key]) && round($old_price_by_key[$key], 2) !== round((float)$item['unit_price'], 2)) {
                    $price_changes[] = [
                        'product_id'     => (int)$item['product_id'],
                        'variant_id'     => !empty($item['variant_id']) ? (int)$item['variant_id'] : null,
                        'quantity'       => (float)$item['quantity'],
                        'old_unit_price' => $old_price_by_key[$key],
                        'new_unit_price' => (float)$item['unit_price'],
                    ];
                }
            }
            $price_change_reason = trim($_POST['price_change_reason'] ?? '');
            if (!empty($price_changes) && $price_change_reason === '') {
                throw new Exception('A reason is required when changing item prices — please fill in "Reason for Price Change".');
            }
            ...
            // Update order
            $order_updated = $db->update('credit_orders', [
            ...
            // Delete old items — this was previously unchecked. Database::query()
            // silently swallows a failed DELETE (never throws), so a transient DB
            // error here used to fall straight through into the INSERT loop below
            // with the old rows still in place, leaving TWO full sets of line items
            // for the same order_id (the "duplicate-item edit" bug found in the 8 Aug
            // sweep) — and the transaction would still commit, since nothing threw.
            $db->query("DELETE FROM credit_order_items WHERE order_id = ?", [$order_id]);
            if ($db->error()) {
                throw new Exception('Failed to remove the old line items before re-saving.');
            }
            ...
            foreach ($items as $item) {
                ...
                $item_inserted = $db->insert('credit_order_items', [
                    'order_id' => $order_id,
                ...
                if (!$item_inserted) {
                    throw new Exception('Failed to save a line item — the order was not updated.');
                }
            }

            // Record price revision history — matched to the freshly-inserted
            // item rows by product+variant (their ids just changed above).
            if (!empty($price_changes)) {
                $new_items_for_price_link = $db->query("SELECT id, product_id, variant_id FROM credit_order_items WHERE order_id = ?", [$order_id])->results();
                $new_item_id_by_key = [];
                foreach ($new_items_for_price_link as $ni) {
                    $new_item_id_by_key[$ni->product_id . ':' . ($ni->variant_id ?? 0)] = $ni->id;
                }
                foreach ($price_changes as $pc) {
                    $pc_key = $pc['product_id'] . ':' . ($pc['variant_id'] ?? 0);
                    $revision_inserted = $db->insert('credit_order_price_revisions', [
                        'order_id'           => $order_id,
                        'order_item_id'      => $new_item_id_by_key[$pc_key] ?? null,
                        'product_id'         => $pc['product_id'],
                        'variant_id'         => $pc['variant_id'],
                        'quantity'           => $pc['quantity'],
                        'old_unit_price'     => $pc['old_unit_price'],
                        'new_unit_price'     => $pc['new_unit_price'],
                        'reason'             => $price_change_reason,
                        'changed_by_user_id' => $user_id,
                    ]);
                    if (!$revision_inserted) {
                        throw new Exception('Failed to record the price revision history.');
                    }
                }
            }

            // Post the same dispatch accounting (journal + customer_ledger + invoice
            // snapshot) that cr/credit_dispatch.php posts, whenever this edit moves the
            // order into goods_on_board/shipped/delivered — Admin Edit's raw status
            // change previously had NO accounting side-effect at all, so an order could
            // be marked shipped/delivered and never appear in the customer's ledger or
            // the general ledger. postCreditOrderDispatchAccounting() is idempotent
            // (skips if already posted), so this is safe to call on every save.
            if (in_array($status, ['goods_on_board', 'shipped', 'delivered'], true)) {
                postCreditOrderDispatchAccounting($db, $order_id, $user_id, [
            ...
            if (!empty($price_changes)) {
                $changes[] = count($price_changes) . " item price(s) revised — {$price_change_reason}";
            }
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" 
                            ...
                            <option value="escalated" <?php echo $order->status === 'escalated' ? 'selected' : ''; ?>>Escalated (over credit limit)</option>
                ...
                <div id="itemsContainer">
                    ...
                    <div class="item-row" data-item-index="<?php echo $index; ?>">
                        <div class="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                                    ...
                                    <optgroup label="<?php echo htmlspecialchars($product->name); ?>">
                                        ...
                                        <?php if ($product->variants):
                                            ...
                                            foreach ($variants as $variant_str):
                                                ...
                                                // BUG FIX: For the currently selected option on an existing item row,
                                                // use the STORED historical unit_price as data-price, NOT the current
                                                // market price from product_prices. This prevents updateItemPrice()
                                                // from silently overwriting the locked invoice price.
                                                $is_selected_option = ($item->product_id == $product->id && $item->variant_id == $var_id);
                                                $option_price = $is_selected_option ? $item->unit_price : $var_price;
                                        ?>
                                        <option value="<?php echo $product->id; ?>|<?php echo $var_id; ?>"
                                                data-price="<?php echo htmlspecialchars($option_price); ?>"
                                                data-market-price="<?php echo htmlspecialchars($var_price); ?>"
                                                <?php echo $is_selected_option ? 'selected' : ''; ?>>
                            ...
                            <!-- Unit Price — locked to historical invoice price -->
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1 flex items-center gap-1">
                                    Price
                                    <span title="Locked to the price at time of order. Click 🔄 to use current market price."
                                          style="color:#b45309;font-size:10px;cursor:help">🔒</span>
                                </label>
                                <div class="flex gap-1 items-center">
                                    <input type="number"
                                           class="item-price flex-1 px-2 py-2 text-sm border border-amber-400 rounded focus:ring-2 focus:ring-blue-500 bg-amber-50"
                                           value="<?php echo $item->unit_price; ?>"
                                           data-original-price="<?php echo $item->unit_price; ?>"
                                           step="0.01"
                                           min="0"
                                           onchange="calculateLineTotal(this)"
                                           title="Invoice price locked at ৳<?php echo number_format($item->unit_price, 2); ?>. Change only if intentionally revising.">
                                    <button type="button"
                                            onclick="useCurrentMarketPrice(this)"
                                            title="Replace with current market price"
                                            style="font-size:12px;padding:5px 6px;background:#f59e0b;border:none;border-radius:4px;cursor:pointer;color:#fff;white-space:nowrap"
                                    >🔄</button>
                                </div>
                            </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Reason for Price Change <span class="text-xs text-gray-400 font-normal">(required only if you change an item's unit price below)</span>
                        </label>
                        <textarea name="price_change_reason"
                                  rows="2" placeholder="e.g. Pricing Engine revision on 19 Aug — flour rate increased before dispatch"
...
// BUG FIX: For EXISTING item rows (those rendered server-side with a locked invoice price),
// we do NOT auto-fill the price from data-price — because data-price on the selected option
// has been set to the stored historical price, but switching to a different product should
// fill in that product's current market price. We only fill for new items (no data-original-price).
function updateItemPrice(select) {
    const selectedOption = select.options[select.selectedIndex];
    const row = select.closest('.item-row');
    const priceInput = row.querySelector('.item-price');

    // Is this an existing (server-rendered) row with a locked original price?
    const isExistingRow = priceInput.dataset.originalPrice !== undefined;

    if (isExistingRow) {
        // The admin changed the product on an existing row.
        // Use the market price of the newly selected product (data-market-price),
        // NOT the locked historical price. Alert them so it's not silent.
        const marketPrice = selectedOption.dataset.marketPrice || selectedOption.dataset.price || 0;
        priceInput.value = marketPrice;
        priceInput.style.borderColor = '#ef4444'; // Red border — price was changed
        priceInput.title = '⚠️ Price changed to current market price because product was switched. Verify before saving.';
    } else {
        // New item row added via addItem() — fill current market price normally.
...
// Intentional "use current market price" button handler (🔄 button next to price input).
// Only fires when the admin explicitly clicks it.
function useCurrentMarketPrice(btn) {
    ...
    if (!confirm(`Replace locked invoice price ৳${parseFloat(originalPrice).toFixed(2)} with current market price ৳${parseFloat(marketPrice).toFixed(2)}?\n\nThis will permanently change the invoice amount.`)) {
        return;
    }
    priceInput.value = marketPrice;
    priceInput.style.borderColor = '#ef4444';
    priceInput.title = '⚠️ Price manually updated to current market price.';
    calculateLineTotal(priceInput);
}
