...
/* ─── Financial application for 'bill' resolution ───────────────────────────
   ...
   throw on failure, so the caller's transaction would still commit even if, say,
   the debit note failed to post: the OD would end up 'approved and billed' with no
   ledger trail justifying the extra amount owed. Now throws on any failure so the
...
function _applyOverDeliveryBilling($db, $order, $od_id, $od_number, $amount, $user_id): void {
    ...
    if ($db->error()) throw new Exception('Failed to update the order total/balance for this over-delivery.');

    $db->query(
        "UPDATE customers SET current_balance = current_balance + ? WHERE id = ?",
        [$amount, $customer_id]
    );
    if ($db->error()) throw new Exception('Failed to update the customer balance for this over-delivery.');
    ...
    if (!$ledger_id) throw new Exception('Failed to post the customer ledger debit note for this over-delivery.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_od') {
    if (!$can_record) {
        ...
        try {
            ...
            if (!$od_id) throw new Exception('Failed to create the over-delivery record.');

            foreach ($lines as $l) {
                $item_id = $db->insert('credit_order_over_delivery_items', array_merge(['od_id' => $od_id], $l));
                if (!$item_id) throw new Exception('Failed to save an over-delivery line item.');
            }
    ...
    if (!$can_approve) {
        ...
        try {
            ...
            if ($action === 'approve_od') {
                ...
                if ($db->error()) throw new Exception('Failed to mark the over-delivery as approved.');
                $pdo->commit();
                ...
                if ($db->error()) throw new Exception('Failed to mark the over-delivery as rejected.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'mark_retrieved') {
    if (!$can_approve) {
        ...
        try {
            ...
            if ($db->error()) throw new Exception('Failed to mark the goods as retrieved.');
