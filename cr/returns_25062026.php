    ...
    if (!$is_admin) {
        ...
        try {
            ...
            if ($action === 'approve_return') {
                ...
                // Bug 17 fix: was absolute write — race condition when two returns process
                // concurrently (read stale balance from JOIN, compute, write back).
                // Use SQL-level atomic relative decrement instead.
                $db->query(
                    "UPDATE customers SET current_balance = GREATEST(0, current_balance - ?) WHERE id = ?",
                    [(float)$ret->total_returned_amount, $ret->customer_id]
                );
