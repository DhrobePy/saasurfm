...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'collect_payment') {
    ...
    if ($customer_id <= 0) {
    ...
    } elseif ($collect_cap !== null && $payment_amount > $collect_cap) {
        // Over the cap → park as a pending request (maker/checker) instead of rejecting.
        ...
        if ($req_id) {
            auditLogPayment('queued', $req_id, 'REQ-' . $req_id,
                'Field payment of ৳' . number_format($payment_amount, 2) . ' queued for approval (over ৳'
                . number_format($collect_cap, 0) . ' limit) by ' . ($currentUser['display_name'] ?? 'user'));
