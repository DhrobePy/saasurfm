...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'record_delivery') {
    try {
        ...
        // "Is this order now fully delivered?" is the single shared decision
        // in updateOrderDeliveryStatus() (core/functions/helpers.php) — also
        // used by the gate-pass delivery confirmation (verify_delivery.php),
        // so both entry points agree on when an order actually becomes
        // 'delivered' rather than duplicating this logic two places.
        $old_balance_due = (float)$order->balance_due;
        $new_status      = updateOrderDeliveryStatus($db, $order_id);
        $new_balance_due = (float)($db->query("SELECT balance_due FROM credit_orders WHERE id = ?", [$order_id])->first()->balance_due ?? 0);
        if ($new_status === 'delivered') {
            $is_final = 1;   // reflect in this delivery's notification/audit text below
        }
        ...
        try {
            if (defined('TELEGRAM_BOT_TOKEN') && defined('TELEGRAM_CHAT_ID') && defined('TELEGRAM_NOTIFICATIONS_ENABLED') && TELEGRAM_NOTIFICATIONS_ENABLED) {
                ...
                $notifier = new TelegramNotifier(TELEGRAM_BOT_TOKEN, getTelegramChatId('dispatch'));
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2 text-center whitespace-nowrap">
                                <a href="credit_invoice_print.php?id=<?php echo (int)$selected_order_id; ?>&delivery_id=<?php echo (int)$pd->id; ?>"
                                   target="_blank" class="inline-flex items-center px-2 py-1 border border-indigo-300 text-indigo-600 rounded hover:bg-indigo-50 cursor-pointer" title="Print this delivery's invoice">
                                    <i class="fas fa-print text-xs"></i>
                                </a>
                                <a href="dispatch_slip.php?id=<?php echo (int)$selected_order_id; ?>&delivery_id=<?php echo (int)$pd->id; ?>"
                                   target="_blank" class="inline-flex items-center px-2 py-1 border border-amber-300 text-amber-600 rounded hover:bg-amber-50 cursor-pointer ml-1" title="Print this delivery's gate pass">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-center whitespace-nowrap">
                        <a href="credit_invoice_print.php?id=<?php echo (int)$d->order_id; ?>&delivery_id=<?php echo (int)$d->id; ?>"
                           target="_blank" class="inline-flex items-center px-1.5 text-indigo-600 hover:text-indigo-800" title="Print this delivery's invoice">
                            <i class="fas fa-print text-xs"></i>
                        </a>
                        <a href="dispatch_slip.php?id=<?php echo (int)$d->order_id; ?>&delivery_id=<?php echo (int)$d->id; ?>"
                           target="_blank" class="inline-flex items-center px-1.5 text-amber-600 hover:text-amber-800" title="Print this delivery's gate pass">
                            <i class="fas fa-id-card text-xs"></i>
                        </a>
                        <a href="?order_id=<?php echo $d->order_id; ?>"
                           class="text-xs text-indigo-600 hover:underline ml-1">+ Add delivery</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
