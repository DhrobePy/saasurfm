...
// Price revision history — self-migrating, same table admin_edit.php writes to
try { $db->getPdo()->exec("
    CREATE TABLE IF NOT EXISTS `credit_order_price_revisions` (
...
$price_revisions = [];
try {
    $price_revisions = $db->query(
        "SELECT r.*, p.base_name AS product_name, pv.grade, pv.weight_variant,
                u.display_name AS changed_by_name
         FROM credit_order_price_revisions r
         JOIN products p ON r.product_id = p.id
         LEFT JOIN product_variants pv ON r.variant_id = pv.id
         LEFT JOIN users u ON r.changed_by_user_id = u.id
         WHERE r.order_id = ?
...
try {
    $payments = $db->query(
        "SELECT pa.allocated_amount,
                cp.id AS payment_id, cp.payment_number, cp.payment_date, cp.payment_method,
                ...
                (SELECT COUNT(*) FROM payment_allocations pa2 WHERE pa2.payment_id = cp.id) AS alloc_count
...
// credit_payment_receipt.php restricts to a narrower role list than this page —
// only render the receipt link as clickable for roles that can actually open it,
// so sales/production/dispatch users viewing an order don't hit a dead-end click.
$can_view_receipt = in_array($currentUser['role'] ?? '', ['Superadmin', 'admin', 'Accounts', 'collector']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_admin && isset($_POST['gate_action'])) {
    ...
    try {
        if ($ga === 'release_production_hold') {
            ...
            // Upsert so an admin can clear a default-held order that has no gate row yet.
            clearOrderDispatch($order_id, (int)$user_id, $note !== '' ? $note : 'Admin override');
            ...
            if (in_array($order->status, ['goods_on_board', 'shipped', 'delivered'])) {
        ...
        } elseif ($ga === 'reconcile_balance' && ($currentUser['role'] ?? '') === 'Superadmin') {
            // Recompute amount_paid from surviving payment allocations and fix balance_due.
            // Use case: a ledger entry was deleted via the audit page without reversing the order row.
            // Advance allocations are excluded (they live in advance_paid, which also carries
            // creation-time advances with no allocation rows — so advance_paid is never recomputed).
            // INNER JOIN drops orphaned allocations whose payment record was deleted.
            $sum_row = $db->query(
                "SELECT COALESCE(SUM(pa.allocated_amount), 0) AS total_alloc
                 FROM payment_allocations pa
                 INNER JOIN customer_payments cp ON cp.id = pa.payment_id
                 WHERE pa.order_id = ? AND cp.payment_type NOT LIKE '%advance%'",
                [$order_id]
            )->first();
            $correct_paid = (float)$sum_row->total_alloc;
            $correct_due  = max(0, (float)$order->total_amount - (float)($order->advance_paid ?? 0) - $correct_paid);
            $db->query(
                "UPDATE credit_orders SET amount_paid = ?, balance_due = ? WHERE id = ?",
                [$correct_paid, $correct_due, $order_id]
            );
            logGateEvent($order_id, 'balance_reconciled',
                sprintf('Balance reconciled by %s — amount_paid ৳%s, balance_due ৳%s',
                    $currentUser['display_name'] ?? 'admin',
                    number_format($correct_paid, 2),
                    number_format($correct_due, 2)));
            auditLog('credit_orders', 'reconcile',
                "Balance reconciled for {$order->order_number}: paid=৳" . number_format($correct_paid, 2)
                . " due=৳" . number_format($correct_due, 2),
                ['order_id' => $order_id, 'amount_paid' => $correct_paid, 'balance_due' => $correct_due]
            );
            $_SESSION['success_flash'] = "Balance reconciled — amount paid: ৳" . number_format($correct_paid, 2)
                . " | balance due: ৳" . number_format($correct_due, 2);
        }
    } catch (Exception $e) {
...
// Feature #5: ensure this order has a dispatch-hold row under the global hold, so
// the conditions card + clearance controls render (idempotent).
ensureDefaultDispatchHold($order_id);
...
<div class="mb-6 flex flex-wrap gap-2">
    ...
    <?php if (!empty($order->is_other_sales)): ?>
    <span class="inline-block px-4 py-2 text-sm font-semibold rounded-full bg-purple-100 text-purple-800">
        <i class="fas fa-right-left mr-2"></i>Other Sales — Direct Commodity Resale
    </span>
    <?php endif; ?>
</div>
...
// Auto-hide buttons whose target page this user cannot open. Conservative:
// only hides when an explicit page whitelist EXCLUDES the target — privilege
// users with no whitelist and role-fallback users see everything (the target
// page still enforces its own access on click, so hiding is purely cosmetic).
$ov_cs_pages = null;   // null = no restriction known
if (!$is_admin) {
    $ov_perms = getUserCustomPerms();
    $ov_cs    = $ov_perms['credit_sales'] ?? null;
    if ($ov_cs !== null && !empty($ov_cs['enabled']) && !empty($ov_cs['allowed_pages'])) {
        $ov_cs_pages = $ov_cs['allowed_pages'];
    }
}
function ov_page_ok(string $page_key): bool {
    global $ov_cs_pages;
    return $ov_cs_pages === null || in_array($page_key, $ov_cs_pages);
}

if (in_array($ov_st, ['pending_approval', 'escalated']) && ov_page_ok('credit_order_approval')) {
...
if (in_array($ov_st, ['shipped', 'delivered'])) {
    if (ov_page_ok('returns'))       $ov_actions[] = ['fa-undo',          'orange', 'Return Goods',  'returns.php?order_id=' . $order_id];
    if (ov_page_ok('over_delivery')) $ov_actions[] = ['fa-truck-loading', 'blue',   'Over-Delivery', 'over_delivery.php?order_id=' . $order_id];
}
if (!in_array($ov_st, ['pending_approval', 'escalated', 'rejected', 'cancelled']) && ov_page_ok('order_amendment')) {
    $ov_actions[] = ['fa-file-signature', 'teal',  'Amend Order',      'order_amendment.php?order_id=' . $order_id];
}
if ($gate['has_conditions'] && (int)($gate['row']->dispatch_hold ?? 0) === 1 && ov_page_ok('payment_watch')) {
...
<div class="mb-6 bg-white rounded-lg shadow-md px-4 py-3 flex flex-wrap items-center gap-2">
    ...
    <?php endforeach; ?>
    <?php if (in_array($ov_st, ['goods_on_board', 'shipped', 'delivered'])): ?>
    ...
    <?php endif; ?>
    <?php if (in_array($ov_st, ['ready_to_ship', 'shipped', 'delivered'])): ?>
    <a href="dispatch_slip.php?id=<?php echo $order_id; ?>" target="_blank"
       class="inline-flex items-center gap-1.5 px-3 py-1.5 border-2 border-indigo-300 text-indigo-700 text-xs font-semibold rounded-lg hover:bg-indigo-50 transition-colors cursor-pointer">
        <i class="fas fa-qrcode"></i>Dispatch Slip
    </a>
    <?php endif; ?>
</div>
<?php endif; ?>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <?php
            // Superadmin repair tool: compare recorded amount_paid against what surviving,
            // non-advance payment allocations actually back. Shown only when they disagree
            // (e.g. a payment's ledger entry was deleted without reversing the order row).
            if (($currentUser['role'] ?? '') === 'Superadmin'):
                ...
                try {
                    ...
                    $rcn_backed = (float)$rcn_row->total_alloc;
                } catch (Exception $e) {}
                $rcn_recorded = (float)($order->amount_paid ?? 0);
                if ($rcn_backed !== null && abs($rcn_backed - $rcn_recorded) > 0.009):
            ?>
            <div class="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg flex flex-wrap items-center justify-between gap-3">
                <p class="text-xs text-orange-800">
                    <i class="fas fa-exclamation-triangle mr-1"></i>
                    Recorded paid <strong>৳<?php echo number_format($rcn_recorded, 2); ?></strong>
                    does not match payment records (<strong>৳<?php echo number_format($rcn_backed, 2); ?></strong>)
                    — likely a deleted payment that was not reversed on this order.
                    <?php if ($rcn_backed < $rcn_recorded): ?>
                    <br><span class="text-orange-600">Reconcile will lower paid to ৳<?php echo number_format($rcn_backed, 2); ?> and raise the balance due. For very old orders recorded before allocation tracking, verify the payment history below first.</span>
                    <?php endif; ?>
                </p>
                <form method="POST" onsubmit="return confirm('Set amount paid to ৳<?php echo number_format($rcn_backed, 2); ?> (from payment records) and recompute the balance due?');">
                    <input type="hidden" name="gate_action" value="reconcile_balance">
                    <button type="submit" class="px-4 py-2 bg-orange-600 text-white rounded-lg text-xs font-bold hover:bg-orange-700 cursor-pointer">
                        <i class="fas fa-sync-alt mr-1"></i>Reconcile Balance
                    </button>
                </form>
            </div>
            <?php endif; endif; ?>
        ...
        <div id="deliveries" class="bg-white rounded-lg shadow-md p-6">
                ...
                <table class="min-w-full text-sm divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2 text-center">
                                <a href="credit_invoice_print.php?id=<?php echo $order->id; ?>&delivery_id=<?php echo $d->id; ?>"
                                   target="_blank" class="inline-flex items-center px-2 py-1 border border-indigo-300 text-indigo-600 rounded hover:bg-indigo-50 cursor-pointer" title="Print this delivery">
        ...
        <?php if (!empty($price_revisions)): ?>
        <div class="bg-white rounded-lg shadow-md p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4"><i class="fas fa-tags mr-2 text-amber-500"></i>Price Revision History</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Qty</th>
                            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">Old Price</th>
                            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase">New Price</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php foreach ($price_revisions as $pr):
                            $pr_vd = [];
                            if ($pr->grade)          $pr_vd[] = 'Grade ' . $pr->grade;
                            if ($pr->weight_variant) $pr_vd[] = $pr->weight_variant;
                            $pr_delta = (float)$pr->new_unit_price - (float)$pr->old_unit_price;
                        ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-3 py-2 text-gray-600 whitespace-nowrap"><?php echo date('d-M-Y g:ia', strtotime($pr->created_at)); ?></td>
                            <td class="px-3 py-2">
                                <div class="font-medium text-gray-900"><?php echo htmlspecialchars($pr->product_name); ?></div>
                                <?php if ($pr_vd): ?><div class="text-xs text-gray-400"><?php echo htmlspecialchars(implode(' · ', $pr_vd)); ?></div><?php endif; ?>
                            </td>
                            <td class="px-3 py-2 text-right"><?php echo number_format($pr->quantity, 2); ?></td>
                            <td class="px-3 py-2 text-right text-gray-500 line-through">৳<?php echo number_format($pr->old_unit_price, 2); ?></td>
                            <td class="px-3 py-2 text-right font-semibold <?php echo $pr_delta > 0 ? 'text-red-600' : 'text-green-600'; ?>">
                                ৳<?php echo number_format($pr->new_unit_price, 2); ?>
                                <span class="text-[10px] font-normal"><?php echo $pr_delta > 0 ? '▲' : '▼'; ?><?php echo number_format(abs($pr_delta), 2); ?></span>
                            </td>
                            <td class="px-3 py-2 text-gray-700 max-w-xs"><?php echo nl2br(htmlspecialchars($pr->reason)); ?></td>
                            <td class="px-3 py-2 text-xs text-gray-500"><?php echo htmlspecialchars($pr->changed_by_name ?? '—'); ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-3 py-2 font-medium">
                                ...
                                <a href="credit_payment_receipt.php?id=<?php echo (int)$p->payment_id; ?>" target="_blank"
                                   class="text-green-700 hover:text-green-800 hover:underline" title="View full receipt">
                                    <?php echo htmlspecialchars($p->payment_number ?? '—'); ?>
                                </a>
                                <?php else: ?>
                                <span class="text-green-700"><?php echo htmlspecialchars($p->payment_number ?? '—'); ?></span>
                                <?php endif; ?>
                                <?php if ((int)$p->alloc_count > 1): ?>
                                <span class="block text-[10px] text-gray-400 mt-0.5">split across <?php echo (int)$p->alloc_count; ?> invoices</span>
                                <?php endif; ?>
                            </td>
