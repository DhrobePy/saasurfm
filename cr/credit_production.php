...
// Self-migrating: delivery_priority also drives the Delivery Schedule tab's
// sort below — same column cr/delivery_schedule.php provisions, ensured here
// too so this page doesn't depend on that one having loaded first.
...
// ── Delivery Schedule (due today / overdue) ──────────────────────────────────
// Same active-pipeline statuses as the production queue above, PLUS 'shipped'
// — that's what a partially-delivered order's status becomes once the first
// partial trip goes out (updateOrderDeliveryStatus() only moves it all the
// way to 'delivered' once everything's out). Previously excluded here, so a
// partial shipment past its required_date silently disappeared from this
// Overdue table the moment any of it shipped (found live via a real ACI order).
$due_today_orders = [];
$overdue_orders   = [];
try {
    $due_today_orders = $db->query(
        "SELECT co.*, c.name as customer_name, c.phone_number as customer_phone, b.name as branch_name
         FROM credit_orders co
         JOIN customers c ON co.customer_id = c.id
         LEFT JOIN branches b ON co.assigned_branch_id = b.id
         WHERE co.status IN ('approved','in_production','produced','ready_to_ship','shipped')
           AND co.assigned_branch_id IS NOT NULL
           AND co.required_date = CURDATE()
           $branch_filter
    ...
    error_log('Delivery schedule query: ' . $e->getMessage());
}

// Payment/dispatch hold — surfaced here so production managers can see an
// order due today is stuck at Accounts regardless of production progress.
$hold_reason_by_order = [];
foreach (array_merge($due_today_orders, $overdue_orders) as $o) {
    $reason = getOrderDispatchHoldReason((int)$o->id);
    if ($reason !== null) $hold_reason_by_order[(int)$o->id] = $reason;
}
...
if (!in_array($active_tab, ['queue', 'delivery', 'orders', 'report', 'payments'])) $active_tab = 'queue';
...
$tabs[] = ['key' => 'delivery', 'label' => 'Delivery Schedule', 'icon' => 'fa-truck-fast'];
...
══════════════════════════════════════════════════════════ -->
<div class="<?php echo $active_tab !== 'delivery' ? 'hidden' : ''; ?>">
<?php
$delivery_vehicle_badge = function ($type) {
    if (($type ?? '') === 'mini_truck') {
...
$render_delivery_table = function ($rows, $show_days_overdue) use ($status_cls, $status_labels, $delivery_vehicle_badge, $hold_reason_by_order) {
    if (empty($rows)) {
        echo '<div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6 text-center text-sm text-gray-400">No orders in this list.</div>';
        return;
    }
    ?>
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-xs">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200 text-left">
                    ...
                    <th class="px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">Vehicle</th>
                    <th class="px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">Req. Date</th>
                    <?php if ($show_days_overdue): ?>
                    <th class="px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap text-center">Days Overdue</th>
                    <?php endif; ?>
                    <th class="px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap text-right">Amount</th>
                    <th class="px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">Status</th>
                    <th class="px-3 py-2 font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap text-center">Actions</th>
            ...
            <?php foreach ($rows as $o):
                $days_overdue = $show_days_overdue && $o->required_date
                    ? (int)floor((strtotime('today') - strtotime($o->required_date)) / 86400) : 0;
            ?>
            <tr class="hover:bg-red-50/30 transition-colors">
                <td class="px-3 py-2.5 whitespace-nowrap">
                    ...
                    <?php if (isset($hold_reason_by_order[(int)$o->id])): ?>
                    <div class="mt-0.5">
                        <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 text-[10px] font-bold" title="<?php echo htmlspecialchars($hold_reason_by_order[(int)$o->id]); ?>">
                            <i class="fas fa-lock"></i>Payment Hold
                        </span>
                    </div>
                    <?php endif; ?>
                </td>
                ...
                <td class="px-3 py-2.5 whitespace-nowrap"><?php echo $delivery_vehicle_badge($o->delivery_type ?? ''); ?></td>
                <td class="px-3 py-2.5 whitespace-nowrap text-gray-600">
                    <?php echo $o->required_date ? date('d M Y', strtotime($o->required_date)) : '—'; ?>
                </td>
                <?php if ($show_days_overdue): ?>
                <td class="px-3 py-2.5 text-center whitespace-nowrap font-bold text-red-600"><?php echo $days_overdue; ?></td>
                <?php endif; ?>
...
<div class="mb-5">
    <h3 class="text-sm font-bold text-gray-800 mb-2 flex items-center gap-2">
        <i class="fas fa-calendar-day text-blue-500"></i>Due for Delivery Today
        <span class="text-xs font-normal text-gray-400">(<?php echo date('d M Y'); ?>)</span>
        <span class="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-[11px] font-semibold"><?php echo count($due_today_orders); ?></span>
    </h3>
    <?php $render_delivery_table($due_today_orders, false); ?>
</div>

<div>
    <h3 class="text-sm font-bold text-gray-800 mb-2 flex items-center gap-2">
        <i class="fas fa-triangle-exclamation text-red-500"></i>Overdue Deliveries
        <span class="px-2 py-0.5 rounded-full bg-red-100 text-red-700 text-[11px] font-semibold"><?php echo count($overdue_orders); ?></span>
    </h3>
    <?php $render_delivery_table($overdue_orders, true); ?>
</div>
