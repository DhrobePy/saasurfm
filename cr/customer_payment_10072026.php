...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'record_payment' && !$error) {
    try {
        ...
        // Per-user collection ceiling (admin/privileges.php → Action Limits).
        // No legacy fallback: only an explicit collect_payment limit applies.
        // Over the cap → the receipt is parked as a pending request (maker/checker)
        // instead of being rejected. Nothing posts to the ledger until a checker
        // whose own limit covers the amount reviews and posts it.
        $exec_pending_req_id = (int)($_POST['pending_req_id'] ?? 0);
        if (!$is_admin) {
            $collect_cap = getUserActionLimit((int)($currentUser['id'] ?? 0), 'collect_payment');
            if ($collect_cap !== null && $payment_amount > $collect_cap) {
                if ($exec_pending_req_id) {
                    // A checker tried to post someone's request but their own cap
                    // doesn't cover it either — never re-queue a queued request.
                ...
                auditLogPayment('queued', $req_id, 'REQ-' . $req_id,
                    ...
                    . ' queued for approval (over ৳' . number_format($collect_cap, 0) . ' limit) by '
                    . ($currentUser['display_name'] ?? 'user'));
                $_SESSION['success_flash'] = 'This receipt (৳' . number_format($payment_amount, 0)
                    . ') is above your collection limit, so it was sent for approval as request #' . $req_id
