...
/* Previous Due — ALWAYS computed live as: customer's TRUE current outstanding
 * minus THIS invoice's own remaining due (when this invoice has posted to the
 * ledger). Never derived from stored balance_after or the snapshot, both of
 * which double-counted advances that were later allocated to this invoice
 * (the payment appeared in "Paid (this invoice)" AND inside the balance-before,
 * so it was subtracted twice → e.g. Total Due showed −43,000 instead of 47,000).
 * balance_after chains are also unreliable after backdated reconciliations. */
$previous_due = 0.0;
try {
    // 1) True outstanding right now: initial_due + ledger debits − credits
    //    (opening-balance rows excluded — initial_due is added explicitly).
    if (function_exists('getCustomerOutstanding')) {
        $outstanding_now = (float)getCustomerOutstanding((int)$order->customer_id);
    } else {
        $ob_row = $db->query(
            "SELECT COALESCE(c.initial_due, 0)
                    + COALESCE(SUM(CASE WHEN cl.reference_type != 'initial_due' THEN cl.debit_amount  ELSE 0 END), 0)
                    - COALESCE(SUM(CASE WHEN cl.reference_type != 'initial_due' THEN cl.credit_amount ELSE 0 END), 0) AS ob
             FROM customers c
             LEFT JOIN customer_ledger cl ON cl.customer_id = c.id
             WHERE c.id = ?
             GROUP BY c.id, c.initial_due",
            [$order->customer_id]
        )->first();
        $outstanding_now = (float)($ob_row->ob ?? 0);
    }

    // 2) This invoice's own remaining due (unclamped; invariant:
    //    balance_due = total_amount − advance_paid − amount_paid).
    $this_inv_due = (float)$order->total_amount
                  - (float)($order->advance_paid ?? 0)
                  - (float)($order->amount_paid  ?? 0);

    // 3) Subtract this invoice's due ONLY if it has posted to the ledger
    //    (posting happens at dispatch). A pre-dispatch print isn't part of
    //    the outstanding yet, so previous due = full outstanding.
    $posted = $db->query(
    ...
    $previous_due = $posted ? ($outstanding_now - $this_inv_due) : $outstanding_now;
    // Suppress float dust (−0.00)
    if (abs($previous_due) < 0.005) $previous_due = 0.0;
