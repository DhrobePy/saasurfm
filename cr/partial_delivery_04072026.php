...
$eligible_statuses = ['approved', 'in_production', 'produced', 'ready_to_ship', 'shipped'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'record_delivery') {
    try {
        ...
        $db->query(
            "INSERT INTO credit_order_workflow (order_id, action, from_status, to_status, performed_by_user_id, comments, created_at)
             VALUES (?, 'deliver', ?, ?, ?, ?, NOW())",
                ...
                <div>
                    ...
                    <p class="text-sm text-red-700 mt-1">
                        ...
                        <?php if ($pd_gate['threshold'] !== null && $pd_gate['current'] !== null): ?>
                            <?php if (($pd_gate['row']->condition_type ?? '') === 'outstanding_below'): ?>
                            Customer outstanding is <strong>৳<?php echo number_format($pd_gate['current'], 0); ?></strong>,
