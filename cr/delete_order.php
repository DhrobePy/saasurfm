...
try {
    ...
    // These three also carry a real FOREIGN KEY to credit_orders.id — missing
    // any of them makes the final DELETE below fail with an FK violation.
    // (That's exactly what happened for a batch of older orders: the delete
    // silently "succeeded" — archived a duplicate copy, left the order live —
    // because the failure was swallowed instead of surfaced. See recycleArchiveDelete().)
    recycleArchiveDelete($batch, 'credit_order_audit',    'order_id', $order_id);
    recycleArchiveDelete($batch, 'trip_order_assignments','order_id', $order_id);
    recycleArchiveDelete($batch, 'trip_consolidation_suggestions', 'base_order_id',      $order_id);
    recycleArchiveDelete($batch, 'trip_consolidation_suggestions', 'suggested_order_id', $order_id);
