...
try {
    ...
    // Feature #8: goods_on_board also prints — the invoice/ledger posts at that step.
    if (!in_array($check_order->status, ['goods_on_board','shipped','delivered'])) {
        die("Order #{$check_order->order_number} (status: {$check_order->status}) cannot be printed yet — only goods-on-board/shipped/delivered orders.<br><a href='javascript:history.back()'>← Back</a>");
    }
    ...
    // Partial-delivery print: show only what was actually loaded/handed over on
    // THIS delivery run, not the full order — that's the whole point of a
    // delivery-scoped print vs the standing order invoice.
    $delivery = null;
    $delivery_items_raw = [];
    if ($delivery_id > 0) {
        $delivery = $db->query("SELECT * FROM credit_order_deliveries WHERE id = ? AND order_id = ?", [$delivery_id, $order_id])->first();
        if (!$delivery) die("Delivery record not found for this order.<br><a href='javascript:history.back()'>← Back</a>");
        $delivery_items_raw = $db->query(
            "SELECT di.*, p.base_name AS product_name, pv.grade, pv.weight_variant, pv.unit_of_measure, pv.sku AS variant_sku,
                    coi.quantity AS order_item_qty, coi.discount_amount AS order_item_discount
             FROM credit_order_delivery_items di
             JOIN products p ON di.product_id = p.id
             LEFT JOIN product_variants pv ON di.variant_id = pv.id
             LEFT JOIN credit_order_items coi ON di.order_item_id = coi.id
             WHERE di.delivery_id = ? ORDER BY di.id ASC", [$delivery_id]
        )->results();
        if (empty($delivery_items_raw)) die("No items found for this delivery.");
    }
    $is_delivery_print = $delivery !== null;

} catch (Exception $e) { die("Error: " . $e->getMessage()); }

// Approved amendments only — pending/rejected ones never actually changed the
// order, so they'd be noise (or worse, confusing) on a printed customer document.
$order_amendments = [];
try {
    $order_amendments = $db->query(
        "SELECT a.amd_number, a.amendment_type, a.reason, a.delta_amount, a.approved_at, a.mode,
                u.display_name AS approved_by_name
         FROM order_amendments a
         LEFT JOIN users u ON a.approved_by_user_id = u.id
         WHERE a.order_id = ? AND a.status = 'approved'
         ORDER BY a.approved_at ASC", [$order_id]
    )->results();
} catch (Exception $e) { /* table may not exist yet on older orders — treat as no amendments */ }
...
$render_items = [];
if ($is_delivery_print) {
    foreach ($delivery_items_raw as $it) {
        $obj = new stdClass();
        $vd  = [];
        ...
        $obj->quantity        = $it->qty_delivered;
        $obj->unit_price      = $it->unit_price;
        // Discounts are recorded per order-item, not per delivery — prorate the
        // item's discount by how much of its ordered quantity this shipment
        // covers (order_item_id links each delivery line back to its order line).
        $order_qty      = (float)($it->order_item_qty ?? 0);
        $order_discount = (float)($it->order_item_discount ?? 0);
        $per_unit_discount    = $order_qty > 0 ? $order_discount / $order_qty : 0;
        $obj->discount_amount = round($per_unit_discount * (float)$it->qty_delivered, 2);
        $obj->line_total      = round(((float)$it->qty_delivered * (float)$it->unit_price) - $obj->discount_amount, 2);
        $render_items[]       = $obj;
    }
} elseif ($snap_items) {
...
// Delivery-scoped totals — the items table subtotal and grand total must
// reflect only THIS shipment, never the order's full subtotal/total_amount.
// Footer "subtotal" is gross (pre-discount), matching the full-order layout
// below, where a separate Discount row bridges subtotal down to the grand total.
$render_discount_total = $is_delivery_print ? array_sum(array_map(fn($i) => (float)$i->discount_amount, $render_items)) : 0.0;
$render_subtotal    = $is_delivery_print
    ? array_sum(array_map(fn($i) => (float)$i->quantity * (float)$i->unit_price, $render_items))
    : (float)$order->subtotal;
$render_grand_total = $is_delivery_print ? ($render_subtotal - $render_discount_total) : (float)$order->total_amount;

$delivered_to_date = 0.0;
if ($is_delivery_print) {
    $dtd = $db->query("SELECT COALESCE(SUM(total_amount_delivered), 0) AS s FROM credit_order_deliveries WHERE order_id = ?", [$order_id])->first();
    $delivered_to_date = (float)($dtd->s ?? 0);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Invoice — <?php echo htmlspecialchars($is_delivery_print ? $delivery->delivery_number : $order->order_number); ?></title>
<style>
...
══════════════════════════════════════════════════════════ -->
  ...
  <div class="inv-header">
    ...
    <div class="inv-right">
      <div class="inv-word"><?php echo $is_delivery_print ? 'PARTIAL DELIVERY' : 'INVOICE'; ?></div>
      <div class="inv-num"><?php echo htmlspecialchars($is_delivery_print ? $delivery->delivery_number : ($order->order_number ?? '')); ?></div>
      <?php if ($is_delivery_print): ?>
      <div class="inv-date">Order: <?php echo htmlspecialchars($order->order_number); ?></div>
      <?php endif; ?>
      <div class="inv-date">Date: <?php echo $is_delivery_print
            ? ($delivery->delivery_date ? date('d M Y', strtotime($delivery->delivery_date)) : '—')
            : ($invoice_date ? date('d M Y', strtotime($invoice_date)) : '—'); ?></div>
      <div><span class="s-pill"><?php echo $is_delivery_print
            ? ($delivery->is_final ? 'FINAL DELIVERY' : 'PARTIAL DELIVERY')
            : strtoupper(str_replace('_', ' ', $order->status ?? '')); ?></span></div>
    </div>
  </div>
  ...
  <?php if (!empty($order_amendments)): ?>
  <div style="border:1px solid var(--ink);border-radius:4px;padding:7px 10px;margin-bottom:10px">
    <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.03em;margin-bottom:3px">
      This order has been amended (<?php echo count($order_amendments); ?>)
    </div>
    <?php foreach ($order_amendments as $amd):
        $amd_type_labels = [
            'transport_change' => 'Transport Change', 'price_revision' => 'Price Revision',
            'qty_change' => 'Quantity Change', 'freight_charge' => 'Freight / Delivery Charge',
            'rebate' => 'Rebate / Discount', 'correction' => 'Entry Correction',
        ]; ?>
    <div style="font-size:10.5px;line-height:1.5">
      <strong><?php echo htmlspecialchars($amd->amd_number); ?></strong>
      — <?php echo htmlspecialchars($amd_type_labels[$amd->amendment_type] ?? $amd->amendment_type); ?><?php if ((float)$amd->delta_amount != 0): ?>
      (<?php echo $amd->delta_amount > 0 ? '+' : '−'; ?>৳<?php echo number_format(abs($amd->delta_amount), 2); ?>)
      <?php endif; ?>
      <?php if ($amd->approved_at): ?>on <?php echo date('d M Y', strtotime($amd->approved_at)); ?><?php endif; ?>
      — <?php echo htmlspecialchars($amd->reason); ?>
    </div>
    <?php endforeach; ?>
  </div>
  <?php endif; ?>

  <div class="sec-lbl"><?php echo $is_delivery_print ? 'Delivered Items (This Shipment)' : 'Order Items'; ?></div>
  <table class="itbl">
    <thead>
      <tr>
    ...
    <tfoot>
      <tr>
        ...
        <td colspan="2"></td>
        <td class="r" style="color:var(--ink)">৳<?php echo number_format($render_subtotal, 2); ?></td>
      </tr>
    </tfoot>
  </table>

  <?php
  $ship_truck       = $is_delivery_print ? $delivery->truck_number   : ($order->truck_number ?? null);
  $ship_driver      = $is_delivery_print ? $delivery->driver_name    : ($order->driver_name ?? null);
  $ship_contact     = $is_delivery_print ? $delivery->driver_contact : ($order->driver_contact ?? null);
  $ship_date        = $is_delivery_print ? $delivery->delivery_date  : ($order->shipped_date ?? null);
  $ship_date_label  = $is_delivery_print ? 'Delivery Date' : 'Shipped';
  ?>
  <div class="bot-row">
    <div class="bot-left">
      <?php if (!empty($ship_truck) || !empty($ship_driver) || !empty($ship_date)): ?>
        <div class="ship-ttl"><?php echo $is_delivery_print ? 'This Shipment' : 'Shipping Details'; ?></div>
        <?php if ($ship_truck): ?><div class="kv2"><span class="k">Truck No.:</span><span class="v"><?php echo htmlspecialchars($ship_truck); ?></span></div><?php endif; ?>
        <?php if ($ship_driver):  ?><div class="kv2"><span class="k">Driver:</span><span class="v"><?php echo htmlspecialchars($ship_driver); ?></span></div><?php endif; ?>
        <?php if ($ship_contact): ?><div class="kv2"><span class="k">Contact:</span><span class="v"><?php echo htmlspecialchars($ship_contact); ?></span></div><?php endif; ?>
        <?php if ($ship_date): ?><div class="kv2"><span class="k"><?php echo $ship_date_label; ?>:</span><span class="v"><?php echo date('d M Y', strtotime($ship_date)); ?></span></div><?php endif; ?>
        <?php if (!$is_delivery_print && $is_delivered && $order->delivered_date): ?><div class="kv2"><span class="k">Delivered:</span><span class="v"><?php echo date('d M Y', strtotime($order->delivered_date)); ?></span></div><?php endif; ?>
      <?php else: ?>
    ...
    <div class="bot-right">
      <div class="tots-inner">
        <?php if (!$is_delivery_print && $order->discount_amount > 0): ?>
        <div class="tr2"><span class="tl">Discount</span><span class="tv">−৳<?php echo number_format($order->discount_amount, 2); ?></span></div>
        <?php endif; ?>
        <?php if ($is_delivery_print && $render_discount_total > 0): ?>
        <div class="tr2"><span class="tl">Discount (this shipment)</span><span class="tv">−৳<?php echo number_format($render_discount_total, 2); ?></span></div>
        <?php endif; ?>
        <?php if (!$is_delivery_print && $order->tax_amount > 0): ?>
        <div class="tr2"><span class="tl">Tax / VAT</span><span class="tv">৳<?php echo number_format($order->tax_amount, 2); ?></span></div>
        <?php endif; ?>
      </div>
      <div class="grand-row">
        <span class="gl"><?php echo $is_delivery_print ? 'This Delivery Amount' : 'Invoice Amount'; ?></span>
        <span class="gv">৳<?php echo number_format($render_grand_total, 2); ?></span>
      </div>
      <?php if ($is_delivery_print): ?>
      <!-- Delivery-scoped print: no per-invoice paid/due breakdown here — that's
           order-level state and doesn't map to a single partial shipment. Show
           honest order-level context instead (total, delivered-to-date, balance). -->
      <div class="tr2" style="padding:5px 12px;">
        <span class="tl">Order Total</span>
        <span class="tv">৳<?php echo number_format($order->total_amount, 2); ?></span>
      </div>
      <div class="tr2" style="padding:5px 12px;border-bottom:1px dashed var(--line-lt);">
        <span class="tl">Delivered to Date</span>
        <span class="tv">৳<?php echo number_format($delivered_to_date, 2); ?></span>
      </div>
      <?php if (showInvoiceOutstandingEnabled()): ?>
      <div class="outstanding-row">
        <span class="ol">Order Balance Due</span>
        <span class="ov">৳<?php echo number_format($order->balance_due, 2); ?></span>
      </div>
      <?php endif; ?>
      <?php else: ?>
      <?php
        // Feature #7: Paid-against-this-invoice → Invoice Due → Previous Due → Total Due
        $inv_paid  = (float)($order->advance_paid ?? 0) + (float)($order->amount_paid ?? 0);
        $inv_due   = max(0, (float)$order->total_amount - $inv_paid);
        $total_due = $inv_due + $previous_due;
      ?>
      <div class="tr2" style="padding:5px 12px;">
        <span class="tl">Paid (this invoice)</span>
        <span class="tv">−৳<?php echo number_format($inv_paid, 2); ?></span>
      </div>
      <div class="tr2" style="padding:5px 12px;<?php echo showInvoiceOutstandingEnabled() ? 'border-bottom:1px dashed var(--line-lt);' : ''; ?>">
        <span class="tl">Invoice Due</span>
        <span class="tv">৳<?php echo number_format($inv_due, 2); ?></span>
      </div>
      <?php if (showInvoiceOutstandingEnabled()): ?>
      <div class="tr2" style="padding:5px 12px;border-bottom:1px dashed var(--line-lt);">
      ...
      <div class="outstanding-row">
        <span class="ol">Total Due</span>
        <span class="ov">৳<?php echo number_format($total_due, 2); ?></span>
      </div>
      <?php endif; ?>
      <?php endif; ?>
    </div>
...
══════════════════════════════════════════════════════════ -->
  ...
  <div class="strip-head dr-head">
    ...
    <div class="sh-right">
      <div class="sh-inv"><?php echo htmlspecialchars($is_delivery_print ? $delivery->delivery_number : $order->order_number); ?></div>
      <div class="sh-date"><?php echo $ship_date ? date('d M Y', strtotime($ship_date)) : ($invoice_date ? date('d M Y', strtotime($invoice_date)) : '—'); ?></div>
    </div>
  </div>

  <div class="strip-info">
    <div class="si-cell" style="flex:1.4">
      <div class="si-lbl"><?php echo $is_delivery_print ? 'Delivery No.' : 'Invoice No.'; ?></div>
      <div class="si-val"><?php echo htmlspecialchars($is_delivery_print ? $delivery->delivery_number : $order->order_number); ?></div>
    </div>
    ...
    <div class="si-cell" style="flex:1.2">
      ...
      <div class="si-val"><?php echo htmlspecialchars($ship_truck ?: '—'); ?></div>
    </div>
  </div>
  ...
  <div class="strip-sig">
    ...
    <div class="ss-box">
      <div class="ss-line">
        ...
        <div class="ss-sub"><?php echo $ship_driver ? htmlspecialchars($ship_driver) : 'Name: ____________________'; ?></div>
      </div>
    </div>
    <div class="ss-box">
      <div class="ss-line">
