...
$allowed_roles = ['Superadmin', 'admin', 'Accounts', 'accounts-srg', 
    ...
    try {
        switch ($_POST['action']) {
            ...
            case 'get_order_timeline':
                // Get order_id from POST (since JS was updated to send POST)
                $order_id = (int)$_POST['order_id']; 
                
                // FIX: Removed 'notes' from SELECT as it does not exist in schema
                $timeline = $db->query(
                    "SELECT 
                        action_type,
                        field_name,
