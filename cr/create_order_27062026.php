...
// Get customers with TRUE balance from ledger (not stale current_balance)
$customers = $db->query(
    "SELECT c.id, c.name, c.phone_number, c.credit_limit, c.initial_due, c.status,
            COALESCE(tb.true_balance, c.initial_due, 0) AS true_balance
     FROM customers c
     LEFT JOIN (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_order') {
    try {
        ...
        // Use true ledger balance, not the stale current_balance column
        $cust_rec = $db->query(
            "SELECT c.credit_limit, c.name,
                    COALESCE(tb.true_balance, c.initial_due, 0) AS true_balance
             FROM customers c
             LEFT JOIN (
                 SELECT customer_id, balance_after AS true_balance
        ...
        // Only enforce for non-admin roles — Superadmin/admin can still submit
        $user_role = $_SESSION['user_role'] ?? '';
        $is_admin  = in_array($user_role, ['Superadmin', 'admin']);

        if (!$is_admin && $credit_limit > 0 && $balance_due > $available_credit) {
            throw new Exception(
                "Credit limit exceeded for {$cust_rec->name}. " .
                "Current outstanding: ৳" . number_format($current_due, 2) . " | " .
                "Limit: ৳" . number_format($credit_limit, 2) . " | " .
                "Available: ৳" . number_format(max(0, $available_credit), 2) . " | " .
                "This order needs: ৳" . number_format($balance_due, 2) . ". " .
                "Please collect payment first or reduce the order amount."
            );
        }
...
<div class="bg-white rounded-lg shadow-md p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <p id="selected_customer_label" class="mt-1 text-xs text-green-700 font-medium hidden"></p>
        </div>
...
function selectCustomer(id) {
    ...
    const label = document.getElementById('selected_customer_label');
