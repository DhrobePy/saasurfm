...
try {
    ...
    if ($is_superadmin) {
        ...
        foreach ($linkedPayments as $pay) {
            ...
            // Delete all payment_allocations for this payment (current + other orders)
            $db->query("DELETE FROM payment_allocations WHERE payment_id = ?", array($pid));

            // Delete journal entry + transaction lines for this payment
            ...
            // Delete pending bank_transaction bridged from this payment
            $db->query(
                "DELETE FROM bank_transactions WHERE reference_number = ? AND status = 'pending'",
                array($pay->payment_number)
            );

            // Delete customer_ledger entries for this payment
            $db->query(
                "DELETE FROM customer_ledger WHERE reference_type = 'customer_payments' AND reference_id = ?",
                array($pid)
            );

            // Delete the payment record itself
            $db->query("DELETE FROM customer_payments WHERE id = ?", array($pid));
    ...
    $paymentsNote = !empty($paymentsCascaded)
        ...
        : '';
    auditLog('credit_orders', 'deleted', "Credit order {$orderNumDisplay} deleted by {$userDisplay} (was: {$order->status}); accounting reversed: balance_due ৳{$order->balance_due}{$paymentsNote}", [
        'record_id'             => $order_id,
