...
// HARD role guard: clearance is an ACCOUNTS function. The privilege toggle alone
// is not enough — a mis-ticked checkbox must never hand clearance power to
// production/dispatch/sales roles. Only accounts-family roles (plus admins)
// may grant or revoke, and they still need the privilege toggle as well.
$is_accounts_role = in_array($user_role, ['Accounts', 'accounts-srg', 'accounts-demra']);

$can_grant  = $is_admin || ($is_accounts_role && userCanPageAction('credit_sales', 'payment_watch', 'can_grant_clearance'));
$can_revoke = $is_admin || ($is_accounts_role && userCanPageAction('credit_sales', 'payment_watch', 'can_revoke_clearance'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'grant_clearance') {
    if (!$can_grant) {
        ...
        try {
            ...
            // Amount-based conditions can only be cleared EARLY by an admin.
            // Accounts may grant once the numbers are actually satisfied;
            ...
            if (!$is_admin && $cond->condition_type !== 'manual') {
                ...
                if (!in_array($g['dispatch'], ['condition_met', 'cleared'])) {
                    ...
                    throw new Exception("Payment condition for {$cond->order_number} is NOT met yet{$short}. "
                        . "Only an admin can clear it early.");
                }
...
<?php foreach ($held_orders as $h):
    ...
    if ($threshold !== null && $current !== null && $threshold > 0) {
        if ($h->condition_type === 'outstanding_below') {
    ...
    <div class="bg-white rounded-xl border shadow-sm overflow-hidden
        ...
        <div class="px-5 py-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="flex flex-col justify-center gap-2">
                <?php
                // Non-admins may only grant amount-based conditions AFTER they are met
                $grant_locked = !$is_admin && $h->condition_type !== 'manual' && !$cond_met;
                ?>
                ...
                <div class="text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg p-3">
                    ...
                    Early release requires an admin (from the order page).
                </div>
                ...
                <form method="POST" class="space-y-2"
                    ...
                    <button type="submit"
                        ...
                        <?php if (!$cond_met && $is_admin): ?><span class="font-normal text-xs">(admin early release)</span><?php endif; ?>
                    </button>
                </form>
                <?php elseif ($is_cleared): ?>
