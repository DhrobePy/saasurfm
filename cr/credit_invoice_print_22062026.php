...
$invoice_date = $order->shipped_date ?? $order->order_date;
...
.co-bar{width:38px;height:2.5px;margin:7px 0;background:linear-gradient(to right,var(--bll),var(--teal));border-radius:2px}
.co-meta{font-size:9.5px;color:#94a3b8;margin:2px 0;display:flex;align-items:center;gap:5px}

.inv-block{position:relative;z-index:1;text-align:right}
...
.inv-num{font-size:12.5px;font-weight:700;color:#e2e8f0}
...
@media print{
  ...
  /* ── Header ── */
  .inv-header{padding:16px 22px 13px!important}
  .co-bn    {font-size:19px!important}
  .co-en    {font-size:9.5px!important}
  .co-bar   {margin:5px 0!important}
  .co-meta  {font-size:9px!important;margin:1.5px 0!important}
  .inv-word {font-size:28px!important}
  .inv-num  {font-size:11.5px!important}
  ...
  <div class="content">
    ...
    <table class="itbl">
      ...
      foreach ($items as $item):
        ...
        /* Clean quantity: strip trailing zeros, no unit label */
      ...
      ?>
        <tr>
          ...
          <td>
            ...
            <?php if ($vd): ?><div class="p-meta"><?php echo htmlspecialchars(implode(' · ',$vd)); ?></div><?php endif; ?>
          </td>
          <td class="c">
