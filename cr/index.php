...
----------------------------- */
//$allowed_roles = ['Superadmin', 'admin', 'Accounts','accounts-srg', 'accounts-demra','sales-srg', 'sales-demra', 'sales-other','production manager-srg', 'production manager-demra','dispatch-srg', 'dispatch-demra', 'dispatchpos-demra', 'dispatchpos-srg','collector'];
...
$is_accounts    = userCanPageAction('credit_sales', 'credit_order_approval', 'can_approve')
               || userCanPageAction('credit_sales', 'customer_payment', 'can_collect');
...
if ($is_production) {
    if ($user_branch !== null) {
        $in_prod = $db->query("
            ...
            FROM credit_orders
            WHERE status IN ('approved', 'in_production') AND assigned_branch_id = ?
        ", [$user_branch])->first();
    } else {
        $in_prod = $db->query("
            SELECT COUNT(*) AS count
...
if ($is_dispatcher) {
    if ($user_branch !== null) {
        $ready = $db->query("
            SELECT COUNT(*) AS count
            FROM credit_orders
            WHERE status = 'ready_to_ship' AND assigned_branch_id = ?
        ", [$user_branch])->first();
    } else {
        $ready = $db->query("
            SELECT COUNT(*) AS count
...
<div class="bg-white rounded-lg shadow-md mb-8 p-4 border border-gray-200">
    <div class="flex flex-wrap gap-4 justify-center md:justify-start">
        ...
        <a href="all_sales.php" class="flex flex-col items-center p-3 text-gray-600 hover:bg-gray-100 hover:text-gray-800 rounded-lg transition-colors" title="All Sales">
            <i class="fas fa-list-alt text-2xl mb-1"></i>
            <span class="text-xs font-medium">All Sales</span>
        </a>
        <?php endif; ?>
        ...
        <?php if (!empty($orders)): ?>
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-primary-50 transition-colors" data-order-id="<?php echo $order->id; ?>">
                        <td class="px-6 py-4 text-sm font-medium text-primary-700">
                            ...
                            <span class="ml-1 inline-block px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-purple-100 text-purple-800" title="Other Sales — direct commodity resale">OS</span>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm text-right font-medium <?php echo ($order->due_amount ?? 0) > 0 ? 'text-red-600' : 'text-green-600'; ?>">
                        ...
                        <td class="px-6 py-4 text-center text-sm font-medium">
                            ...
                            <a href="credit_invoice_print.php?id=<?php echo $order->id; ?>" target="_blank" class="text-green-600 hover:text-green-900 px-2" title="Print Invoice">
                            ...
                            <?php
                                ...
                                if ($is_superadmin) {
                                    ...
                                    $can_delete = in_array($order->status, ['pending_approval', 'escalated'])
                                               && ($is_admin || $order->created_by_user_id == $user_id)
                                               && $order->amount_paid == 0;
                                }
                                ?>
                            ...
                            <?php if ($can_delete): ?>
                                <button 
                                    type="button"
                                    onclick="deleteOrder(<?= $order->id ?>, '<?= htmlspecialchars($order->order_number ?? 'CR-'. $order->id) ?>')"
                                    class="inline-flex items-center px-2.5 py-1.5 text-xs font-medium rounded text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-red-500 ml-1"
                                    title="Delete this order (only possible in pending/escalated stage)"
                                >
