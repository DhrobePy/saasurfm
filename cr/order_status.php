...
// 'order_status' is claimed by both the credit_sales and production modules
// (see getModuleRegistry()) — bare restrict_access() auto-detects only the
// first (credit_sales), so a grant made under 'production' alone was silently
// ignored (same bug class fixed in credit_dispatch.php). Confirmed real impact
// live: dispatch-demra and production manager-srg users had a working
// 'production' grant for this page that restrict_access() never checked.
// Accept a grant from either module; otherwise fall through to the role list
// the page's own logic already expects (see $is_accounts/$is_production/
// $is_sales/$is_dispatch below).
$allowed_roles = [
    'Superadmin', 'admin', 'Accounts', 'accounts-srg', 'accounts-demra',
    'accountspos-srg', 'accountspos-demra',
...
if (!userHasPageGrant('production', 'order_status') && !userHasPageGrant('credit_sales', 'order_status')) {
    restrict_access($allowed_roles, 'credit_sales', 'order_status');
}

ensureCreditOrderHoldStatus();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        // Bug 10 fix: CSRF verification on all AJAX POST actions
        $sess_tok = $_SESSION['csrf_token'] ?? '';
        $recv_tok = $_POST['csrf_token'] ?? $_SERVER['HTTP_X_CSRF_TOKEN'] ?? '';
        ...
        switch ($_POST['action']) {
            ...
            case 'update_priority':
                ...
                try {
                    // Update sort_order directly based on position in the dragged array
                    $update_stmt = $db->getPdo()->prepare(
                        "UPDATE credit_orders SET sort_order = ? WHERE id = ?"
                    );
                    foreach ($order_ids as $position => $oid) {
                        $update_stmt->execute([$position + 1, (int)$oid]);
                    }
            ...
            case 'update_status':
                ...
                $allowed_statuses = ['draft','pending_approval','approved','in_production',
                                     'produced','ready_to_ship','goods_on_board','shipped','delivered','cancelled','hold'];

                if (!in_array($new_status, $allowed_statuses)) {
                    throw new Exception('Invalid status');
                }

                // Bug 19 fix: Enforce valid workflow transitions — arbitrary status jumps
                // are not allowed even for admins.
                $current_order = $db->query("SELECT status FROM credit_orders WHERE id = ?", [$order_id])->first();
                if (!$current_order) throw new Exception('Order not found');

                $current = $current_order->status;
                $valid_transitions = [
                    'draft'            => ['pending_approval', 'cancelled'],
                    'pending_approval' => ['approved', 'rejected', 'escalated', 'cancelled'],
                    'escalated'        => ['approved', 'rejected', 'cancelled'],
                    'approved'         => ['in_production', 'cancelled', 'hold'],
                    'hold'             => ['approved', 'cancelled'],
                    'in_production'    => ['produced', 'cancelled', 'hold'],
                    'produced'         => ['ready_to_ship', 'hold'],
                    'ready_to_ship'    => ['goods_on_board', 'shipped', 'hold'],
                    'goods_on_board'   => ['shipped', 'delivered', 'hold'],
                    'shipped'          => ['delivered'],
                    'delivered'        => ['cancelled'],   // admin-only reversal
                    'cancelled'        => [],
                    'rejected'         => [],
                ];
                $allowed_next = $valid_transitions[$current] ?? [];
                if (!in_array($new_status, $allowed_next)) {
                    throw new Exception("Invalid transition: '{$current}' → '{$new_status}' is not permitted.");
                }
                ...
                $db->insert('credit_order_workflow', [
                    ...
                    'comments'            => 'Admin manual status update via Order Status page',
                ]);
            ...
            case 'bulk_update_status':
                ...
                // This action was previously unvalidated — a typo in the raw JS
                // prompt() that collects $new_status went straight to the database,
                // silently failing (Database wrapper swallows the ENUM-constraint
                // error) on every selected order while still reporting "N orders
                // updated". Reuse the same allow-list the single-order update uses.
                $bulk_allowed_statuses = ['draft','pending_approval','approved','in_production',
                                           'produced','ready_to_ship','goods_on_board','shipped','delivered','cancelled','hold'];
                if (!in_array($new_status, $bulk_allowed_statuses, true)) {
                    throw new Exception("Invalid status '{$new_status}'. Must be one of: " . implode(', ', $bulk_allowed_statuses));
                }
                ...
                foreach ($order_ids as $order_id) {
                    ...
                    if ($db->error()) {
                        throw new Exception('Failed to update order #' . (int)$order_id . ': ' . ($db->errorInfo()[2] ?? 'unknown error'));
                    }
                    $updated_count++;
                }
                ...
                echo json_encode([
                    'success' => true,
                    'message' => $updated_count . ' orders updated',
                    'count' => $updated_count
                ]);
                break;
                
            case 'collect_payment':
                // Bug 9 fix: Previous implementation was a "shadow payment" — it only
                // updated credit_orders.amount_paid but created NO customer_payments record,
                // NO customer_ledger entry, NO journal entry, and did NOT update
                // customers.current_balance.  Money collected was invisible to accounting.
                if (!$is_accounts) {
                    throw new Exception('Unauthorized action');
                }
                ...
                $order = $db->query(
                    "SELECT co.*, c.current_balance as cust_balance, c.initial_due
                     FROM credit_orders co
                ...
                $outstanding = (float)$order->balance_due;
                if ($amount <= 0) throw new Exception('Payment amount must be positive');
                if ($amount > $outstanding + 0.01) throw new Exception('Payment amount exceeds outstanding balance');

                // Resolve accounts (reuse file-scope lookups when available, otherwise fetch)
                $cash_account_row = $db->query(
                    "SELECT id FROM chart_of_accounts WHERE account_type = 'Petty Cash' OR name = 'Undeposited Funds' ORDER BY CASE WHEN name = 'Undeposited Funds' THEN 0 ELSE 1 END LIMIT 1"
                )->first();
                $ar_account_row = $db->query(
                    "SELECT id FROM chart_of_accounts WHERE account_type = 'Accounts Receivable' LIMIT 1"
                )->first();
                if (!$cash_account_row || !$ar_account_row) throw new Exception('Chart of Accounts not configured correctly');

                $deposit_account_id = $cash_account_row->id; // Default: cash
                if ($payment_method !== 'Cash') {
                    $bank_account_id = (int)($_POST['bank_account_id'] ?? 0);
                    if ($bank_account_id) {
                        $ba = $db->query("SELECT chart_of_account_id FROM bank_accounts WHERE id = ?", [$bank_account_id])->first();
                        if ($ba) $deposit_account_id = $ba->chart_of_account_id;
                    }
                }
                ...
                $seq = $last_pay ? ((int)substr($last_pay->payment_number, -4) + 1) : 1;
                $payment_number = sprintf("PAY-%s-%04d", $date_prefix, $seq);

                // 2. Insert customer_payments record
                $payment_id = $db->insert('customer_payments', [
                    ...
                    'allocated_to_invoices'=> json_encode([$order_id => $amount]),
                    'notes'                => "Collected via Order Status page — Order #{$order->order_number}",
                    'created_by_user_id'   => $user_id,
                ]);
                if (!$payment_id) throw new Exception('Failed to create payment record');

                // 3. Update credit_orders
                $db->query(
                    "UPDATE credit_orders SET amount_paid = amount_paid + ?, balance_due = balance_due - ? WHERE id = ?",
                    [$amount, $amount, $order_id]
                );

                // 4. customer_ledger entry
                $prev_led = $db->query(
                    "SELECT balance_after FROM customer_ledger WHERE customer_id = ? ORDER BY id DESC LIMIT 1",
                    [$order->customer_id]
                )->first();
                $prev_bal = $prev_led ? (float)$prev_led->balance_after : (float)$order->initial_due;
                ...
                $ledger_id = $db->insert('customer_ledger', [
                    ...
                    'description'        => "Payment received for order #{$order->order_number} ({$payment_method})",
                    'debit_amount'       => 0,
                ...
                // 5. Update customer current_balance atomically
                $db->query(
                    "UPDATE customers SET current_balance = current_balance - ? WHERE id = ?",
                    [$amount, $order->customer_id]
                );

                // 6. Double-entry journal (Debit Cash/Bank, Credit AR)
                $journal_id = $db->insert('journal_entries', [
                    'transaction_date'     => date('Y-m-d'),
                    'description'          => "Payment {$payment_number} from customer #{$order->customer_id}",
                    'related_document_type'=> 'customer_payments',
                    'related_document_id'  => $payment_id,
                    'created_by_user_id'   => $user_id,
                ]);
                $db->insert('transaction_lines', ['journal_entry_id' => $journal_id, 'account_id' => $deposit_account_id, 'debit_amount' => $amount, 'credit_amount' => 0]);
                $db->insert('transaction_lines', ['journal_entry_id' => $journal_id, 'account_id' => $ar_account_row->id, 'debit_amount' => 0, 'credit_amount' => $amount]);

                // 7. Back-link journal to payment and ledger
                ...
                echo json_encode([
                    'success'     => true,
                    'message'     => "Payment ৳" . number_format($amount, 2) . " recorded (Receipt #{$payment_number})",
            ...
            case 'get_order_timeline':
                ...
                // Workflow history (main timeline source)
                $timeline = $db->query(
                    "SELECT
                        cow.action      AS action_type,
                        cow.from_status AS old_value,
                        cow.to_status   AS new_value,
                        cow.comments    AS field_name,
                        cow.created_at,
                ...
                // Order items for the modal
                $modal_items = $db->query(
                    "SELECT coi.quantity, coi.unit_price, coi.line_total,
                ...
                // Order header info
                $modal_order = $db->query(
                    "SELECT co.order_number, co.total_amount, co.balance_due, co.amount_paid,
                            co.status, c.name AS customer_name
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <div>
                ...
                <select name="status" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <option value="goods_on_board" <?php echo $status_filter === 'goods_on_board' ? 'selected' : ''; ?>>Goods on Board</option>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody id="ordersTableBody" class="bg-white divide-y divide-gray-200">
                ...
                <tr class="order-row hover:bg-gray-50 transition" data-order-id="<?php echo $order->id; ?>">
                    ...
                    <!-- Priority -->
                    <td class="px-6 py-4">
                        <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full priority-<?php echo $order->priority ?? 'normal'; ?>">
                            <?php echo ucfirst($order->priority ?? 'normal'); ?>
                        </span>
...
<div id="timelineModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg shadow-2xl max-w-3xl w-full animate-fade-in max-h-[92vh] flex flex-col">
        <!-- Modal Header -->
        <div class="flex items-center justify-between p-5 border-b bg-gray-50 rounded-t-lg">
            <div>
                <h3 class="text-xl font-bold text-gray-900 flex items-center gap-2" id="timelineModalTitle">
                    <i class="fas fa-receipt text-indigo-600"></i> Order Details
                </h3>
                <p class="text-sm text-gray-500 mt-0.5" id="timelineModalSubtitle">Loading...</p>
            </div>
            <button onclick="closeTimelineModal()" class="text-gray-400 hover:text-gray-600 transition p-1">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>

        <!-- Tabs -->
        <div class="flex border-b px-5 bg-white" id="timelineTabs">
            <button onclick="switchTab('items')" id="tab-items"
                    class="px-4 py-3 text-sm font-medium border-b-2 border-indigo-600 text-indigo-700 mr-2">
                <i class="fas fa-boxes mr-1"></i> Order Items
            </button>
            <button onclick="switchTab('timeline')" id="tab-timeline"
                    class="px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700">
...
<script>
// Bug 10 fix: Expose CSRF token for all AJAX POST requests.
...
function updateOrderPriority() {
    ...
    fetch(window.location.href, {
        ...
        body: `action=update_priority&csrf_token=${encodeURIComponent(CSRF_TOKEN)}&order_ids=${JSON.stringify(orderIds)}`
    })
...
// ── Order Details Modal (Timeline + Items) ──────────────────
let _modalData = null;  // cached response data
...
function showTimeline(orderId) {
    ...
    document.getElementById('timelineModalSubtitle').textContent = 'Loading…';
    contentDiv.innerHTML = `<div class="text-center py-10"><i class="fas fa-spinner fa-spin text-4xl text-gray-400"></i><p class="text-gray-500 mt-3">Loading order details…</p></div>`;

    // Reset tabs
    document.getElementById('tab-items').className    = 'px-4 py-3 text-sm font-medium border-b-2 border-indigo-600 text-indigo-700 mr-2';
    document.getElementById('tab-timeline').className = 'px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700';
    ...
    .then(data => {
        ...
        if (data.order) {
            document.getElementById('timelineModalTitle').innerHTML =
                `<i class="fas fa-receipt text-indigo-600"></i> ${escapeHtml(data.order.order_number)}`;
            document.getElementById('timelineModalSubtitle').textContent =
                `${escapeHtml(data.order.customer_name)} — Status: ${(data.order.status||'').replace(/_/g,' ')}`;
        }
    ...
    .catch(err => {
        contentDiv.innerHTML = `<div class="text-center py-10">
            ...
            <p class="text-red-600 font-semibold">Failed to load</p>
            <p class="text-sm text-gray-500 mt-2">${escapeHtml(err.message)}</p>
            <button onclick="showTimeline(${orderId})" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
...
function switchTab(tab) {
    ...
    const active   = 'px-4 py-3 text-sm font-medium border-b-2 border-indigo-600 text-indigo-700 mr-2';
    const inactive = 'px-4 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700';
    tabItems.className    = tab === 'items'    ? active : inactive + ' mr-2';
    tabTimeline.className = tab === 'timeline' ? active : inactive;
    if (_modalData) renderTab(tab);
}
...
function renderTab(tab) {
    ...
    if (tab === 'items') {
        const items = _modalData.items || [];
        const order = _modalData.order || {};
        if (items.length === 0) {
            contentDiv.innerHTML = `<div class="text-center py-8 text-gray-500"><i class="fas fa-box-open text-3xl mb-2 block text-gray-300"></i>No items found</div>`;
            return;
        }
        let html = `<table class="min-w-full text-sm divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
        ...
        items.forEach(item => {
            const variant = [item.grade, item.weight_variant].filter(Boolean).join(' ');
            html += `<tr class="hover:bg-gray-50">
                <td class="px-3 py-2">
                    <div class="font-medium text-gray-900">${escapeHtml(item.product_name)}</div>
                    ${variant ? `<div class="text-xs text-gray-400">${escapeHtml(variant)}</div>` : ''}
                </td>
                <td class="px-3 py-2 text-right">${parseFloat(item.quantity||0).toFixed(2)} ${escapeHtml(item.unit_of_measure||'')}</td>
                <td class="px-3 py-2 text-right">৳${parseFloat(item.unit_price||0).toFixed(2)}</td>
                <td class="px-3 py-2 text-right font-semibold">৳${parseFloat(item.line_total||0).toFixed(2)}</td>
            </tr>`;
        });
        html += `</tbody>
            <tfoot class="bg-gray-50 border-t-2 border-gray-300 text-sm font-semibold">
                <tr><td colspan="3" class="px-3 py-2 text-right">Total Amount:</td>
                    <td class="px-3 py-2 text-right text-blue-700">৳${parseFloat(order.total_amount||0).toFixed(2)}</td></tr>
                <tr><td colspan="3" class="px-3 py-2 text-right">Amount Paid:</td>
                    <td class="px-3 py-2 text-right text-green-700">৳${parseFloat(order.amount_paid||0).toFixed(2)}</td></tr>
                <tr><td colspan="3" class="px-3 py-2 text-right">Balance Due:</td>
                    <td class="px-3 py-2 text-right text-red-600">৳${parseFloat(order.balance_due||0).toFixed(2)}</td></tr>
            </tfoot>
        </table>`;
        contentDiv.innerHTML = html;

    } else { // timeline
        const timeline = _modalData.timeline || [];
        if (timeline.length === 0) {
            contentDiv.innerHTML = `<div class="text-center py-8 text-gray-500">
                <i class="fas fa-clock text-3xl mb-2 block text-gray-300"></i>
                <p>No workflow events yet.</p><p class="text-xs mt-1 text-gray-400">Events appear as the order progresses.</p></div>`;
            return;
        }
        const actionIcons = {
            'approve':  'fa-check-circle text-green-600',
            'reject':   'fa-times-circle text-red-600',
            'create':   'fa-plus-circle text-blue-600',
            'produce':  'fa-industry text-purple-600',
            'ship':     'fa-truck text-teal-600',
            'deliver':  'fa-box-open text-green-600',
            'escalate': 'fa-exclamation-circle text-orange-600',
            'payment':  'fa-money-bill-wave text-emerald-600',
        };
        let html = '<div class="relative"><div class="absolute left-4 top-0 bottom-0 w-0.5 bg-gray-200"></div><div class="space-y-3">';
        timeline.forEach(event => {
            const icon = actionIcons[event.action_type] || 'fa-arrow-right text-gray-500';
            const actionLabel = (event.action_type||'').replace(/_/g,' ').replace(/\b\w/g, l => l.toUpperCase());
            let dateStr = event.created_at || '';
            try { dateStr = new Date(event.created_at).toLocaleString('en-GB', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit'}); } catch(e){}

            const fromStatus = (event.old_value||'').replace(/_/g,' ');
            const toStatus   = (event.new_value||'').replace(/_/g,' ');
            const comment    = event.field_name || '';

            html += `<div class="flex gap-3 relative">
                <div class="flex-shrink-0 z-10">
                    <div class="w-8 h-8 rounded-full bg-white border-2 border-gray-200 flex items-center justify-center shadow-sm">
                        <i class="fas ${icon} text-xs"></i>
                    </div>
                </div>
                <div class="flex-1 bg-gray-50 rounded-lg p-3 min-h-[56px]">
                    <div class="flex justify-between">
                        <span class="font-semibold text-sm text-gray-800">${escapeHtml(actionLabel)}</span>
                        <span class="text-xs text-gray-400">${escapeHtml(dateStr)}</span>
                    </div>
                    <p class="text-xs text-gray-500 mt-0.5">
                        <span class="bg-gray-200 px-1 rounded">${escapeHtml(fromStatus)}</span>
                        <i class="fas fa-arrow-right mx-1 text-gray-400"></i>
                        <span class="bg-blue-100 text-blue-700 px-1 rounded">${escapeHtml(toStatus)}</span>
                    </p>
                    ${comment ? `<p class="text-xs text-gray-600 mt-1 italic">${escapeHtml(comment)}</p>` : ''}
                    <p class="text-xs text-gray-400 mt-1"><i class="fas fa-user mr-1"></i>${escapeHtml(event.user_name||'System')}</p>
                </div>
            </div>`;
        });
        html += '</div></div>';
...
function bulkUpdateStatus() {
    ...
    const status = prompt('Enter new status (draft, pending_approval, approved, in_production, produced, ready_to_ship, goods_on_board, shipped, delivered, cancelled, hold):');
