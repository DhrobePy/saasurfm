...
// A user with an explicit delegated ৳ approve_order limit can approve escalated
// orders (both the 80%-usage tier and, when the limit covers the amount, a genuine
// credit-limit breach — see Feature #9 below), so they must also be able to SEE
// escalated orders in their queue — not just role admins / override-privilege users.
// (getUserActionLimit self-migrates its own tables, safe to call this early.)
$has_delegated_approval_limit = !$is_role_admin && getUserActionLimit((int)$user_id, 'approve_order', true) !== null;

// $is_admin governs queue visibility (escalated orders) and stats
$is_admin = $is_role_admin || $has_escalate_override || $has_delegated_approval_limit;

// Feature #9: over-credit-limit orders need a Superadmin, OR an approver whose
// delegated ৳ limit covers the full order amount (checked per-order in the POST handler).
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'approve') {
    ...
    if (!$branch_id) {
        ...
        try {
            ...
            // Feature #9: an order that breaches the customer's CREDIT LIMIT normally
            // requires a Superadmin — UNLESS the approver has an explicit delegated ৳
            // approve_order limit (admin/privileges.php) that covers the FULL order
            // amount. A configured personal limit is explicit delegated authority and
            // is honored even for a genuine credit-limit breach, not just the
            // 80%-usage tier. Checked unconditionally (even for role admins) so it's
            // the explicit ৳ limit that decides — not merely holding the admin role.
            // The escalate-override privilege alone (no ৳ limit) still does NOT
            // unlock a real credit-limit breach — only an explicit ৳ limit or the
            // Superadmin role does.
            ...
            $breach_override_limit  = getUserActionLimit((int)$user_id, 'approve_order', true);
            $breach_covered_by_limit = $breach_override_limit !== null
                                      && (float)$order->total_amount <= $breach_override_limit;

            if ($over_credit_limit && !$is_superadmin_role && !$breach_covered_by_limit) {
                ...
                throw new Exception("This order exceeds the customer's credit limit by ৳"
                    ...
                    . " — only a Superadmin, or an approver with a delegated ৳ limit covering the full order amount, can approve it. It remains escalated for Superadmin review.");
            }
            ...
            if ($must_escalate) {
                ...
                // Other Sales orders skip production entirely — approval sends
                // them straight to 'ready_to_ship', reusing credit_dispatch.php's
                // existing status-based eligibility with zero changes there.
                $new_status = !empty($order->is_other_sales) ? 'ready_to_ship' : 'approved';
                $action = !empty($order->is_other_sales) ? 'approved (Other Sales — direct to Ready-to-Ship)' : 'approved';
            ...
            // Freely choosable (incl. past dates, for reconciliation) for Superadmin/
            // admin; enforced server-side too — the client-side min= was previously
            // decorative only, so a non-admin could bypass it via a tampered request.
            ...
            if (!$is_role_admin && $required_date < date('Y-m-d')) {
                throw new Exception("Required delivery date cannot be in the past.");
            }
            $db->query(
            ...
            // New rule: an order that crosses the customer's CREDIT LIMIT auto-attaches
            // a payment-linked ship hold — production/ready-to-ship proceed normally,
            // and the order auto-releases to ship once the customer's total due
            // (incl. this invoice) is paid back within their limit. Forced regardless
            // of the approver's special-instructions panel, so the policy is uniform.
            // Off by default (admin/settings.php → "Auto-release over-limit orders").
            $force_credit_limit_hold = creditLimitAutoReleaseEnabled() && $over_credit_limit && $new_status === 'approved';
            if ($force_credit_limit_hold) {
                $disp_hold = 1;
            }
            $conditions_summary = '';
            ...
            if (($prod_hold || $disp_hold) && $new_status === 'approved') {
                if ($force_credit_limit_hold) {
                    $cond_type    = 'outstanding_after_ship';
                    $cond_amount  = (float)$customer->credit_limit;
                    $auto_release = 1;
                } else {
                ...
                // Both calls were previously unchecked — a failed DELETE or INSERT here
                // silently left the order without its intended production/dispatch hold,
                // since Database::query()/::insert() never throw on failure. condition_type
                // is UNIQUE-keyed per order_id, so a failed DELETE also risks a duplicate-key
                // failure on the INSERT that follows.
                $db->query("DELETE FROM order_approval_conditions WHERE order_id = ?", [$order_id]);
                if ($db->error()) {
                    throw new Exception('Failed to clear previous approval conditions for this order.');
                }
                $condition_inserted = $db->insert('order_approval_conditions', [
                    'order_id'            => $order_id,
                ...
                if (!$condition_inserted) {
                    throw new Exception('Failed to save approval conditions (production/dispatch hold) for this order.');
                }
...
<?php foreach ($orders as $order): 
    ...
    // Mirrors the POST-handler check: a delegated ৳ limit covering the FULL order
    // amount unlocks even a genuine credit-limit breach (credit_usage_pct >= 999).
    $breach_covered_by_limit = $my_approval_limit !== null && (float)$order->total_amount <= $my_approval_limit;
?>
<div class="bg-white rounded-lg shadow-md mb-3 overflow-hidden">
    ...
    <div class="p-6 bg-gray-50">
        ...
        <?php if ($credit_usage_pct >= 999 && creditLimitAutoReleaseEnabled()): ?>
        <div class="mb-4 p-3 bg-blue-50 border border-blue-300 rounded-lg text-sm text-blue-900">
            <i class="fas fa-truck-ramp-box mr-1"></i>
            This order is over the customer's <strong>৳<?php echo number_format((float)$order->credit_limit, 0); ?> credit limit</strong>.
            If approved, production proceeds normally, but <strong>shipping will auto-hold</strong> until the customer's
            total due (including this invoice) is paid back to ৳<?php echo number_format((float)$order->credit_limit, 0); ?>
            or below — it will then <strong>ship automatically</strong>, no manual clearance needed.
        </div>
        <?php endif; ?>

        <?php // Feature #9: over-credit-limit orders need a Superadmin, OR a delegated
        // ৳ limit covering the full order amount — hide the approve form otherwise.
        if ($credit_usage_pct >= 999 && !$is_superadmin && !$breach_covered_by_limit): ?>
        <div class="p-4 bg-red-50 border-2 border-red-300 rounded-lg text-red-800 text-sm">
            ...
            escalated. Only a Superadmin, or an approver with a delegated ৳ limit covering the full order amount, can
            approve it — you can still <strong>reject</strong> it below if needed.
        </div>
        <?php elseif ($credit_usage_pct >= 999 && $breach_covered_by_limit): ?>
        <div class="mb-4 p-3 bg-amber-50 border border-amber-300 rounded-lg text-sm text-amber-800">
            <i class="fas fa-gavel mr-1"></i>
            This order is over the customer's <strong>credit limit</strong>, not just the 80% usage tier — but your
            delegated approval limit of <strong>৳<?php echo number_format($my_approval_limit, 0); ?></strong> covers
            it, so you can approve it below. This will be stamped in the order history.
        </div>
        <?php endif; ?>
        <?php if (!($credit_usage_pct >= 999 && !$is_superadmin && !$breach_covered_by_limit)): ?>
        <!-- Approve Form -->
        <form method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    <?php echo !empty($order->is_other_sales) ? 'Confirm Source Branch / Warehouse / Dock *' : 'Assign to Branch for Production *'; ?>
                </label>
                <?php if (!empty($order->is_other_sales)): ?>
                <p class="text-xs text-purple-600 mb-2"><i class="fas fa-right-left mr-1"></i>Other Sales order — no production step. This is the source picked at order creation; change it only if it's wrong.</p>
                <?php endif; ?>
                <select name="branch_id" required class="w-full px-4 py-2 border rounded-lg">
                    <option value="">-- Select Branch --</option>
                    <?php foreach ($branches as $branch): ?>
                    <option value="<?php echo $branch->id; ?>" <?php echo ((int)$order->assigned_branch_id === (int)$branch->id) ? 'selected' : ''; ?>><?php echo htmlspecialchars($branch->name); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" name="required_date" required class="w-full px-4 py-2 border rounded-lg"
                       ...
                       <?php if (!$is_role_admin): ?>min="<?php echo date('Y-m-d'); ?>"<?php endif; ?>>
                <p class="text-xs text-gray-500 mt-1">
                    Delivery by: <?php echo $req_date_lbl; ?> — adjust if needed
                    <?php if ($is_role_admin): ?><span class="text-amber-600">(admin: past dates allowed, for reconciliation)</span><?php endif; ?>
                </p>
            </div>
