...
try {
    ...
    if ($inv) {
        ...
        if ($db->error()) throw new Exception('Failed to restore inventory quantity while undoing this sale.');
    }
    ...
    if (!empty($ledger_entries)) {
        ...
        // from that point forward permanently stale, with the delete still commit
        // and report success (same class already fixed in cr/delete_payment.php).
