...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'collect_commodity_payment') {
    try {
        ...
        $dr_line_id = $db->insert('transaction_lines', ['journal_entry_id' => $journal_id, 'account_id' => $deposit_account_id, 'debit_amount' => $amount, 'credit_amount' => 0, 'description' => $journal_desc]);
        ...
        // $cl_id is stored on commodity_sale_payments.customer_ledger_id, and
        // delete_commodity_payment.php locates this exact row by it to reverse the
        // ledger effect — a silently swallowed insert failure here (Database::insert()
        // never throws) would leave $cl_id false, the customer under-credited, and the
        // reversal flow permanently unable to find a ledger row to undo.
        ...
        if (!$cl_id) throw new Exception('Failed to post the customer ledger entry for this payment.');
        $db->query("UPDATE customers SET current_balance = ? WHERE id = ?", [$new_balance, $sale_row->customer_id]);
        if ($db->error()) throw new Exception('Failed to update the customer cached balance for this payment.');
        ...
        // Previously unchecked: a silent failure here would post the payment/journal/ledger
        // ("receipt succeeds") while the sale still shows its full balance due — a real risk
        // of double-collecting from the customer later.
        $db->query(
            "UPDATE commodity_sales SET amount_paid = amount_paid + ?, balance_due = total_amount - advance_paid - amount_paid WHERE id = ?",
            [$amount, $sid]
        );
        if ($db->error()) throw new Exception('Failed to update the commodity sale paid amount/balance for this payment.');
        ...
        if ($db->error()) throw new Exception('Failed to link the journal entry to this payment.');
