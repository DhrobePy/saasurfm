...
if ($supplier->current_balance > ($supplier->credit_limit * 0.8)) {
    $warnings[] = [
        ...
        'message' => "Outstanding balance at " . number_format(($supplier->current_balance / max(1, $supplier->credit_limit) * 100), 0) . "% of credit limit. Payment collection urgently needed.",
        'severity' => 'high'
    ];
}
...
<div class="w-full px-4 py-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6 print:grid-cols-5">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <p class="text-2xl font-bold text-blue-600 mt-2">৳<?php echo number_format(($stats->total_orders_value ?? 0) / 1000000, 1); ?>M</p>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
            <p class="text-sm font-medium text-gray-600">Outstanding</p>
            <p class="text-2xl font-bold text-red-600 mt-2">৳<?php echo number_format(($stats->current_balance ?? 0) / 1000000, 1); ?>M</p>
        </div>
    </div>
