...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_admin) {
    $del_id = (int)($_POST['note_id'] ?? 0);
    if ($_POST['action'] === 'delete' && $del_id) {
        $r = $po_manager->deleteAdjustmentNote($del_id, $currentUser['id'], $currentUser['display_name'] ?? 'Admin');
        $_SESSION[$r['success'] ? 'success' : 'error'] = $r['message'];
    }
    header('Location: purchase_adnan_adjustments.php?' . http_build_query(array_filter([
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center whitespace-nowrap">
                            ...
                            <a href="purchase_adnan_view_adjustment.php?id=<?php echo $note->id; ?>"
                               class="text-green-600 hover:text-green-800 mr-2" title="Approve / Post">
                                <i class="fas fa-check-circle"></i>
                            </a>
                            <?php endif; ?>
                            <?php if ($is_admin): ?>
                            <form method="POST" class="inline"
                                  onsubmit="return confirm('<?php echo $note->status === 'posted'
                                      ? 'DELETE posted note ' . htmlspecialchars($note->note_number, ENT_QUOTES) . '? This will REVERSE its financial effect on the PO.'
                                      : 'Delete ' . htmlspecialchars($note->note_number, ENT_QUOTES) . '?'; ?>')">
