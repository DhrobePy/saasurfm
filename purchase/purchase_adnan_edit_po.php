...
// Admin and Superadmin can edit POs
...
$user_role_ep  = $currentUser['role'] ?? '';
$is_superadmin = ($user_role_ep === 'Superadmin');
...
// Phase 2: this PO's commodity + its origins/unit (commodity is fixed on edit —
// a PO may already have GRNs; changing origin/qty/price stays within the commodity).
ensureProcurementCatalogTables();
$pc_db = Database::getInstance();
$po_commodity = null;
if (!empty($po->commodity_id)) {
    $po_commodity = $pc_db->query("SELECT id, name, unit FROM purchase_commodities WHERE id = ?", [(int)$po->commodity_id])->first();
}
if (!$po_commodity) {
    $po_commodity = $pc_db->query("SELECT id, name, unit FROM purchase_commodities WHERE name = 'Wheat' LIMIT 1")->first();
}
$po_unit = $po->unit ?? ($po_commodity->unit ?? 'KG');
$po_origins = [];
if ($po_commodity) {
    foreach ($pc_db->query("SELECT origin_name FROM purchase_commodity_origins WHERE commodity_id = ? AND status = 'active' ORDER BY origin_name ASC", [(int)$po_commodity->id])->results() as $o) {
        $po_origins[] = $o->origin_name;
    }
}
// Always include the PO's current origin even if it was later removed from the list.
if ($po->wheat_origin && !in_array($po->wheat_origin, $po_origins, true)) {
    array_unshift($po_origins, $po->wheat_origin);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // ── Delivery lock action (optional, admin only) ────────────
        $delivery_lock_action = trim($_POST['delivery_lock_action'] ?? '');
        $delivery_lock_reason = trim($_POST['delivery_lock_reason'] ?? '');

        // ── Basic validation ───────────────────────────────────────
        if (empty($po_number))         throw new Exception("PO Number is required");
        if (empty($po_date))           throw new Exception("PO Date is required");
        if ($quantity_kg <= 0)         throw new Exception("Quantity must be greater than zero");
        if ($unit_price_per_kg <= 0)   throw new Exception("Unit price must be greater than zero");

        // Validate lock reason when action requested
        if (in_array($delivery_lock_action, ['lock', 'unlock']) && empty($delivery_lock_reason)) {
            throw new Exception("A reason is required when changing the delivery lock status.");
        }

        // ── Duplicate PO number check ──────────────────────────────
        if ($po_number !== $po->po_number) {
            $check_stmt = $db->prepare("SELECT id FROM purchase_orders_adnan WHERE po_number = ? AND id != ?");
        ...
        foreach ($suppliers as $s) {
            if ($s->id == $supplier_id) { $supplier = $s; break; }
        }
        if (!$supplier) throw new Exception("Invalid supplier selected");
        ...
        if ($po->po_date != $po_date) {
            $changes[] = "PO Date: " . date('d M Y', strtotime($po->po_date)) . " → " . date('d M Y', strtotime($po_date));
        ...
        $qty_changed   = (abs((float)$po->quantity_kg - $quantity_kg) > 0.001);
        $price_changed = (abs((float)$po->unit_price_per_kg - $unit_price_per_kg) > 1e-10);
        if ($qty_changed) {
        ...
        if ($price_changed) {
            $changes[] = "Unit Price: ৳" . rtrim(rtrim(number_format((float)$po->unit_price_per_kg, 10), '0'), '.') . " → ৳" . rtrim(rtrim(number_format($unit_price_per_kg, 10), '0'), '.');
            $old_values['unit_price_per_kg'] = $po->unit_price_per_kg;
            $new_values['unit_price_per_kg'] = $unit_price_per_kg;
        }
        $old_total_calc = (float)$po->quantity_kg * (float)$po->unit_price_per_kg;
        if (abs($old_total_calc - $total_order_value) > 0.01) {
            $changes[] = "Total Value: ৳" . number_format($old_total_calc, 2) . " → ৳" . number_format($total_order_value, 2);
            $old_values['total_order_value'] = $old_total_calc;
        ...
        // Delivery lock change counts as a change too
        $lock_change_made = false;
        if ($delivery_lock_action === 'lock'   && !(int)$po->is_delivery_locked) $lock_change_made = true;
        if ($delivery_lock_action === 'unlock' &&  (int)$po->is_delivery_locked) $lock_change_made = true;
        if ($lock_change_made) {
            $lockLabel = ($delivery_lock_action === 'lock') ? 'Delivery Locked' : 'Delivery Re-opened';
            $changes[] = "{$lockLabel} — Reason: {$delivery_lock_reason}";
        }
        ...
        $stmt = $db->prepare($sql);
        $stmt->execute([
            $po_number, $po_date, $supplier_id, $supplier->name,
            $wheat_origin, $quantity_kg, $unit_price_per_kg,
            $total_order_value, $expected_delivery_date, $remarks, $po_id
        ]);

        // ── Apply delivery lock change ─────────────────────────────
        if ($lock_change_made) {
            $new_lock = ($delivery_lock_action === 'lock') ? 1 : 0;
            $lock_sql = "UPDATE purchase_orders_adnan
                         SET is_delivery_locked         = ?,
                             delivery_lock_reason       = ?,
                             delivery_locked_by_user_id = ?,
                             delivery_locked_at         = NOW()
                         WHERE id = ?";
            $lock_stmt = $db->prepare($lock_sql);
            $lock_stmt->execute([$new_lock, $delivery_lock_reason, $user_id, $po_id]);
        }
        ...
        // ── Recalculate delivery_status if qty/price changed ───────
        // Done AFTER commit so the UPDATE is visible in the SELECT
        if ($qty_changed || $price_changed) {
            $po_manager->recalculatePOStatusAndTotals($po_id);
        }

        $_SESSION['success_message'] = count($changes) . " change(s) saved successfully.";
        redirect('purchase/purchase_adnan_view_po.php?id=' . $po_id);
    ...
    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 p-4 mb-6 rounded-r-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            ...
            <div>
                ...
                <p class="font-semibold text-gray-900"><?php echo number_format($po->quantity_kg, 2); ?> <?php echo htmlspecialchars($po_unit); ?></p>
            </div>
            <div>
                <p class="text-gray-600">Unit Price</p>
                <p class="font-semibold text-gray-900">৳<?php echo rtrim(rtrim(number_format((float)$po->unit_price_per_kg, 10), '0'), '.'); ?>/<?php echo htmlspecialchars($po_unit); ?></p>
            </div>
            <div>
        ...
        <form method="POST" id="editForm" class="p-6">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Commodity (fixed on edit) + Origin -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Commodity</label>
                        <div class="w-full px-3 py-2 border border-gray-200 bg-gray-50 rounded-lg text-gray-700 text-sm">
                            <i class="fas fa-boxes-stacked text-gray-400 mr-1"></i><?php echo htmlspecialchars($po_commodity->name ?? 'Wheat'); ?>
                            <span class="text-xs text-gray-400">· unit <?php echo htmlspecialchars($po_unit); ?></span>
                        </div>
                        <p class="text-xs text-gray-400 mt-1">Commodity can't be changed on an existing PO — cancel and recreate if needed.</p>
                        ...
                        <select name="wheat_origin" id="wheat_origin" required
                            ...
                            <?php foreach ($po_origins as $o): ?>
                            <option value="<?php echo htmlspecialchars($o); ?>" <?php echo $po->wheat_origin === $o ? 'selected' : ''; ?>><?php echo htmlspecialchars($o); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Quantity (<?php echo htmlspecialchars($po_unit); ?>) <span class="text-red-500">*</span>
                        </label>
                        ...
                        <p class="text-xs text-gray-500 mt-1">
                            <i class="fas fa-info-circle"></i> Original: <strong><?php echo number_format($po->quantity_kg, 2); ?> <?php echo htmlspecialchars($po_unit); ?></strong>
                        </p>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Unit Price (৳/<?php echo htmlspecialchars($po_unit); ?>) <span class="text-red-500">*</span>
                        </label>
                        <input type="number" name="unit_price_per_kg" id="unit_price_per_kg" required
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                               value="<?php echo $po->unit_price_per_kg; ?>"
                               step="any" min="0.0000000001">
                        <p class="text-xs text-gray-500 mt-1">
                            <i class="fas fa-info-circle"></i> Original: <strong>৳<?php echo rtrim(rtrim(number_format((float)$po->unit_price_per_kg, 10), '0'), '.'); ?>/<?php echo htmlspecialchars($po_unit); ?></strong>
                        </p>
                    </div>
            ...
            <!-- ══════════════════════════════════════════════════
                 DELIVERY LOCK CONTROL (Admin / Superadmin only)
            ═══════════════════════════════════════════════════ -->
            <div class="mb-8">
                <h3 class="text-lg font-semibold text-gray-900 mb-4 pb-2 border-b flex items-center gap-2">
                    <i class="fas fa-lock text-red-600"></i> Delivery Lock Control
                </h3>

                <!-- Current lock status banner -->
                <?php if ((int)$po->is_delivery_locked): ?>
                <div class="flex items-start gap-3 bg-red-50 border border-red-300 rounded-lg p-4 mb-4">
                    <i class="fas fa-lock text-red-500 mt-0.5 text-lg"></i>
                    <div>
                        <p class="font-semibold text-red-800">Delivery is currently LOCKED — no further GRNs can be recorded.</p>
                        <?php if (!empty($po->delivery_lock_reason)): ?>
                        <p class="text-sm text-red-700 mt-1">Reason: <?php echo htmlspecialchars($po->delivery_lock_reason); ?></p>
                        <?php endif; ?>
                        <?php
                        if (!empty($po->delivery_locked_at)):
                            // Fetch locked-by name
                            $lock_user = null;
                            if (!empty($po->delivery_locked_by_user_id)) {
                                $lu_stmt = Database::getInstance()->getPdo()->prepare("SELECT display_name FROM users WHERE id = ?");
                                $lu_stmt->execute([$po->delivery_locked_by_user_id]);
                                $lock_user = $lu_stmt->fetchColumn();
                            }
                        ?>
                        <p class="text-xs text-red-600 mt-1">
                            Locked by <?php echo htmlspecialchars($lock_user ?? 'Admin'); ?>
                ...
                <div class="flex items-center gap-3 bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                    ...
                    <p class="text-green-800 font-medium">Delivery is currently OPEN — GRNs can be recorded normally.</p>
                </div>
                <?php endif; ?>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Lock action selector -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Change Delivery Lock Status
                        </label>
                        <select name="delivery_lock_action" id="delivery_lock_action"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                            <option value="">— No change —</option>
                            <?php if (!(int)$po->is_delivery_locked): ?>
                            <option value="lock">🔒 Lock delivery (prevent further GRNs)</option>
                            <?php else: ?>
                            <option value="unlock">🔓 Re-open delivery (allow GRNs again)</option>
                            <?php endif; ?>
                        </select>
                        <p class="text-xs text-gray-500 mt-1">
                            <i class="fas fa-info-circle"></i>
                            Locking stops ALL further GRNs regardless of quantity. Quantity increase alone re-opens automatically.
                        </p>
                    </div>

                    <!-- Reason (shown/required when action selected) -->
                    <div id="lockReasonSection" class="hidden">
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Reason <span class="text-red-500">*</span>
                        </label>
                        <textarea name="delivery_lock_reason" id="delivery_lock_reason" rows="3"
                                  class="w-full px-3 py-2 border border-red-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500"
                                  placeholder="Mandatory: explain why you are locking / re-opening this PO…"></textarea>
                        <p class="text-xs text-red-600 mt-1">
                            <i class="fas fa-exclamation-circle"></i> This reason is saved to the audit log.
                        </p>
                    </div>
                </div>
            </div>
...
// Format unit price with full precision (up to 10dp, trailing zeros stripped)
function fmtPrice(val) {
    let s = val.toFixed(10).replace(/0+$/, '').replace(/\.$/, '');
    const num = parseFloat(s);
    return num.toLocaleString('en-US', { minimumFractionDigits: 1, maximumFractionDigits: 10 });
}
...
function detectChanges() {
    ...
    if (Math.abs(unitPrice - originalValues.unit_price_per_kg) > 1e-10) {
        changes.push(`<strong>Unit Price:</strong> ৳${fmtPrice(originalValues.unit_price_per_kg)} → ৳${fmtPrice(unitPrice)}`);
    }
    const oldTotal = originalValues.quantity_kg * originalValues.unit_price_per_kg;
    if (Math.abs(total - oldTotal) > 0.01) {
        changes.push(`<strong>Total Value:</strong> ৳${oldTotal.toFixed(2)} → ৳${total.toFixed(2)}`);
    }
    ...
    // Delivery lock action
    const lockAction = lockActionSel ? lockActionSel.value : '';
    if (lockAction === 'lock') {
        changes.push(`<strong>Delivery:</strong> Will be <span class="text-red-600 font-bold">LOCKED</span>`);
    } else if (lockAction === 'unlock') {
        changes.push(`<strong>Delivery:</strong> Will be <span class="text-green-600 font-bold">RE-OPENED</span>`);
    }
...
// Delivery lock reason visibility
const lockActionSel   = document.getElementById('delivery_lock_action');
const lockReasonSec   = document.getElementById('lockReasonSection');
const lockReasonInput = document.getElementById('delivery_lock_reason');
if (lockActionSel) {
    lockActionSel.addEventListener('change', function () {
        if (this.value) {
            lockReasonSec.classList.remove('hidden');
            lockReasonInput.required = true;
        } else {
            lockReasonSec.classList.add('hidden');
            lockReasonInput.required = false;
        }
        detectChanges();
    });
}
