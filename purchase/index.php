...
// balance_due uses total_adjustment_amount so posted DANs/CANs are reflected correctly
$total_pos_sql = "
    SELECT
        COUNT(*) as count,
        ...
        COALESCE(SUM(
            (SELECT COALESCE(SUM(expected_quantity),0) FROM goods_received_adnan g
             WHERE g.purchase_order_id = po.id AND g.grn_status != 'cancelled')
...
$recent_pos_sql = "
    SELECT
        ...
        po.supplier_name,
        po.wheat_origin,
        (SELECT COALESCE(SUM(g.expected_quantity),0) FROM goods_received_adnan g
         WHERE g.purchase_order_id = po.id AND g.grn_status != 'cancelled') as total_expected_qty,
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition">
                    ...
                    <?php
                    $recent_balance = floatval($po->total_expected_qty ?? 0) * floatval($po->unit_price_per_kg)
                                    + floatval($po->total_adjustment_amount ?? 0)
                                    - floatval($po->total_paid ?? 0);
                    ?>
                    <?php if ($recent_balance > 0): ?>
                    <div class="mt-2 text-xs text-red-600">
                        <i class="fas fa-exclamation-circle"></i> Due: ৳<?php echo number_format($recent_balance, 0); ?>
                    </div>
                    <?php endif; ?>
                </div>
                <?php endforeach; ?>
                ...
                <div class="border rounded-lg p-4 <?php echo $payment->days_since_order > 30 ? 'border-red-300 bg-red-50' : ($payment->days_since_order > 14 ? 'border-orange-300 bg-orange-50' : 'border-gray-200'); ?>">
                    ...
                    <div class="flex items-center justify-between text-sm mb-2">
                        ...
                        <span class="font-bold <?php echo $payment->days_since_order > 30 ? 'text-red-700' : 'text-gray-900'; ?>">
                            ৳<?php echo number_format($payment->computed_balance, 2); ?>
                        </span>
                    </div>
