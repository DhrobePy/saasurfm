...
class PurchaseAgentActions
{
    ...
    public static function handleUpdatePOStatus(array $f, $db, int $uid): array
    {
        ...
        // Previously unchecked — Database::query() never throws, so a silently
        // swallowed failure here would still tell the chat transcript "status
        // updated" when the PO's status never actually changed.
        if ($db->error()) return ['success'=>false,'error'=>'DB update failed — status was not changed.'];
    ...
    public static function handleClosePO(array $f, $db, int $uid): array
    {
        ...
        // Previously unchecked — a silently swallowed failure here would still tell
        // the chat transcript "closed" when the PO's delivery_status never changed.
        if ($db->error()) return ['success'=>false,'error'=>'DB update failed — PO was not closed.'];
