...
// page_key was 'purchase_procurement_catalog', which never matched what
// scanModulePages() actually keys the Privileges checkbox by (the file's real
// basename, 'procurement_catalog') — so toggling this page's checkbox in
// Privileges silently did nothing. Confirmed live: user_id=26 already had
// 'procurement_catalog' (the basename) stored in their allowed_pages from an
// admin toggling it, but the grant never took effect because this page was
// checking a different key. Aligned to the basename so that existing grant
// (and any future ones) actually works.
restrict_access(['Superadmin', 'admin'], 'purchase', 'procurement_catalog');
...
ensureCommodityIsSellableColumn(); // Commodity Trading (Jul 2026) — same catalog serves buying AND selling
    ...
    try {
        ...
        switch ($action) {
            case 'add_commodity': {
                ...
                $sellable = !empty($_POST['is_sellable']) ? 1 : 0;
                ...
                $db->query("INSERT INTO purchase_commodities (name, unit, inventory_account_id, is_sellable) VALUES (?, ?, ?, ?)", [$name, $unit, $acct, $sellable]);
                if ($db->error()) throw new Exception('Failed to add the commodity.');
                auditLog('purchase_commodities', 'created', "Procurement commodity added: {$name} ({$unit})" . ($sellable ? ' — sellable' : ''));
            ...
            case 'edit_commodity': {
                ...
                $db->query("UPDATE purchase_commodities SET name = ?, unit = ?, inventory_account_id = ?, status = ?, is_sellable = ? WHERE id = ?",
                    [$name, $unit, $acct, $status, $sellable, $cid]);
                if ($db->error()) throw new Exception('Failed to update the commodity.');
                auditLog('purchase_commodities', 'updated', "Procurement commodity #{$cid} updated: {$name} ({$unit}, {$status})" . ($sellable ? ' — sellable' : ' — not sellable'));
            ...
            case 'add_origin': {
                ...
                if ($db->error()) throw new Exception('Failed to add the origin.');
            ...
            case 'save_supplier_commodities': {
                ...
                // Previously unchecked delete-then-reinsert — the same bug class fixed
                // in cr/admin_edit.php: Database::query() never throws, so a silently
                // swallowed DELETE failure fell through into the INSERT loop with the
                // OLD mappings still in place, leaving duplicate supplier-commodity rows
                // instead of the intended clean replace. The re-insert loop's own empty
                // try/catch was also a no-op for the same reason (query() never throws
                // a catchable PDOException). Wrapped in a transaction for atomicity —
                // either the full old→new replace happens, or nothing changes.
                $pdo = $db->getPdo();
                $pdo->beginTransaction();
                $db->query("DELETE FROM supplier_commodities WHERE supplier_id = ?", [$sid]);
                if ($db->error()) throw new Exception('Failed to clear the previous goods list for this supplier.');
                foreach (array_unique($picked) as $cid) {
                    if ($cid > 0) {
                        $db->query("INSERT INTO supplier_commodities (supplier_id, commodity_id) VALUES (?, ?)", [$sid, $cid]);
                        if ($db->error()) throw new Exception("Failed to save commodity #{$cid} to this supplier's goods list.");
                    }
                }
                $pdo->commit();
        ...
        <form method="POST" class="grid grid-cols-1 sm:grid-cols-5 gap-3">
            ...
            <div class="flex items-end pb-2">
                <label class="inline-flex items-center gap-1.5 text-xs text-gray-600 cursor-pointer">
                    <input type="checkbox" name="is_sellable" value="1" checked class="rounded text-rose-600">
                    Sellable (Trading)
                </label>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div class="flex flex-wrap items-start justify-between gap-3">
                ...
                <form method="POST" class="flex flex-wrap items-end gap-2">
                    ...
                    <label class="inline-flex items-center gap-1.5 text-xs text-gray-600 cursor-pointer pb-1.5">
                        <input type="checkbox" name="is_sellable" value="1" <?php echo !empty($c->is_sellable) ? 'checked' : ''; ?> class="rounded text-rose-600">
                        Sellable
                    </label>
