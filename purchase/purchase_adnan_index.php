...
if (!$user_has_filters && !isset($_GET['order_status_filter'])) {
    $filters['order_status_filter'] = 'all_active';
}
...
// Commodity column (Phase 2 procurement catalog): map commodity_id → name/unit,
// and pull the live per-commodity origin list so the Origin filter isn't wheat-only.
ensureProcurementCatalogTables();
$pc_db = Database::getInstance();
$commodity_map = [];
foreach ($pc_db->query("SELECT id, name, unit FROM purchase_commodities")->results() as $cm) {
    $commodity_map[(int)$cm->id] = ['name' => $cm->name, 'unit' => $cm->unit];
}
$origin_filter_options = [];
foreach ($pc_db->query("SELECT DISTINCT origin_name FROM purchase_commodity_origins WHERE status = 'active' ORDER BY origin_name ASC")->results() as $o) {
    $origin_filter_options[$o->origin_name] = $o->origin_name;
}
...
foreach ($recent_orders as $order) {
    ...
    $balance          = $expected_payable + floatval($order->total_adjustment_amount ?? 0) - ($order->total_paid ?? 0);
...
$cur_status = $filters['order_status_filter'] ?? 'all_active';
[$sl_label, $sl_text, $sl_bg] = $status_labels[$cur_status] ?? $status_labels['all_active'];
            ...
            <div class="px-4 py-3 flex flex-wrap items-end gap-2">
                ...
                <div class="min-w-[160px]">
                    ...
                    <select name="order_status_filter" onchange="this.form.submit()"
                        ...
                        <option value="all_active"  <?php echo $cur_status === 'all_active'  ? 'selected' : ''; ?>>✅ All Active (Default)</option>
                        <option value="in_progress" <?php echo $cur_status === 'in_progress' ? 'selected' : ''; ?>>📋 In Progress</option>
                ...
                <div class="min-w-[120px]">
                    ...
                    <select name="wheat_origin"
                        ...
                        <?php foreach ($origin_filter_options as $v => $l): ?>
                        <option value="<?php echo htmlspecialchars($v); ?>" <?php echo ($filters['wheat_origin'] ?? '') == $v ? 'selected' : ''; ?>><?php echo htmlspecialchars($l); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <table class="w-full text-xs">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-3 py-2.5 text-left font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">Commodity</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="<?php echo $row_cls; ?> transition-colors">
                        ...
                        <?php $__cm = $commodity_map[(int)($order->commodity_id ?? 0)] ?? ['name' => 'Wheat', 'unit' => 'KG']; ?>
                        <td class="px-3 py-2 whitespace-nowrap">
                            <span class="px-1.5 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-700">
                                <?php echo htmlspecialchars($__cm['name']); ?>
                            </span>
                        </td>
                        ...
                        <td class="px-3 py-2 text-right font-semibold text-gray-700 whitespace-nowrap">
                            <?php echo number_format($order->quantity_kg, 3); ?>
                        </td>
                        ...
                        <td class="px-3 py-2 text-right whitespace-nowrap font-semibold
                            ...
                            <?php echo number_format($received, 3); ?>
                        </td>

                        <td class="px-3 py-2 text-right whitespace-nowrap font-bold
                            <?php echo $expected_payable > 0 ? 'text-purple-600' : 'text-gray-300'; ?>">
                            <?php echo $expected_payable > 0 ? '৳' . number_format($expected_payable, 2) : '—'; ?>
                        </td>

                        <td class="px-3 py-2 text-right whitespace-nowrap font-semibold text-green-600">
                            ৳<?php echo number_format($order->total_paid ?? 0, 2); ?>
                        </td>

                        <td class="px-3 py-2 text-right whitespace-nowrap font-semibold">
                            <?php if ($actual_due > 0): ?>
                                <span class="text-red-600">৳<?php echo number_format($actual_due, 2); ?></span>
                            <?php else: ?>
                        ...
                        <td class="px-3 py-2 text-right whitespace-nowrap">
                            <?php if ($advance_paid > 0): ?>
                                <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-blue-50 text-blue-700 font-bold">
                                    <i class="fas fa-arrow-up text-[8px]"></i>৳<?php echo number_format($advance_paid, 2); ?>
                                </span>
                            <?php else: ?>
                                <span class="text-gray-200">—</span>
                            <?php endif; ?>
                        ...
                        <td class="px-3 py-2 text-center whitespace-nowrap">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <?php else: ?>
                                <button onclick="reopenPO(<?php echo $order->id; ?>, '<?php echo htmlspecialchars($order->po_number, ENT_QUOTES); ?>')"
                                        class="text-purple-400 hover:text-purple-700 transition-colors" title="Reopen PO">
                    ...
                    <tr class="bg-gray-50 border-t-2 border-gray-200">
                        <td colspan="5" class="px-3 py-2.5 text-right text-xs font-bold text-gray-600 uppercase tracking-wide">
                        ...
                        <td class="px-3 py-2.5 text-right text-xs font-bold text-gray-700 whitespace-nowrap">
                            <?php echo number_format($totals['ordered_quantity'], 3); ?>
                        </td>
...
function reopenPO(poId, poNumber) {
    if (!confirm(`Reopen PO #${poNumber}?\n\nThis will:\n— Allow further goods receipt\n— Restore delivery status to "in_progress"\n\nContinue?`)) return;
    fetch('purchase_adnan_close_po.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'po_id=' + poId + '&action=reopen'
    })
    .then(r => r.json())
    .then(d => { alert(d.message); if (d.success) location.reload(); })
    .catch(e => alert('Error: ' + e));
