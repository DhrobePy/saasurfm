...
$filters = [
    ...
    'show_closed' => $_GET['show_closed'] ?? '',  // ✅ NEW: Explicit control
...
// ============================================================================
// ✅ NEW: Default behavior - Exclude closed and cancelled orders
// Only include them if explicitly filtered
// ============================================================================
$user_has_filters = !empty(array_filter($filters, function($key) {
    return !in_array($key, ['limit', 'show_closed']);
}, ARRAY_FILTER_USE_KEY));

// If NO user filters AND show_closed is not explicitly set to 'yes'
// Then exclude closed delivery_status and cancelled po_status
if (!$user_has_filters && !isset($_GET['show_closed'])) {
    $filters['exclude_closed'] = true;  // ✅ This will be used in PurchaseAdnanManager
}

if (!$user_has_filters && !isset($_GET['show_complete'])) {
    $filters['exclude_completed'] = true;  // ✅ This will be used in PurchaseAdnanManager
}
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Total Payable</th>
                    ...
                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Delivery Status</th>
                ...
                <?php else: ?>
                    ...
                    ?>
                        <tr class="hover:bg-gray-50">
                            ...
                            <!-- ✅ NEW COLUMN: Total Expected Payable -->
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-bold
                                ...
                                ?>">
                                <?php if ($expected_payable > 0): ?>
                                    ৳<?php echo number_format($expected_payable, 0); ?>
                                <?php else: ?>
