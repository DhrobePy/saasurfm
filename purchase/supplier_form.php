...
// This page is excluded from the privileges UI (legacy list), so it can never
// appear in a page whitelist — gate it through the All Suppliers page grant
// plus the 'Add Supplier' (can_create) action instead. Admins always pass.
restrict_access(['Superadmin', 'admin', 'Accounts', 'accounts-demra', 'accounts-srg'],
                'purchase', 'purchase_adnan_supplier_summary');
if (!userCanPageAction('purchase', 'purchase_adnan_supplier_summary', 'can_create')) {
    $_SESSION['error_flash'] = 'You do not have permission to add suppliers.';
    ...
    if (empty($errors)) {
        ...
        try {
            if ($supplier_id > 0) {
                ...
                if ($db->error()) throw new Exception('Failed to update the supplier.');
                ...
                // Wrapped in a transaction now — both writes were previously unchecked
                // (Database::query() never throws), so a silently swallowed suppliers
                // INSERT failure would leave $new_id stale/0, and the opening-balance
                // ledger INSERT would still fire against that bad id — an orphaned
                // ledger row referencing a supplier that doesn't exist, while the page
                // told the user "Supplier created successfully."
                $pdo = $db->getPdo();
                $pdo->beginTransaction();
                try {
                    $db->query($insert_sql, $data);
                    if ($db->error()) throw new Exception('Failed to create the supplier.');
                    $new_id = $db->getPdo()->lastInsertId();

                    // Create opening balance ledger entry if > 0
                    if ($opening_balance > 0) {
                        // Database::query() uses real (non-emulated) prepared statements
                        // (PDO::ATTR_EMULATE_PREPARES => false in Database.php) — MySQL's
                        // native protocol does NOT support binding one array value to the
                        // same named placeholder used twice, so the previous ":amount"
                        // reused for both credit_amount and balance always threw HY093
                        // ("Invalid parameter number"), silently swallowed by
                        // Database::query(). This meant the opening-balance ledger entry
                        // has never actually posted for ANY supplier created with an
                        // opening balance — fixed by giving each occurrence its own
                        // placeholder.
                        $ledger_sql = "INSERT INTO supplier_ledger (
                            ...
                            :amount, :amount2, 'Opening Balance', :user_id
                        )";

                        $db->query($ledger_sql, [
                            'supplier_id' => $new_id,
                            'amount' => $opening_balance,
                            'amount2' => $opening_balance,
                            'user_id' => $user_id
                        ]);
                        if ($db->error()) throw new Exception('Failed to post the opening balance ledger entry.');
                    }
                    $pdo->commit();
                } catch (Exception $e) {
                ...
                redirect('/purchase/suppliers.php', 'Supplier created successfully', 'success');
            }
