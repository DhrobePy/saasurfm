...
try {
    ...
    if ($payment->is_posted == 0) {
        // Not posted yet (no accounting) → archive to Recycle Bin instead of erasing.
        $batch = recycleBegin('purchase_payment',
            'Purchase payment ' . ($payment->payment_voucher_number ?? "#$payment_id")
            . ' — PO ' . ($payment->po_number ?? '') . ' · ৳' . number_format((float)$payment->amount_paid, 2));
        recycleArchiveDelete($batch, 'purchase_payments_adnan', 'id', $payment_id);
        ...
        echo json_encode([
            'success' => true,
            'message' => "Pending payment {$payment->payment_voucher_number} moved to Recycle Bin (batch #{$batch}) — restorable by Superadmin."
        ]);
        exit;
    }
