...
if (!empty($recent_orders)) {
    foreach ($recent_orders as $order) {
        ...
        $due_for_order = $expected_payable + floatval($order->total_adjustment_amount ?? 0) - ($order->total_paid ?? 0);
        ...
        $totals['due'] += $due_for_order;  // Use calculated due, not database column
    }
}
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">List of all Purchase Orders</h1>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <?php else: ?>
                        <?php foreach ($recent_orders as $order): 
                            ...
                            $expected_qty = getExpectedQuantityForPO($order->id);
                            $expected_payable = $expected_qty * $order->unit_price_per_kg;
                            $due_amount = $expected_payable + floatval($order->total_adjustment_amount ?? 0) - ($order->total_paid ?? 0);
                        ...
                        ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <?php echo $order->po_date ? date('d M Y', strtotime($order->po_date)) : '—'; ?>
                                </td>
