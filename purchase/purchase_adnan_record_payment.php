...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_action'] ?? '') === 'supplier_sale') {
    try {
        ensureSupplierLedgerAdjustmentBarterMethod();
        ...
        // Auto-generate voucher number SLS-YYYY-NNNN — MAX-existing-sequence pattern
        // (not COUNT(*)), since adj_number is UNIQUE-constrained and COUNT(*) drifts
        // below the highest number ever issued after any row deletion, risking a
        // duplicate-key collision.
        $yr   = date('Y');
        $c_q  = $db_pdo->prepare("SELECT adj_number FROM supplier_ledger_adjustments WHERE YEAR(created_at) = ? AND adj_number LIKE ? ORDER BY id DESC LIMIT 1");
        $c_q->execute([$yr, "SLS-{$yr}-%"]);
        $last_adj = $c_q->fetchColumn();
        $next_seq = $last_adj ? ((int)substr($last_adj, -4) + 1) : 1;
        $adj_number = 'SLS-' . $yr . '-' . str_pad((string)$next_seq, 4, '0', STR_PAD_LEFT);
        ...
        if ($result['success']) {
            ...
            try {
                if (defined('TELEGRAM_NOTIFICATIONS_ENABLED') && TELEGRAM_NOTIFICATIONS_ENABLED) {
                    ...
                    $telegram = new TelegramNotifier(TELEGRAM_BOT_TOKEN, getTelegramChatId('payment'));
                        ...
                        <div class="mb-4">
                            ...
                            <select name="purchase_order_id" id="purchase_order_id" 
                                ...
                                <?php foreach ($outstanding_pos as $po):
                                    ...
                                    $po_computed_balance = max(0,
                                        floatval($po->total_expected_qty ?? 0) * floatval($po->unit_price_per_kg)
                                        + floatval($po->total_adjustment_amount ?? 0)
                                        - floatval($po->total_paid ?? 0)
                                    );
                                ?>
                                <option value="<?php echo $po->id; ?>"
                                        ...
                                        data-balance="<?php echo $po_computed_balance; ?>"
                                    ...
                                    (Balance: ৳<?php echo number_format($po_computed_balance, 2); ?>
                ...
                <div class="p-6">
                    ...
                    <div class="mb-3">
                        ...
                        <h4 class="text-2xl font-bold text-red-600">
                            ৳<?php
                                $total_balance_due = 0;
                                foreach ($outstanding_pos as $_po) {
                                    $total_balance_due += max(0,
                                        floatval($_po->total_expected_qty ?? 0) * floatval($_po->unit_price_per_kg)
                                        + floatval($_po->total_adjustment_amount ?? 0)
                                        - floatval($_po->total_paid ?? 0)
                                    );
                                }
