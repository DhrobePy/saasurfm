...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $is_admin) {
    ...
    switch ($action) {
        ...
        case 'cancel':
            ...
            break;
        case 'delete':
            $r = $po_manager->deleteAdjustmentNote($note_id, $user_id, $user_name);
            if ($r['success']) {
                $_SESSION['success'] = $r['message'];
                redirect('purchase/purchase_adnan_adjustments.php');
            }
            break;
        default:
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-lg shadow p-5">
                ...
                <form method="POST">
                    ...
                    <button type="submit"
                            class="w-full border border-gray-400 text-gray-600 py-2 px-4 rounded-lg hover:bg-gray-100 flex items-center justify-center gap-2 text-sm"
                            onclick="return confirm('Permanently delete <?php echo htmlspecialchars($note->note_number, ENT_QUOTES); ?>? This cannot be undone.')">
            ...
            <?php if ($is_admin && in_array($note->status, ['posted', 'cancelled'])): ?>
            <div class="bg-white rounded-lg shadow p-5">
                <h6 class="font-semibold text-gray-700 mb-3">Admin Actions</h6>
                <?php if ($note->status === 'posted'): ?>
                <p class="text-xs text-gray-500 mb-3">
                    <i class="fas fa-exclamation-triangle text-amber-500 mr-1"></i>
                    Deleting a posted note will reverse its financial effect on PO #<?php echo htmlspecialchars($note->po_number); ?>.
                </p>
                <?php endif; ?>
                <form method="POST">
                    <input type="hidden" name="action" value="delete">
                    <button type="submit"
                            class="w-full bg-red-600 text-white py-2 px-4 rounded-lg hover:bg-red-700 flex items-center justify-center gap-2 text-sm"
                            onclick="return confirm('<?php echo $note->status === 'posted'
                                ? 'DELETE posted note ' . htmlspecialchars($note->note_number, ENT_QUOTES) . '? This will REVERSE its financial effect on the PO. This cannot be undone.'
                                : 'Permanently delete ' . htmlspecialchars($note->note_number, ENT_QUOTES) . '? This cannot be undone.'; ?>')">
                        <i class="fas fa-trash-alt"></i> Delete Note
                    </button>
                </form>
            </div>
            <?php elseif (in_array($note->status, ['posted', 'cancelled'])): ?>
