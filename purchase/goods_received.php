...
// LIMIT/OFFSET are embedded in the SQL string (not in $params), so reuse $params directly
            ...
            <table class="min-w-full divide-y divide-gray-200" id="grnsTable">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Unload Point</th>
                    ...
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                <?php echo $grn->grn_date ? date('d M Y', strtotime($grn->grn_date)) : '—'; ?>
                            </td>
                            ...
                            <!-- Unload Point -->
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                <?php echo htmlspecialchars($grn->branch_name ?: ($grn->unload_point_name ?: 'N/A')); ?>
                            </td>
                ...
                <tfoot class="bg-gray-50 border-t-2 border-gray-300">
                    <tr>
                        <td colspan="7" class="px-4 py-3 text-sm font-bold text-gray-700">
                            <i class="fas fa-sigma text-gray-500 mr-1"></i>
                            Filtered Total
                            <span class="ml-2 text-xs font-normal text-gray-500">
                                (<?php echo number_format($stats->total_grns); ?> GRN<?php echo $stats->total_grns != 1 ? 's' : ''; ?>, all pages)
                            </span>
                        </td>
                        <td class="px-4 py-3 text-right text-sm font-bold text-gray-800 whitespace-nowrap">
                            <?php echo number_format($stats->total_expected_qty, 0); ?>
                            <span class="text-xs font-normal text-gray-500">KG</span>
                        </td>
                        <td class="px-4 py-3 text-right text-sm font-bold text-gray-800 whitespace-nowrap">
                            <?php echo number_format($stats->total_received_qty, 0); ?>
                            <span class="text-xs font-normal text-gray-500">KG</span>
                        </td>
                        <td class="px-4 py-3 text-right text-sm font-bold whitespace-nowrap
                            <?php echo $stats->total_variance_qty < 0 ? 'text-red-600' : 'text-green-600'; ?>">
                            <?php echo ($stats->total_variance_qty > 0 ? '+' : '') . number_format($stats->total_variance_qty, 0); ?>
                            <span class="text-xs font-normal">KG</span>
                        </td>
                        <td class="px-4 py-3 text-right text-sm font-bold text-orange-700 whitespace-nowrap">
