...
function getModuleRegistry(): array {
    return [
        'credit_sales' => [
            ...
            'active_files' => [
                'sales_dashboard', 'index', 'all_sales', 'product_sales_report', 'create_order',
            ...
            'nav' => [
                ...
                ['file' => 'product_sales_report',  'page_key' => 'product_sales_report',  'label' => 'Product Sales Report', 'icon' => 'fa-tags', 'admin_only' => true],
        ...
        'production' => [
            ...
            'icon'         => 'fa-industry',
            'color'        => 'amber',
            'active_files' => ['credit_production', 'credit_dispatch', 'partial_delivery', 'order_status', 'sales_report', 'production_requirement', 'delivery_schedule', 'dispatch_report'],
            'nav' => [
                ...
                ['file' => 'delivery_schedule',      'page_key' => 'delivery_schedule',      'label' => 'Delivery Schedule',          'icon' => 'fa-calendar-check'],
                ...
                ['file' => 'dispatch_report',        'page_key' => 'dispatch_report',        'label' => 'Dispatch Report',            'icon' => 'fa-truck-loading'],
            ],
            'page_actions' => [
                'credit_production' => [
                ...
                'dispatch_report'   => ['can_export' => 'Export Dispatch Report'],
            ],
        ],
        'customers' => [
            'folder' => 'customers',
        ...
        'products' => [
            ...
            'nav' => [
                ...
                ['file' => 'price_manager',    'page_key' => 'price_manager',    'label' => 'Price Manager',    'icon' => 'fa-money-bill-wave', 'admin_only' => true],
            ],
            'page_actions' => [
        ...
        'purchase' => [
            ...
            'nav' => [
                ...
                ['file' => 'procurement_catalog',             'page_key' => 'procurement_catalog',    'label' => 'Procurement Catalog', 'icon' => 'fa-boxes-stacked', 'admin_only' => true],
            ...
            'page_actions' => [
                ...
                'procurement_catalog'             => ['can_manage' => 'Manage Procurement Catalog'],
        ...
        'pos' => [
            ...
            'nav' => [
                ...
                ['file' => 'edit_order',        'page_key' => 'edit_order',        'label' => 'Edit POS Sale',    'icon' => 'fa-edit', 'admin_only' => true],
                ['file' => 'pending_approvals', 'page_key' => 'pending_approvals', 'label' => 'Pending Approvals','icon' => 'fa-user-check'],
                ['file' => 'verify_exit',       'page_key' => 'verify_exit',       'label' => 'Exit Verification','icon' => 'fa-qrcode'],
            ],
            'page_actions' => [
                ...
                'pending_approvals'  => ['can_decide' => 'Decide Pending Approvals'],
                'verify_exit'        => ['can_scan' => 'Scan Exit Verification'],
            ],
        ],
        'collector' => [
            'folder' => 'collector',
        ...
        // Logistics (Transport) — vehicles/drivers/fuel/maintenance/rentals/trips.
        // Previously had NO module registry entry at all, so it was 100% invisible
        // to the Privileges UI (every page in logistics/ was governed solely by its
        // own hardcoded $allowed_roles list, with no way to delegate access via
        // admin/privileges.php). Registering the folder here at least makes
        // logistics/index.php privilege-toggleable.
        // NOTE: scanModulePages() only globs the TOP LEVEL of a module folder
        // (glob("{folder}/*.php")) — it does not recurse. Nearly all of this
        // module's real pages live one level down (drivers/, fuel/, maintenance/,
        // rentals/, trips/, vehicles/), so they remain invisible to the Privileges
        // checklist even with this registry entry, and are NOT added to nav below.
        // Making them delegable needs a deliberate change to scanModulePages()
        // (recursive scan) AND detect_page_module() (page_key must include the
        // subdirectory, since today it collapses to the bare basename — e.g.
        // drivers/index.php, fuel/index.php, trips/index.php etc. would all
        // resolve to the same page_key 'index' and collide). Left as-is; flagged
        ...
        'logistics' => [
            ...
            'color'  => 'cyan',
            'nav' => [
                ['file' => 'index', 'page_key' => 'index', 'label' => 'Logistics Dashboard', 'icon' => 'fa-truck-moving'],
            ],
            'page_actions' => [],
        ],
        'admin' => [
...
function isAutoHiddenPage(string $basename): bool {
    ...
    // Telegram A/B variant copies (e.g. "credit_dispatch_with_telegram",
    // "customer_payment_without_telegram") — NOT a page whose real name
    // legitimately contains "telegram" (e.g. telegram_ai_users.php, the real
    // admin page for managing Telegram AI bot access — a blanket substring
    // match here previously hid it from the Privileges UI's toggle list
    // entirely, so it could never be delegated to anyone).
    if (preg_match('/_(with|without)_telegram$/', $basename)) return true;
    ...
    if (in_array($basename, [
        ...
        'migrate_fix',           // bank/: one-time v_bank_statement VIEW migration ("DELETE THIS FILE after running")
        'migrate_add_btx_col',   // bank/: one-time v_bank_statement column migration ("Run once as Superadmin, then delete")
        'adjust_bk',             // bank/: duplicate copy of migrate_add_btx_col.php (same one-time migration script)
...
/**
 * Signature for the delivery-confirmation QR (binds the order number, and —
 * for a partial-delivery-scoped pass — the specific delivery too, so a QR
 * can't be replayed against a different delivery by editing the URL). Omitting
 * $deliveryId reproduces the exact legacy signature, so every whole-order QR
 * already printed/in circulation before this change stays valid.
 */
function deliveryQrSignature(string $orderNumber, ?int $deliveryId = null): string {
    $payload = 'DELIV|' . $orderNumber . ($deliveryId !== null ? '|' . $deliveryId : '');
    return substr(hash_hmac('sha256', $payload, getInvoiceQrSecret()), 0, 16);
}

/**
 * Delivery-confirmation records (Feature #17 gate-pass/delivery control).
 * `delivery_id` (nullable) scopes a row to one specific partial-delivery trip
 * (credit_order_deliveries.id) — NULL means the legacy whole-order flow. An
 * order can now have MANY confirmation rows (one per partial trip), so the
 * old order-only UNIQUE was dropped in favor of an app-level NULL-safe lookup
 * (`order_id <=> ? AND delivery_id <=> ?`) before every insert.
 */
function ensureDeliveryConfirmTable(): void {
    ...
    $db  = Database::getInstance();
    $pdo = $db->getPdo();
    // Fully migrated already (has both the gate-pass columns AND delivery_id)?
    try { $pdo->query("SELECT `gate_out_at`, `delivery_id` FROM `cr_delivery_confirmations` LIMIT 1"); return; } catch (Exception $e) {}
        ...
        try {
            $pdo->exec("
                CREATE TABLE IF NOT EXISTS `cr_delivery_confirmations` (
                  ...
                  KEY `idx_dc_delivery` (`delivery_id`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
            ");
        } catch (Exception $e) { error_log('ensureDeliveryConfirmTable create: ' . $e->getMessage()); }
        return;
    }

    // Existing table — migrate forward whatever's missing (each ALTER is
    // independently guarded since we don't know which historical version this
    // installation is starting from).
    foreach ([
        ...
        "ALTER TABLE `cr_delivery_confirmations` ADD COLUMN `delivery_id` bigint UNSIGNED DEFAULT NULL AFTER `order_id`",
        "ALTER TABLE `cr_delivery_confirmations` ADD INDEX `idx_dc_delivery` (`delivery_id`)",
        "ALTER TABLE `cr_delivery_confirmations` ADD INDEX `idx_dc_order` (`order_id`)",
    ] as $sql) {
        try { $pdo->exec($sql); } catch (Exception $e) { /* column/index may already exist */ }
    }
    // Drop the old order-only UNIQUE — it no longer fits now that one order can
    // have many delivery-scoped confirmation rows. Uniqueness is enforced in
    // application logic instead (see the NULL-safe lookup note above).
    try { $pdo->exec("ALTER TABLE `cr_delivery_confirmations` DROP INDEX `uk_dc_order`"); } catch (Exception $e) { /* may not exist */ }
}

/**
 * Single source of truth for "is this order now fully delivered" — recomputes
 * from cumulative credit_order_deliveries vs total ordered qty (or an explicit
 * is_final delivery), updates credit_orders.status/balance_due, and returns the
 * resulting status. Used by both partial_delivery.php (after recording a
 * delivery) and verify_delivery.php (after a gate-pass delivery confirmation) —
 * previously duplicated inline in partial_delivery.php only, so the gate-pass
 * path had no way to make this same decision correctly.
 */
function updateOrderDeliveryStatus($db, int $order_id): string {
    $order = $db->query("SELECT amount_paid FROM credit_orders WHERE id = ?", [$order_id])->first();
    if (!$order) return 'unknown';

    $cumulative_amount = (float)($db->query(
        "SELECT COALESCE(SUM(total_amount_delivered), 0) AS total FROM credit_order_deliveries WHERE order_id = ?",
        [$order_id]
    )->first()->total ?? 0);
    ...
    $total_delivered_qty = (float)($db->query(
        "SELECT COALESCE(SUM(total_qty_delivered), 0) AS total FROM credit_order_deliveries WHERE order_id = ?",
        [$order_id]
    )->first()->total ?? 0);

    $has_final = (int)($db->query(
        "SELECT COUNT(*) AS c FROM credit_order_deliveries WHERE order_id = ? AND is_final = 1", [$order_id]
    )->first()->c ?? 0) > 0;

    $new_balance_due = max(0, $cumulative_amount - (float)$order->amount_paid);
    $fully_delivered = $has_final || ($total_delivered_qty >= $total_ordered_qty - 0.001);
    $new_status = $fully_delivered ? 'delivered' : 'shipped';

    $db->query(
        "UPDATE credit_orders SET status = ?, balance_due = ?, updated_at = NOW() WHERE id = ?",
        [$new_status, $new_balance_due, $order_id]
    );
...
/**
 * Delivery Schedule Alert — "due for delivery today" + "overdue" orders,
 * across ALL branches, to the shared production Telegram group. Driven by
 * scripts/delivery_schedule_alert.php on a fixed twice-daily cron (11:00 and
 * 18:00) — always sends when called (no dedup/window gating like the
 * shortfall alert above), since it's a scheduled status report, not a
 * conditional problem alert. Same active-pipeline status set and required_date
 * semantics as the Delivery Schedule tab on cr/credit_production.php.
 */
function sendDeliveryScheduleAlert(): bool {
    if (!defined('TELEGRAM_NOTIFICATIONS_ENABLED') || !TELEGRAM_NOTIFICATIONS_ENABLED) return false;
    if (!defined('TELEGRAM_BOT_TOKEN')) return false;
    $db = Database::getInstance();
    $today = date('Y-m-d');
    ...
    try {
        $due_today = $db->query(
            "SELECT co.id, co.order_number, co.total_amount, co.delivery_type, co.required_date,
        ...
        $overdue = $db->query(
            "SELECT co.id, co.order_number, co.total_amount, co.delivery_type, co.required_date,
             ...
             WHERE co.status IN ('approved','in_production','produced','ready_to_ship')
               AND co.assigned_branch_id IS NOT NULL
               AND co.required_date < ?
             ORDER BY co.required_date ASC, b.name, co.id",
            [$today]
        )->results();

        // Product/variant/quantity per order, for the item breakdown line under each order.
        $items_by_order = [];
        $order_ids = array_unique(array_merge(array_map(fn($o) => (int)$o->id, $due_today), array_map(fn($o) => (int)$o->id, $overdue)));
        if (!empty($order_ids)) {
            ...
            foreach ($flat as $it) {
                ...
                $items_by_order[$it->order_id][] = number_format((float)$it->quantity, 0) . 'x ' . $it->product_name
                    . ($variant !== '' ? " ({$variant})" : '');
            }
        }
    } catch (Exception $e) {
        error_log('sendDeliveryScheduleAlert query: ' . $e->getMessage());
        return false;
    }

    $vehicle_label = fn($t) => $t === 'mini_truck' ? '🛻 Mini Truck' : '🚛 Big Truck';

    $render_section = function (array $rows, bool $show_days) use ($vehicle_label, $items_by_order) {
        if (empty($rows)) return "✅ None.\n";
        $by_branch = [];
        foreach ($rows as $r) $by_branch[$r->branch_name ?? 'Unassigned'][] = $r;

        $out = '';
        foreach ($by_branch as $branch_name => $branch_orders) {
            $out .= "📍 <b>" . htmlspecialchars($branch_name) . "</b> (" . count($branch_orders) . ")\n";
            $shown = 0;
            foreach ($branch_orders as $it) {
                if ($shown >= 20) { $out .= "  …and " . (count($branch_orders) - $shown) . " more\n"; break; }
                $days_txt = '';
                if ($show_days) {
                    $d = (int)floor((strtotime('today') - strtotime($it->required_date)) / 86400);
                    $days_txt = " — <b>{$d}d overdue</b>";
                }
                $out .= "  • " . htmlspecialchars($it->order_number) . " — " . htmlspecialchars($it->customer_name)
                      . " — ৳" . number_format($it->total_amount, 0) . " — " . $vehicle_label($it->delivery_type)
                      . $days_txt . "\n";

                $order_items = $items_by_order[$it->id] ?? [];
                if (!empty($order_items)) {
                    $shown_items = array_slice($order_items, 0, 5);
                    $items_txt   = implode(', ', $shown_items);
                    if (count($order_items) > 5) $items_txt .= ' +' . (count($order_items) - 5) . ' more';
                    $out .= "     " . htmlspecialchars($items_txt) . "\n";
                }
                $shown++;
            }
        }
    ...
    $m  = "🚚 <b>Delivery Schedule</b> — " . date('h:i A') . "\n";
    $m .= date('l, d M Y') . "\n";
    $m .= "━━━━━━━━━━━━━━━━━━━━\n\n";
    $m .= "📅 <b>Due Today (" . count($due_today) . ")</b>\n" . $render_section($due_today, false);
    $m .= "\n⚠ <b>Overdue (" . count($overdue) . ")</b>\n" . $render_section($overdue, true);
    ...
    try {
        ...
        error_log('sendDeliveryScheduleAlert: ' . $e->getMessage());
        return false;
    }
}
...
/**
 * Short human-readable reason an order is currently held from dispatch
 * (payment/credit condition, or the global default hold), or null if it's
 * clear to ship. Thin wrapper around getOrderGateState() for pages that just
 * need a one-line "why is this stuck" badge — e.g. the production-facing
 * delivery boards, which don't have Payment Watch's own detail view.
 */
function getOrderDispatchHoldReason(int $order_id): ?string {
    $gate = getOrderGateState($order_id);
    if (in_array($gate['dispatch'], ['open', 'cleared'], true)) return null;

    if (!empty($gate['default_hold'])) {
        return 'Awaiting Accounts clearance (default hold)';
    }
    if ($gate['dispatch'] === 'condition_met') {
        return 'Payment condition met — awaiting manual clearance';
    }

    $type   = $gate['row']->condition_type ?? null;
    $labels = [
        'outstanding_below'      => 'Outstanding must drop below ৳' . number_format((float)($gate['threshold'] ?? 0), 0),
        'outstanding_after_ship' => 'Full settlement required before dispatch',
        'amount_received'        => 'Awaiting ৳' . number_format((float)($gate['threshold'] ?? 0), 0) . ' in payments',
    ];
    $reason = $labels[$type] ?? 'Payment condition not yet met';
    if (!empty($gate['shortfall'])) {
        $reason .= ' (short ৳' . number_format((float)$gate['shortfall'], 0) . ')';
    }
    return $reason;
}
