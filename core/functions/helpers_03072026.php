    ...
    return [
        'credit_sales' => [
            ...
            'active_files' => [
                ...
                'payment_watch', 'over_delivery', 'sales_hub',
            ],
            'nav' => [
...
function getOrderGateState(int $order_id): array {
    ...
    if ((int)$row->dispatch_hold === 1) {
        if ((int)$row->dispatch_cleared === 1) {
            ...
            $order = $db->query("SELECT customer_id FROM credit_orders WHERE id = ?", [$order_id])->first();
                    ...
                    try {
                        $rec = $db->query(
                            "SELECT COALESCE(SUM(amount), 0) AS total
                             ...
                             WHERE customer_id = ? AND payment_date >= DATE(?)",
