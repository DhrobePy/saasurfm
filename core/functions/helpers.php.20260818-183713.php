...
function sendDeliveryScheduleAlert(): bool {
    ...
    try {
        $due_today = $db->query(
            "SELECT co.order_number, co.total_amount, co.delivery_type, co.required_date,
    ...
    $render_section = function (array $rows, bool $show_days) use ($vehicle_label, $today) {
        ...
        $out = '';
        foreach ($by_branch as $branch_name => $items) {
            $out .= "📍 <b>" . htmlspecialchars($branch_name) . "</b> (" . count($items) . ")\n";
            $shown = 0;
            foreach ($items as $it) {
                if ($shown >= 25) { $out .= "  …and " . (count($items) - $shown) . " more\n"; break; }
                $days_txt = '';
                if ($show_days) {
