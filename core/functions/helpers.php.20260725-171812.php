...
function notifyPendingRequestQueued(int $req_id, string $type, float $amount, string $summary): void {
    ...
    try {
        ...
        $label = $type === 'delivery' ? 'DELIVERY' : ($type === 'advance' ? 'ADVANCE' : ($type === 'commodity_sale' ? 'COMMODITY SALE' : ($type === 'commodity_sale_edit' ? 'COMMODITY SALE EDIT' : ($type === 'loan_disbursement' ? 'LOAN DISBURSEMENT' : ($type === 'loan_repayment' ? 'LOAN REPAYMENT' : 'PAYMENT')))));
