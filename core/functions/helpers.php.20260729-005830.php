...
function notifyPendingRequestQueued(int $req_id, string $type, float $amount, string $summary): void {
    ...
    try {
        ...
        $label = $type === 'delivery' ? 'DELIVERY' : ($type === 'advance' ? 'ADVANCE' : ($type === 'commodity_sale' ? 'COMMODITY SALE' : ($type === 'commodity_sale_edit' ? 'COMMODITY SALE EDIT' : ($type === 'loan_disbursement' ? 'LOAN DISBURSEMENT' : ($type === 'loan_repayment' ? 'LOAN REPAYMENT' : ($type === 'pos_credit_sale' ? 'POS CREDIT SALE' : ($type === 'pos_sale_edit' ? 'POS SALE EDIT' : 'PAYMENT')))))));
        ...
        $category_map = [
            ...
            'pos_credit_sale' => 'orders', 'pos_sale_edit' => 'orders',
        ];
