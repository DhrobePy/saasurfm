    ...
    public function deleteAdjustmentNote($id, $userId, $userName) {
        try {
            ...
            if ($note->status === 'posted') {
                ...
                // Remove the supplier credit entry if this was a CAN
                if ($note->note_type === 'credit') {
                    $this->db->prepare(
                        "DELETE FROM supplier_balance_adjustments WHERE adjustment_note_id = ?"
                    )->execute([$id]);
                }
            }

            // Hard-delete the note record
            $this->db->prepare("DELETE FROM purchase_adjustment_notes WHERE id = ?")->execute([$id]);
