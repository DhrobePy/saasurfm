...
class Database
{
    ...
    // Debug: Check what type $params is
    $params_type = gettype($params);
    $params_debug = '';
    
    if (is_array($params)) {
        $params_debug = print_r($params, true);
    } elseif (is_object($params)) {
        $params_debug = 'Object of type: ' . get_class($params);
    } else {
        $params_debug = (string)$params;
    }
    
    // Log with clear type information
    error_log("SQL Error [Type: {$params_type}] [{$e->getCode()}]: {$e->getMessage()} in query: $sql | Params: " . $params_debug);
    
    // Also log to a separate debug file to see if it's being called multiple times
    error_log("DEBUG - Database error at " . date('Y-m-d H:i:s') . " - Params type: " . $params_type, 3, "/tmp/db_debug.log");
}
