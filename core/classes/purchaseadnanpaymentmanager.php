...
class Purchasepaymentadnanmanager {
    ...
    public function recordPayment($data) {
        try {
            ...
            if ($data['payment_method'] === 'bank' && !empty($data['bank_account_id'])) {
                ...
                // ✅ Use cash account name passed from form (from branch_petty_cash_accounts)
    ...
    private function createPaymentJournalEntry($payment_id, $po, $data, $bank_chart_account_id = null) {
        try {
            ...
            if ($data['payment_method'] === 'cash') {
                ...
                // Last resort: find any cash/petty cash account
                if (!$payment_account) {
                    $sql = "SELECT * FROM chart_of_accounts 
     ...
     * Fetches from branch_petty_cash_accounts table
     * ✅ QUICK FIX: Uses branch_petty_cash_accounts table (the actual cash accounts table)
     * 
     * @return array Cash account list with branch information
     */
