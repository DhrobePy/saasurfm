 ...
 */
    ...
    try {
        ...
        // Feature #3 (Option A): archive the ORIGINAL posting instead of booking a
        // separate reversal entry. Keeps the ledger clean and the whole deletion
        // fully restorable from the Recycle Bin (one uniform undo across modules).
        $batch = recycleBegin('expense_voucher',
            "Expense voucher {$voucher->voucher_number} — ৳" . number_format((float)$voucher->total_amount, 2));

        if ($voucher->status === 'approved') {
            // Restore the cash/bank the expense consumed. Snapshot the balance row's
            // before-image first so a restore re-applies the original spend exactly.
            if ($voucher->payment_method === 'bank' && $voucher->bank_account_id) {
                recycleSnapshotBefore($batch, 'bank_accounts', 'id', (int)$voucher->bank_account_id);
                $this->updateBankBalance($voucher->bank_account_id, $voucher->total_amount);
            } elseif ($voucher->payment_method === 'cash' && $voucher->cash_account_id) {
                recycleSnapshotBefore($batch, 'branch_petty_cash_accounts', 'id', (int)$voucher->cash_account_id);
                $this->updateCashBalance($voucher->cash_account_id, $voucher->total_amount);
            }

            // Archive the ORIGINAL expense journal entry + its lines (posted at approval)
            $jstmt = $this->pdo->prepare(
                "SELECT id FROM journal_entries
                 WHERE related_document_type = 'expense_voucher' AND related_document_id = ?"
            );
            $jstmt->execute([$voucher_id]);
            foreach ($jstmt->fetchAll(PDO::FETCH_COLUMN) as $jid) {
                recycleArchiveDelete($batch, 'transaction_lines', 'journal_entry_id', (int)$jid);
                recycleArchiveDelete($batch, 'journal_entries',   'id',               (int)$jid);
            }
        }

        // Archive the voucher itself (+ its action log) instead of erasing it
        recycleArchiveDelete($batch, 'expense_action_log', 'expense_voucher_id', $voucher_id);
        recycleArchiveDelete($batch, 'expense_vouchers',   'id',                 $voucher_id);
        ...
        return [
            'success' => true,
            'message' => "Expense voucher {$voucher->voucher_number} moved to Recycle Bin (batch #{$batch}) — restorable from Admin → Recycle Bin"
        ];
    ...
    private function sendTelegramNotification($voucher_id, $action = 'created') {
        try {
            ...
            $telegram = new TelegramNotifier(TELEGRAM_BOT_TOKEN, getTelegramChatId('expense'));
