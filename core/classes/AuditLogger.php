...
class AuditLogger {
    ...
    /**
     * Self-migrating schema — this table never had a CREATE TABLE anywhere in
     * the codebase, so on a fresh DB (or one restored without it) every single
     * auditLog() call across every module was silently failing: the INSERT hit
     * a PDOException ("table doesn't exist"), which log() catches, error_logs,
     * and swallows — so the whole audit trail was empty with no visible error.
     * Probe-first, DDL outside any transaction, matches this codebase's
    ...
    private static function ensureTable() {
        ...
        $done = true;
        global $db;
        // Probe first — CREATE TABLE IF NOT EXISTS is still DDL and MySQL
        // implicitly commits any open transaction on DDL even when the table
        // already exists. Running this unconditionally on every request was
        // silently ending the caller's transaction the instant they logged an
        // audit entry mid-transaction (found while hardening the POS rewrite —
        // any auditLog() call inside a beginTransaction()/commit() block
        // anywhere in the app was exposed to this).
        try { $db->getPdo()->query("SELECT 1 FROM `system_audit_log` LIMIT 1"); return; } catch (\Throwable $e) {}
        try {
            $db->getPdo()->exec("CREATE TABLE IF NOT EXISTS `system_audit_log` (
                ...
                INDEX `idx_sal_record` (`record_type`, `record_id`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        } catch (\Throwable $e) {
            error_log('AuditLogger::ensureTable failed: ' . $e->getMessage());
        }
    }
