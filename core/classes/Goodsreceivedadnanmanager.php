...
class GoodsReceivedAdnanManager {
    ...
    public function recordGoodsReceived($data) {
        try {
            ...
            // Block GRN if admin has locked this PO for delivery
            if ((int)$po->is_delivery_locked === 1) {
                $lockInfo = '';
                if (!empty($po->delivery_lock_reason)) {
                    $lockInfo = ' Reason: ' . $po->delivery_lock_reason;
                }
                return [
                    'success' => false,
                    'message' => "This purchase order (PO #{$po->po_number}) has been locked by admin — no further GRNs can be recorded.{$lockInfo} Contact Superadmin or Admin to re-open delivery.",
                ];
            }
     ...
     * @return void
     *
     * Made public 8 Aug 2026 so purchase_adnan_update_grn.php can reuse this
     * exact posting logic to recreate the journal entry after reversing the
     * old one on edit — previously it only reversed and never re-posted,
     * permanently desyncing inventory value from the GL on every edited GRN.
     */
    public function createInventoryJournalEntry($grn_id, $po, $quantity) {
        try {
