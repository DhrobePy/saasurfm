...
class Purchasepaymentadnanmanager {
    ...
    public function recordPayment($data) {
        try {
            ...
            // ── Bridge: mirror to bank_transactions so purchase payments appear in
            //    the Bank Statement module.  Matched by account_number between
            //    bank_accounts and bank_tx_accounts.  Fails silently.
            if (in_array($data['payment_method'], ['bank', 'cheque']) && !empty($data['bank_account_id'])) {
                try {
                    $bm_stmt = $this->db->prepare(
                    ...
                    $bm_stmt->execute([(int)$data['bank_account_id']]);
                    $bm_bridge = $bm_stmt->fetch(\PDO::FETCH_OBJ);
                    if ($bm_bridge) {
                        require_once dirname(__DIR__) . '/../bank/BankManager.php';
                        $bankMgr = new BankManager();
                        $bankMgr->createTransaction([
                            'transaction_date'   => $data['payment_date'],
                            'entry_type'         => 'debit', // money OUT: we are paying supplier
                            'bank_tx_account_id' => (int)$bm_bridge->bta_id,
                            'amount'             => $data['amount_paid'],
                            'reference_number'   => $data['reference_number'] ?? null,
                            'cheque_number'      => $data['cheque_number'] ?? null,
                            'payee_payer_name'   => $po->supplier_name ?? null,
                            'description'        => "Purchase payment — {$po->supplier_name} — Voucher #{$voucher_number}",
                        ], $current_user['id'], $current_user['display_name'] ?? ($current_user['username'] ?? 'System'));
                    }
                } catch (\Throwable $bm_ex) {
                    error_log("BankManager bridge (purchase payment): " . $bm_ex->getMessage());
                }
            }
     ...
     *
     * Made public (was private) so purchase_adnan_update_payment.php can re-post a
     * replacement journal entry after reversing the old one on a BANK/CHEQUE payment
     * edit — mirrors the same reuse pattern used for GRN edits via
     * GoodsReceivedAdnanManager::createInventoryJournalEntry().
     */
    public function createPaymentJournalEntry($payment_id, $po, $data, $bank_chart_account_id = null) {
        try {
            $amount = $data['amount_paid'];
     ...
     * @return void
     *
     * Made public (was private) — see createPaymentJournalEntry() note above.
     */
    public function updatePaymentJournalId($payment_id, $journal_id) {
