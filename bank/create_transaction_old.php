...
<div class="w-full max-w-2xl mx-auto px-4 sm:px-6 py-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form method="POST" id="txForm">
            ...
            <!-- Transaction Type -->
            <div class="mb-4">
                <label class="block text-sm font-semibold text-gray-700 mb-1">Transaction Category</label>
                <select name="transaction_type_id"
                    ...
                    foreach ($txTypes as $tt):
                        if ($tt->nature !== $currentNature):
                            if ($currentNature !== null) echo '</optgroup>';
                            echo '<optgroup label="' . ucfirst($tt->nature) . '">';
                            $currentNature = $tt->nature;
                        endif;
                    ?>
                    <option value="<?php echo $tt->id; ?>"
                            <?php echo ($editMode && $editTx->transaction_type_id == $tt->id) ? 'selected' : ''; ?>>
