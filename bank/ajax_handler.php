...
try {
    switch ($action) {
        ...
        // ── Get balance for a bank account (Superadmin/admin only — everyone
        //    else must never see a current/available balance figure) ───────
        case 'get_balance':
        ...
        case 'mark_reconciled':
            ...
            if (!$accountId) throw new Exception('Invalid account ID.');
            $result = $bankManager->markReconciled($accountId, $userId, $userName, $notes ?: null);
            echo json_encode([
                'success'       => true,
