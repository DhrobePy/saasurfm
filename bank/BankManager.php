...
class BankManager {
    ...
    /**
     * $db->query() catches PDOException internally and never rethrows — every
     * write in this class used to fire-and-forget, so a failed UPDATE/DELETE
     * still fell through to write an "approved"/"rejected"/"deleted" audit log,
     * send a Telegram notice, and return success to the caller. Found 4 Aug
     * 2026 in a codebase-wide sweep for this exact pattern. Use this for any
     * write whose failure must actually stop the operation.
     */
    private function checkedQuery($sql, $params, $context) {
        $result = $this->db->query($sql, $params);
        if ($this->db->error()) {
            $info = $this->db->errorInfo();
            throw new Exception("Bank transaction write failed ({$context}): " . ($info[2] ?? 'unknown DB error'));
        }
        return $result;
    }
    ...
    private function initTelegram() {
        ...
        try {
            if (defined('TELEGRAM_BOT_TOKEN') && defined('TELEGRAM_CHAT_ID')
                ...
                return new TelegramNotifier(TELEGRAM_BOT_TOKEN, getTelegramChatId('bank_approved'));
            }
        } catch (Exception $e) {
    ...
    public function generateTransactionNumber($date = null) {
        // MAX-existing-sequence, not COUNT(*) — COUNT(*) collides with a live
        // transaction_number as soon as any row is soft/hard-deleted (the count
        // drops below the highest number actually ever issued). Same fix as
        // Purchaseadnanmanager.php's generateNoteNumber() (DAN/CAN bug, 3 Aug 2026).
        if (!$date) $date = date('Y-m-d');
        $prefix = 'BTX-' . date('Ymd', strtotime($date)) . '-';
        $row = $this->db->query(
            "SELECT transaction_number FROM bank_transactions WHERE transaction_number LIKE ? ORDER BY id DESC LIMIT 1",
            [$prefix . '%']
        )->first();
        $seq = $row ? ((int)substr($row->transaction_number, -4) + 1) : 1;
    ...
    public function saveTransactionType($data) {
        if (!empty($data['id'])) {
            ...
            if (function_exists('auditLog')) {
                auditLog('bank', 'type_updated',
                    'Bank transaction type "' . trim($data['name']) . '" updated',
            ...
            if (function_exists('auditLog')) {
                auditLog('bank', 'type_created',
                    'Bank transaction type "' . trim($data['name']) . '" created (' . $data['nature'] . ')',
                    ['record_id' => (int)$newId]
                );
            }
            return $newId;
    ...
    public function createTransaction($data, $userId, $userName, $ipAddress = null) {
        ...
        // Bridge into the site-wide audit trail (bank_tx_audit_log above is a
        // module-internal, per-transaction log only — it never surfaced here).
        if (function_exists('auditLog')) {
            auditLog('bank', 'created',
                "Bank transaction {$txNumber} created — ৳" . number_format((float)$data['amount'], 2) . ' (' . $data['entry_type'] . ')',
                ['record_id' => $txId, 'reference' => $txNumber, 'data' => [
                    'amount' => $data['amount'], 'entry_type' => $data['entry_type'],
                    'bank_tx_account_id' => $data['bank_tx_account_id'],
                ]]
            );
        }
    ...
    public function approveTransaction($txId, $userId, $userName, $ipAddress = null) {
        ...
        if (function_exists('auditLog')) {
            auditLog('bank', 'approved',
                "Bank transaction {$tx->transaction_number} approved — ৳" . number_format((float)$tx->amount, 2),
                ['record_id' => $txId, 'reference' => $tx->transaction_number]
            );
        }
    ...
    public function rejectTransaction($txId, $reason, $userId, $userName, $ipAddress = null) {
        ...
        if (function_exists('auditLog')) {
            auditLog('bank', 'rejected',
                "Bank transaction {$tx->transaction_number} rejected — reason: {$reason}",
                ['record_id' => $txId, 'reference' => $tx->transaction_number]
            );
        }
        $this->sendTelegramNotification('rejected', $txId, $tx->transaction_number, [
            'amount' => $tx->amount, 'entry_type' => $tx->entry_type, 'reason' => $reason,
        ], $userName);
        return true;
    }
    ...
    public function updateTransaction($txId, $data, $userId, $userName, $ipAddress = null, $role = null) {
        $old = $this->getTransactionById($txId);
        if (!$old) throw new Exception("Transaction not found.");

        // A Superadmin editing a transaction directly already carries full
        // approval authority — auto-approve on save instead of bouncing it to
        // 'pending' for someone else to re-review. Every other role keeps the
        // existing behavior (edit → back to pending, needs (re-)approval).
        $autoApprove = ($role === 'Superadmin');
        ...
        if ($autoApprove) {
            $this->checkedQuery(
                "UPDATE bank_transactions SET
                    ...
                    status='approved', approved_by_user_id=?, approved_at=NOW(),
                ...
                'updateTransaction (auto-approve)'
            );
        } else {
            $this->checkedQuery(
                "UPDATE bank_transactions SET
        ...
        // Audit trail — one entry for the edit itself, matching every other
        // update. Mirrors the internal per-transaction log too.
        $this->writeAuditLog($txId, 'updated', $userId, $userName, $ipAddress, (array)$old, $data);
        if (function_exists('auditLog')) {
            auditLog('bank', 'updated',
                "Bank transaction {$old->transaction_number} updated",
                ['record_id' => $txId, 'reference' => $old->transaction_number]
            );
        }

        // Second entry for the status transition — kept SEPARATE from the
        // 'updated' entry above so the trail reads identically to the normal
        // two-step flow (edit, then a distinct Approve click), just compressed
        // into one save by Superadmin.
        if ($autoApprove) {
            $this->writeAuditLog($txId, 'approved', $userId, $userName, $ipAddress,
                ['status' => $old->status], ['status' => 'approved'],
                'Auto-approved: edited directly by Superadmin ' . $userName
            );
            if (function_exists('auditLog')) {
                auditLog('bank', 'approved',
                    "Bank transaction {$old->transaction_number} auto-approved — edited directly by Superadmin (was: {$old->status})",
                    ['record_id' => $txId, 'reference' => $old->transaction_number]
                );
            }
        }

        // Telegram — one message per edit, not two, even in the auto-approve
        // case (avoids double-notifying for a single save action).
        $this->sendTelegramNotification(
            $autoApprove ? 'updated & auto-approved (Superadmin)' : 'updated',
            $txId, $old->transaction_number, $data, $userName
        );
    ...
    public function unpostTransaction($txId, $userId, $userName, $ipAddress = null, $role = null) {
        ...
        if (function_exists('auditLog')) {
            auditLog('bank', 'unposted',
                "Bank transaction {$tx->transaction_number} unposted (was {$prevStatus})" . ($notes ? " — {$notes}" : ''),
                ['record_id' => $txId, 'reference' => $tx->transaction_number]
            );
        }
    ...
    public function markReconciled($accountId, $userId, $userName, $notes = null) {
        ...
        if (function_exists('auditLog')) {
            auditLog('bank', 'reconciled',
                'Bank account "' . ($acc->account_name ?? "#{$accountId}") . '" marked reconciled — final balance ৳' . number_format($finalBalance, 2),
                ['record_id' => (int)$accountId]
            );
        }
    ...
    public function bulkUnpost(array $ids, $userId, $userName, $reason, $ipAddress = null) {
        ...
        foreach ($ids as $id) {
            ...
            if (function_exists('auditLog')) {
                auditLog('bank', 'bulk_unposted',
                    "Bank transaction {$tx->transaction_number} bulk-unposted (was {$prevStatus}) — reason: {$reason}",
                    ['record_id' => $id, 'reference' => $tx->transaction_number]
                );
            }
    ...
    public function bulkDelete(array $ids, $userId, $userName, $ipAddress = null) {
        ...
        foreach ($ids as $id) {
            ...
            if (function_exists('auditLog')) {
                auditLog('bank', 'deleted',
                    "Bank transaction {$tx->transaction_number} permanently deleted — ৳" . number_format((float)$tx->amount, 2),
                    ['record_id' => $id, 'reference' => $tx->transaction_number]
                );
            }

            // Feature #3: soft-delete to the Recycle Bin instead of erasing, so an
            // accidental bulk delete is fully restorable. A failure on one id
            // (either path) is caught here and counted as skipped rather than
            // aborting the rest of the batch or being silently swallowed.
            try {
                if (function_exists('ensureRecycleBinTables')) {
                    ...
                    $this->checkedQuery("DELETE FROM bank_tx_audit_log WHERE transaction_id = ?", [$id], 'bulkDelete (audit log)');
                    $this->checkedQuery("DELETE FROM bank_transactions WHERE id = ?", [$id], 'bulkDelete (transaction)');
                }
                $success++;
            } catch (Exception $e) {
                error_log("BankManager::bulkDelete failed for transaction {$id}: " . $e->getMessage());
                $skipped[] = $id;
            }
        }
    ...
    public function generateTransferNumber($date = null) {
        // MAX-existing-sequence, not COUNT(*) — see generateTransactionNumber()'s
        // docblock above for why.
        if (!$date) $date = date('Y-m-d');
        $prefix = 'TRF-' . date('Ymd', strtotime($date)) . '-';
        $row = $this->db->query(
            "SELECT transfer_number FROM bank_tx_transfers WHERE transfer_number LIKE ? ORDER BY id DESC LIMIT 1",
            [$prefix . '%']
        )->first();
        $seq = $row ? ((int)substr($row->transfer_number, -4) + 1) : 1;
        return $prefix . str_pad($seq, 4, '0', STR_PAD_LEFT);
    }
    ...
    public function createTransfer($data, $userId, $userName, $ipAddress = null) {
        ...
        if (function_exists('auditLog')) {
            auditLog('bank', 'transfer_created',
                "Bank transfer {$trnNumber} created — ৳" . number_format($amount, 2),
                ['record_id' => $trnId, 'reference' => $trnNumber, 'data' => [
    ...
    public function approveTransfer($trnId, $userId, $userName, $ipAddress = null) {
        ...
        // Three linked writes (debit tx, credit tx, transfer link) — previously
        // unwrapped raw $db->query() calls with no transaction and no error
        // check, so a silently-failed credit insert after a successful debit
        // insert would leave a real, one-sided unbalanced transfer. Now atomic.
        $pdo = $this->db->getPdo();
        ...
        try {
            ...
            $this->checkedQuery(
                ...
                'approveTransfer (debit leg)'
            );
            $fromTxId = $pdo->lastInsertId();
            ...
            $this->checkedQuery(
                ...
                'approveTransfer (credit leg)'
            );
            $toTxId = $pdo->lastInsertId();
        ...
        if (function_exists('auditLog')) {
            auditLog('bank', 'transfer_approved',
                "Bank transfer {$trn->transfer_number} approved — ৳" . number_format((float)$trn->amount, 2) . '; debit/credit entries created',
                ['record_id' => (int)$trnId, 'reference' => $trn->transfer_number]
            );
        }
    ...
    public function rejectTransfer($trnId, $reason, $userId, $userName, $ipAddress = null) {
        ...
        if (function_exists('auditLog')) {
            auditLog('bank', 'transfer_rejected',
                "Bank transfer {$trn->transfer_number} rejected — reason: {$reason}",
                ['record_id' => (int)$trnId, 'reference' => $trn->transfer_number]
            );
        }
