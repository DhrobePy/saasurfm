...
$bankTxAccounts = $db->query(
    "SELECT id, bank_name, account_name, account_number, opening_balance
     FROM bank_tx_accounts WHERE status = 'active' ORDER BY bank_name, account_name"
)->results();
...
if ($accountId) {
    ...
    // Filter by bank_tx_accounts.id — bank_tx rows only
    $allWhere  = 'WHERE bank_tx_account_id = ? AND txn_date BETWEEN ? AND ?';
    $allParams = [$btxAccountId, $dateFrom, $dateTo];
    $preWhere  = 'WHERE bank_tx_account_id = ? AND txn_date < ?';
...
$mainConditions = [];
$mainParams     = [];
if ($accountId)    { $mainConditions[] = 'bank_account_id = ?';    $mainParams[] = $accountId; }
if ($btxAccountId) { $mainConditions[] = 'bank_tx_account_id = ?'; $mainParams[] = $btxAccountId; }
if ($source !== 'all') { $mainConditions[] = 'source = ?'; $mainParams[] = $source; }
                ...
                <select name="account_id"
                    ...
                    <optgroup label="── Bank TX Accounts (Manual Entries)">
                        ...
                        <option value="-<?= $bta->id ?>" <?= $btxAccountId == $bta->id ? 'selected' : '' ?>>
                            <?= htmlspecialchars($bta->bank_name . ' — ' . $bta->account_name . ' (' . $bta->account_number . ')') ?>
                        </option>
                        <?php endforeach; ?>
                    </optgroup>
                    <?php endif; ?>
