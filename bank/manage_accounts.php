...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Both branches were previously unchecked — Database::query() never
        // throws, so a silently swallowed failure here would still redirect with
        // "Bank account created/updated" even though nothing was saved.
        if ($editMode && !empty($_POST['edit_id'])) {
            ...
            if ($db->error()) throw new Exception('Failed to update the bank account.');
            auditLog('bank', 'account_updated',
                "Bank account \"{$bankName} — {$accountName}\" ({$accountNo}) updated",
                ['record_id' => (int)$_POST['edit_id']]
            );
            ...
            if ($db->error()) throw new Exception('Failed to create the bank account.');
            $newAcctId = $db->getPdo()->lastInsertId();
            auditLog('bank', 'account_created',
                "Bank account \"{$bankName} — {$accountName}\" ({$accountNo}) created — opening balance ৳" . number_format($openBal, 2),
                ['record_id' => (int)$newAcctId]
            );
