...
$accountRoles = ['Superadmin', 'admin', 'Accounts', 'accounts-demra', 'accounts-srg'];
        ...
        <div class="mb-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                ...
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow cursor-pointer"
                     onclick="document.getElementById('bank_tx_account_id').value='<?php echo $acc->id; ?>'; filterForm.submit();">
                    ...
                    <p class="text-xs text-gray-400 mt-0.5"><?php echo $acc->account_type; ?></p>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1.5">
                                ...
                                <button onclick="deleteTransaction(<?php echo $tx->id; ?>, '<?php echo htmlspecialchars($tx->transaction_number); ?>')"
                                        title="Delete"
...
Be concise, use bullet points, mention BDT amounts. Focus on practical Bangladeshi banking context.`;
    ...
    try {
        ...
        el.innerHTML = '<p class="text-red-400 text-xs">Could not load AI suggestions. Check API connectivity.</p>';
    }
}
...
function approveTransaction(id, ref) {
    ...
    fetch('<?php echo url('bank/ajax_handler.php'); ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'action=approve&id=' + id + '&csrf=<?php echo $_SESSION['csrf_token'] ?? ''; ?>'
    })
    .then(r => r.json())
    .then(d => {
...
function submitReject() {
    ...
    fetch('<?php echo url('bank/ajax_handler.php'); ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'action=reject&id=' + rejectTxIdGlobal + '&reason=' + encodeURIComponent(reason) + '&csrf=<?php echo $_SESSION['csrf_token'] ?? ''; ?>'
    })
    .then(r => r.json())
    .then(d => {
        closeRejectModal();
...
function deleteTransaction(id, ref) {
    ...
    fetch('<?php echo url('bank/ajax_handler.php'); ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'action=delete&id=' + id + '&csrf=<?php echo $_SESSION['csrf_token'] ?? ''; ?>'
    })
    .then(r => r.json())
    .then(d => {
        if (d.success) { showToast('Transaction deleted.', 'success'); setTimeout(() => location.reload(), 800); }
        else showToast(d.message || 'Error', 'error');
    });
}
