...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'collect_loan_repayment') {
    try {
        ...
        // (MySQL evaluates same-statement SET assignments left-to-right, so
        // balance_due here correctly sees the just-incremented amount_repaid.)
        // Both writes were previously unchecked — a silently swallowed failure here
        // (Database::query() never throws) would still let the transaction commit,
        // leaving a loan_repayments row + journal posted for a "collected" repayment
        // that never actually reduced the loan's balance_due.
        $db->query(
            "UPDATE loans SET amount_repaid = amount_repaid + ?, balance_due = principal_amount - amount_repaid WHERE id = ?",
            [$amount, $lid]
        );
        if ($db->error()) throw new Exception('Failed to update the loan balance.');
        // Close the loan once fully repaid.
        $db->query("UPDATE loans SET status = 'closed' WHERE id = ? AND balance_due <= 0.01", [$lid]);
        if ($db->error()) throw new Exception('Failed to update the loan status.');
        ...
        // delete_loan_repayment.php locates this journal by related_document_id — if this
        // link silently failed to write, the repayment would become unreversible via the
        // normal Recycle-Bin path (an orphaned journal, permanently related_document_id=null).
        ...
        if ($db->error()) throw new Exception('Failed to link the journal entry to this repayment.');
