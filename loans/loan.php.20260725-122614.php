...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_loan') {
    try {
        ...
        $db->insert('transaction_lines', ['journal_entry_id' => $journal_id, 'account_id' => $loan_account_id, 'debit_amount' => $principal, 'credit_amount' => 0, 'description' => $journal_desc]);
        $db->insert('transaction_lines', ['journal_entry_id' => $journal_id, 'account_id' => $deposit_account_id, 'debit_amount' => 0, 'credit_amount' => $principal, 'description' => $journal_desc]);
