 ...
 * Reverses the old ledger/petty-cash/journal effects and reapplies the new
 * ones inside one transaction (stock tracking removed from POS, Jul 2026 —
 * no inventory involved), then records a pos_sale_edits diff row purely for
 * the audit trail (already-decided, not a pending approval).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    try {
        ...
        // Stock tracking removed from POS (Jul 2026) — no inventory adjustment here.
        ...
        // Reverse old cash/credit effects, reapply new ones. Only literal Cash
        // ever touched the physical drawer — same rule as the initial sale.
        $old_cash = (float)$order->cash_paid; $old_credit = (float)$order->credit_amount;
        if ($old_cash > 0.009 && $order->payment_method === 'Cash') {
        ...
        if ($cash_paid_new > 0.009 && $payment_method_new === 'Cash') {
        ...
        // Same Cash-vs-electronic split as the initial sale journal.
        $is_physical_cash = $cash_paid_new > 0.009 && $payment_method_new === 'Cash';
        $is_electronic = $cash_paid_new > 0.009 && !$is_physical_cash;
        $cash_account_id = $is_physical_cash ? getBranchPettyCashAccountId((int)$order->branch_id) : null;
        $undeposited_account_id = $is_electronic ? $db->query("SELECT id FROM chart_of_accounts WHERE name LIKE '%Undeposited%' AND account_type IN ('Other Current Asset', 'Cash') AND is_active = 1 LIMIT 1")->first() : null;
        ...
        // Every insert below was previously unchecked, unlike the surrounding checks
        // in this same function — the balance assertion further down only compares
        // locally-computed PHP totals ($td/$tc), not what actually landed in the DB,
        // so a silently swallowed insert failure (Database::query() never throws)
        // wouldn't trip it: a corrected sale could post an unbalanced/incomplete GL
        // journal entry while the transaction still committed and reported success.
        $td = 0; $tc = 0;
        if ($is_physical_cash) {
            ...
            if ($db->error()) throw new Exception('Failed to post the cash debit line of the edited journal entry.');
            $td += $cash_paid_new;
        } elseif ($is_electronic) {
            if (!$undeposited_account_id) throw new Exception('No active Undeposited Funds account found.');
            $db->query("INSERT INTO transaction_lines (journal_entry_id, account_id, debit_amount, credit_amount, description) VALUES (?, ?, ?, 0, ?)", [$new_journal_id, $undeposited_account_id->id, $cash_paid_new, "Undeposited Funds ({$payment_method_new}, edited) - {$order->order_number}"]);
            if ($db->error()) throw new Exception('Failed to post the undeposited-funds debit line of the edited journal entry.');
            $td += $cash_paid_new;
        }
        if ($credit_amount_new > 0.009) {
            ...
            if ($db->error()) throw new Exception('Failed to post the AR debit line of the edited journal entry.');
        ...
        if ($db->error()) throw new Exception('Failed to post the revenue credit line of the edited journal entry.');
        $tc += $new_subtotal;
