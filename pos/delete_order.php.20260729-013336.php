 ...
 * Reverses inventory, reverses any pos_customer_ledger credit entry, archives
 * (not reversal-entries) the original journal — Option A convention, matching
...
try {
    ...
    $items = $db->query("SELECT * FROM order_items WHERE order_id = ?", [$order_id])->results();
    foreach ($items as $it) {
        $db->query(
            "UPDATE inventory SET quantity = quantity + ?, updated_at = NOW() WHERE variant_id = ? AND branch_id = ?",
            [$it->quantity, $it->variant_id, $order->branch_id]
        );
        if ($db->error()) throw new Exception('Failed to restore inventory: ' . ($db->errorInfo()[2] ?? ''));
    }
    ...
    // Reverse the cash-in petty cash effect, if any.
    if ((float)$order->cash_paid > 0.009) {
