...
if (!$error || (in_array($user_role, ['Superadmin', 'admin']) && $branch_id)) {
    try {
        ...
        //
        // product_prices is branch-wise (product/pricing.php maintains one row
        // per branch per variant) — prefer this branch's own price, falling
        // back to any other active row for that variant if this branch never
        // got one set (rather than hiding the product entirely).
        $sql = "SELECT
                pv.id as variant_id,
                pv.sku,
                pv.weight_variant,
                ...
                (SELECT pp.unit_price
                 ...
                 WHERE pp.variant_id = pv.id
                   AND pp.is_active = 1
                 ORDER BY (pp.branch_id = :branch_id_price) DESC, pp.effective_date DESC, pp.created_at DESC
        ...
        foreach ($products as $product) {
            ...
            if ($price > 0) {
                ...
                $product->origin_name = $product->origin_name ? htmlspecialchars($product->origin_name, ENT_QUOTES, 'UTF-8') : null;
...
function posApp() {
    return {
        ...
        printAllCopies() {
            ...
            // wrapping it in setTimeout loses the browser's "user gesture"
            // trust, so every deferred popup after the first gets silently
            // blocked (this was why only 1 of 3 copies ever opened).
            ['office', 'customer', 'delivery'].forEach(copyType => {
                if (this.printCopies[copyType]) {
                    this.printReceipt(copyType);
                }
            });
                    ...
                    <template x-for="product in filteredProducts" :key="product.variant_id">
                        ...
                        >
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <template x-if="product.origin_name">
                                    <span class="text-xs px-2 py-1 rounded-full font-bold ml-2 whitespace-nowrap bg-indigo-100 text-indigo-800" x-text="product.origin_name"></span>
                                </template>
                            </div>
