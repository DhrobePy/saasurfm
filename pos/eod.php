...
<div class="bg-white rounded-lg shadow-md p-8 mb-8 text-center">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-gray-50 rounded-lg p-5 mb-6 text-left max-w-md mx-auto">
            <label class="block text-sm font-bold text-gray-700 mb-2">Actual cash counted in drawer (optional — defaults to system-expected)</label>
            <input type="number" id="eodActualCash" step="0.01" placeholder="Leave blank to use expected cash"
                   class="w-full px-3 py-2 border-2 border-gray-300 rounded-lg mb-3">
            <label class="block text-sm font-bold text-gray-700 mb-2">If there's a variance, note why (optional)</label>
            <input type="text" id="eodVarianceNotes" placeholder="e.g. ৳200 short — customer change dispute"
                   class="w-full px-3 py-2 border-2 border-gray-300 rounded-lg mb-4">
            <label class="block text-sm font-bold text-gray-700 mb-2">Today's cash — where does it go?</label>
            <div class="space-y-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="eodDisposition" value="petty_cash" checked class="w-4 h-4">
                    <span class="text-sm">Stays in branch petty cash</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="eodDisposition" value="bank_deposit_pending" class="w-4 h-4">
                    <span class="text-sm">Being deposited to the bank (confirm the deposit tomorrow)</span>
                </label>
            </div>
        </div>
...
function processEOD(branchId) {
    const actualCashEl = document.getElementById('eodActualCash');
    const varianceEl = document.getElementById('eodVarianceNotes');
    const dispositionEl = document.querySelector('input[name="eodDisposition"]:checked');
    fetch('eod_process.php', {
        ...
        body: JSON.stringify({
            branch_id: branchId,
            actual_cash: actualCashEl ? actualCashEl.value : '',
            variance_notes: varianceEl ? varianceEl.value : '',
            cash_disposition: dispositionEl ? dispositionEl.value : 'petty_cash'
        })
    })
