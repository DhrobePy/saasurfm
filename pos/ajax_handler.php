 ...
 */
function create_pos_sale_journal_entry($db, $order_id, $order_number, $branch_id, $branch_code, $cash_paid, $credit_amount, $subtotal, $discount_amount, $user_id, $payment_method) {
    ...
    // Only a literal Cash payment hits the branch's physical petty cash
    // account — Bank Deposit/Bank Transfer/Card/Mobile Banking never touched
    // the till, so they land in Undeposited Funds instead (same distinction
    // the pre-rebuild POS journal made; collapsing this into one "cash_paid"
    // bucket was a regression introduced by the split-payment rewrite).
    $is_physical_cash = $cash_paid > 0.009 && $payment_method === 'Cash';
    $is_electronic = $cash_paid > 0.009 && !$is_physical_cash;
    $cash_account_id = $is_physical_cash ? getBranchPettyCashAccountId($branch_id) : null;
    $undeposited_account_id = $is_electronic ? find_account_id($db, 'Undeposited', ['Other Current Asset', 'Cash']) : null;
    ...
    if ($is_physical_cash && !$cash_account_id) throw new Exception('No active petty cash account configured for this branch — set one up under Branch Petty Cash before taking cash sales.');
    if ($is_electronic && !$undeposited_account_id) throw new Exception('Chart of Accounts: no active Undeposited Funds account found for electronic payments.');
    ...
    if ($is_physical_cash) {
        ...
        $db->query(
            "INSERT INTO transaction_lines (journal_entry_id, account_id, debit_amount, credit_amount, description) VALUES (?, ?, ?, 0, ?)",
            [$journal_id, $undeposited_account_id, $cash_paid, "Undeposited Funds ({$payment_method}) - Order #{$order_number}"]
        );
        pos_check_db($db, 'journal line: undeposited funds debit');
        $total_debits += $cash_paid;
    }
...
function execute_pos_sale(Database $db, array $o, int $acting_user_id, ?int $pending_req_id = null): array {
    ...
    // orders.payment_method is a strict ENUM without 'bKash'/'Nagad' values —
    // the POS UI folds those into 'Mobile Banking' + a prefixed reference
    // client-side, but normalize here too so this doesn't hard-fail on a raw
    // DB error if anything (a different caller, a future integration) ever
    // sends the provider name directly instead of pre-mapping it.
    if (in_array($payment_method, ['bKash', 'Nagad'], true)) {
        $payment_reference = $payment_method . ($payment_reference ? " - {$payment_reference}" : '');
    ...
    try {
        ...
        // Only literal Cash hits the physical drawer — Bank/Card/Mobile
        // Banking never touched the till, so EOD's cash count must not
        // include them (see create_pos_sale_journal_entry for the same split).
        if ($cash_paid > 0.009 && $payment_method === 'Cash') {
        ...
        $journal_id = create_pos_sale_journal_entry(
            $db, $order_id, $order_number, $branch_id, $branch_code, $cash_paid, $credit_amount, $subtotal, $total_discount, $acting_user_id, $payment_method
        );
