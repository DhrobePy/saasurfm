...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $branch_id) {
    ...
    try {
        // Previously unchecked — wrapped in a try/catch that can't see a swallowed
        // Database::query() failure, so $success was set unconditionally even if
        // this insert silently failed. This log is exactly the anti-shortage/theft
        // record that matters most when a real variance investigation happens —
        // a silent failure removes it without a trace, right when it's needed.
        ...
        if ($db->error()) throw new Exception('Failed to save the cash verification record.');
