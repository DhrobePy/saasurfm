...
$allowed_roles = ['Superadmin', 'admin', 'Accounts', 'accounts-srg', 'accounts-demra', 'accountspos-demra', 'accountspos-srg'];
...
if (isset($_GET['export']) && $_GET['export'] === 'csv' && !$error) {
    ...
    fputcsv($output, []);
    foreach ($orders as $order) {
        fputcsv($output, ['Order', $order->order_number, $order->order_date ? date('h:i A', strtotime($order->order_date)) : '', $order->customer_name ?? 'Walk-in', $order->payment_method]);
...
function deletePosOrder(orderId, orderNumber) {
    if (!confirm('Delete order ' + orderNumber + '? Inventory and any credit balance will be reversed. This can be restored from the Recycle Bin.')) return;
                    ...
                    <?php foreach($orders as $order):?>
                        <tr class="border-t border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-right text-sm font-medium space-x-3 no-print whitespace-nowrap">
                                ...
                                <?php if ($is_admin): ?>
                                <a href="edit_order.php?id=<?php echo (int)$order->id; ?>" class="text-amber-600 hover:text-amber-900" title="Edit"><i class="fas fa-pen"></i></a>
                                <button type="button" onclick="deletePosOrder(<?php echo (int)$order->id; ?>, '<?php echo htmlspecialchars($order->order_number, ENT_QUOTES); ?>')" class="text-red-600 hover:text-red-900" title="Delete"><i class="fas fa-trash"></i></button>
                                <?php endif; ?>
                            </td>
                        </tr>
