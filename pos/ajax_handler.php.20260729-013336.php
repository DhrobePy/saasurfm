...
function execute_pos_sale(Database $db, array $o, int $acting_user_id, ?int $pending_req_id = null): array {
    ...
    try {
        ...
        foreach ($cart as $item) {
            ...
            // Row-lock the inventory record for the life of this transaction —
            // closes the check-then-decrement race between concurrent terminals.
            $stock_check = $db->query(
                "SELECT quantity FROM inventory WHERE variant_id = ? AND branch_id = ? FOR UPDATE",
                [$variant_id, $branch_id]
            )->first();
            pos_check_db($db, "lock inventory for variant {$variant_id}");
            if (!$stock_check) throw new Exception("Product variant {$variant_id} not found in inventory for this branch");
            ...
            $db->query(
                "UPDATE inventory SET quantity = quantity - ?, updated_at = NOW() WHERE variant_id = ? AND branch_id = ?",
                [$quantity, $variant_id, $branch_id]
            );
            pos_check_db($db, 'decrement inventory');
        }
        ...
        case 'get_products':
            ...
            $sql = "SELECT
                    ...
                    (SELECT pp.unit_price FROM product_prices pp WHERE pp.variant_id = pv.id AND pp.is_active = 1
                     ORDER BY pp.effective_date DESC, pp.created_at DESC LIMIT 1) as unit_price,
                    COALESCE(inv.quantity, 0) as stock_quantity
                FROM product_variants pv
                JOIN products p ON pv.product_id = p.id
                LEFT JOIN inventory inv ON inv.variant_id = pv.id AND inv.branch_id = :branch_id_inv
                WHERE p.status = 'active' AND pv.status = 'active'
                ORDER BY p.base_name, pv.sku";

            $products = $db->query($sql, ['branch_id_inv' => $branch_id])->results();
            $valid_products = [];
