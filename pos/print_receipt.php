...
// Security check — same POS role gate as every other page in this module
// (previously just checked isset($_SESSION['user_id']), which let any logged-in
// staff of any role/branch view or print any branch's receipt by URL-guessing).
$pos_allowed_roles = ['Superadmin', 'admin', 'accountspos-demra', 'accountspos-srg', 'dispatchpos-demra', 'dispatchpos-srg'];
if (!isset($_SESSION['user_id']) || !in_array($_SESSION['user_role'] ?? '', $pos_allowed_roles)) {
    die('Unauthorized');
}
...
<body>
    <div class="receipt">
        ...
        <div class="footer">
            ...
            <?php if ($copy_type === 'customer' || $copy_type === 'delivery'): ?>
            <div style="text-align: center; margin-top: 12px;">
                <canvas id="posExitQrPrint" width="90" height="90"></canvas>
                <div style="font-size: 9px; color: #666; margin-top: 2px;">Show this at exit</div>
            </div>
            <?php endif; ?>
    ...
    <script>
        (function () {
            var el = document.getElementById('posExitQrPrint');
            if (!el || typeof QRious === 'undefined') return;
            var qrUrl = <?php echo json_encode(
                (defined('APP_URL') ? rtrim(APP_URL, '/') : '') . '/pos/verify_exit.php?order=' . urlencode($order->order_number) . '&sig=' . posExitQrSignature($order->order_number)
            ); ?>;
            try { new QRious({ element: el, value: qrUrl, size: 90, level: 'M' }); } catch (e) {}
        })();
