...
try {
    ...
    // Both writes below were previously unchecked — Database::query() silently
    // swallows PDOExceptions (sets an internal error flag, never throws), so a
    // failed audit-trail INSERT was silently skipped (a Superadmin-only "reopen a
    // closed financial day" action left with NO audit trail, despite the UI's own
    // confirm() dialog promising "this action will be logged"), and a failed DELETE
    // still returned success=true — telling the admin the day was unlocked when
    // eod_summary was actually untouched, and every next attempt would keep hitting
    // "EOD has already been run for today." Wrapped in a transaction so a failure on
    // either write rolls back both instead of leaving a half-applied reopen.
    $db->beginTransaction();
    ...
    if ($db->error()) {
        $db->rollback();
        throw new Exception("Failed to record the reopen audit trail entry.");
    }
    ...
    if ($db->error()) {
        $db->rollback();
        throw new Exception("Failed to delete the EOD record.");
    }
