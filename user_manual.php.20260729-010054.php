...
[
  ...
  'steps' => [
    'Open <strong>Trading → Trading Dashboard</strong> for a quick overview: this month\'s sales, profit, stock value, and anything needing attention.',
    'To sell wheat (or any other approved commodity): <strong>Trading → Commodity Sale</strong>. Pick the customer, the commodity, the branch it\'s shipping from, quantity and price (prices are volatile, so you type the price by hand every time — the system doesn\'t assume yesterday\'s rate).',
    'The system checks how much stock is on hand at that branch. If you try to sell more than what\'s recorded as received, it warns you — you can still proceed by ticking <strong>"sell anyway"</strong> (e.g. if a truck is on the way but not yet logged in).',
  ...
  'qa' => [
    ...
    ['q' => 'How is the "cost" of a sale calculated?', 'a' => 'The system keeps a running <strong>weighted-average cost</strong> per commodity per branch, updated every time a purchase (GRN) is received. When you sell, it uses that average automatically — you never have to work it out by hand.'],
    ['q' => 'I made a mistake on a commodity sale — can it be undone?', 'a' => 'Yes. If <em>no payment</em> has been collected against it yet, open <strong>Trading → Commodity Sale</strong>, find it in Recent Sales, and click <strong>Delete</strong> (you\'ll be asked for a reason). This reverses the ledger entry, puts the stock back, and moves the record to the <strong>Recycle Bin</strong> — nothing is ever erased, so a Superadmin can restore it if needed.'],
...
[
  ...
  'qa' => [
    ['q' => 'How does the Recycle Bin work?', 'a' => 'Every deleted business record — orders, payments, customers, ledger entries, expenses, returns, products, bank entries, commodity sales &amp; commodity payments — lands here in a <strong>batch</strong>. Restore brings the whole thing back (including the balances it changed); Purge removes it permanently.'],
