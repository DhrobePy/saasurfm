...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($_GET['action'] === 'delete' && canDeleteExpense()) {
        ...
        if ($voucher && $voucher->status === 'pending') {
            // Feature #3: soft-delete pending vouchers to the Recycle Bin. Pending
            // vouchers have no posted accounting, so this is a clean, restorable move.
            ensureRecycleBinTables();
            $vnum = $db->query("SELECT voucher_number FROM expense_vouchers WHERE id = ?", [$id])->first();
            $pdo  = $db->getPdo();
            $pdo->beginTransaction();
            $batch = recycleBegin('expense_voucher', 'Expense voucher ' . ($vnum->voucher_number ?? "#$id") . " (#$id)");
            recycleArchiveDelete($batch, 'expense_action_log', 'expense_voucher_id', $id);
            recycleArchiveDelete($batch, 'expense_vouchers',   'id',                 $id);
            recycleFinalize($batch);
            $pdo->commit();
            $_SESSION['success_flash'] = 'Expense voucher moved to Recycle Bin (batch #' . $batch . ') — restorable from Admin → Recycle Bin';
        } else {
