...
// The bare restrict_access() above only enforces "deny unless the 'expenses'
// module + this page are explicitly granted via Privileges" — it does NOT
// actually enforce this file's own documented "Superadmin only" intent, so an
// admin could unknowingly delegate edit access to someone else. canEditExpense()
// is the real Superadmin-only gate already used consistently elsewhere in this
// module (canDeleteExpense(), canApproveExpense()).
if (!canEditExpense()) {
    $_SESSION['error_flash'] = 'Only Superadmin can edit expense vouchers.';
