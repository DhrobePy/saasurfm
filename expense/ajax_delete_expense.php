...
try {
    ...
    // Feature #3: archive to the Recycle Bin instead of erasing. Archive the
    // action-log rows first (they'd otherwise CASCADE away), then the voucher.
    $batch = recycleBegin('expense_voucher',
        'Expense voucher ' . ($expense->voucher_number ?? "#$expenseId") . " (#$expenseId)");
    recycleArchiveDelete($batch, 'expense_action_log', 'expense_voucher_id', $expenseId);
    recycleArchiveDelete($batch, 'expense_vouchers',   'id',                 $expenseId);
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Expense voucher moved to Recycle Bin (batch #' . $batch . ') — restorable from Admin → Recycle Bin'
    ]);
