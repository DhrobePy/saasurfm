...
// Bug 14 fix: restrict_access() was called with no arguments — any logged-in user
// could approve/reject expense vouchers (critical privilege escalation).
$allowed_roles = ['Superadmin', 'admin', 'Expense Approver'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve' && $voucher_id) {
        ...
        // Feature #8: approve many vouchers in one action.
        $ids = array_values(array_unique(array_filter(array_map('intval', $_POST['voucher_ids'] ?? []))));
        $approver_id = $_SESSION['user_id'] ?? null;
        if (!$approver_id) {
            $_SESSION['error_flash'] = 'Session expired. Please login again.';
        } elseif (empty($ids)) {
            $_SESSION['error_flash'] = 'No vouchers selected.';
        } else {
            $ok = 0; $fail = 0; $errs = [];
            foreach ($ids as $vid) {
                $result = $expenseManager->approveExpenseVoucher($vid, $approver_id);
                if (!empty($result['success'])) { $ok++; }
                else { $fail++; if (count($errs) < 3) $errs[] = "#{$vid}: " . ($result['message'] ?? 'failed'); }
            }
            if ($ok)   $_SESSION['success_flash'] = "Approved {$ok} voucher(s)" . ($fail ? " — {$fail} failed." : ".");
            if ($fail) $_SESSION['error_flash']   = "{$fail} could not be approved. " . implode(' | ', $errs);
        }
    }
...
<div class="container mx-auto px-4 py-6">
    ...
    <?php else: ?>
        <!-- Feature #8: bulk approve bar -->
        <form id="bulkApproveForm" method="POST"
              onsubmit="return confirm('Approve all selected vouchers? Approved expenses post their accounting immediately.');"
              class="bg-white rounded-lg shadow-md p-4 mb-4 flex flex-wrap items-center gap-3">
            <input type="hidden" name="action" value="bulk_approve">
            <label class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer">
                <input type="checkbox" id="selectAllVouchers" onclick="toggleAllVouchers(this)" class="w-4 h-4 accent-green-600">
                Select all
            </label>
            <span id="bulkCount" class="text-xs text-gray-400">0 selected</span>
            <button type="submit" id="bulkApproveBtn" disabled
                    class="ml-auto px-4 py-2 bg-green-600 text-white text-sm font-semibold rounded-lg hover:bg-green-700 disabled:opacity-40 disabled:cursor-not-allowed">
                        ...
                        <div class="flex justify-between items-start">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center mb-3">
                                    <input type="checkbox" name="voucher_ids[]" value="<?php echo (int)$voucher->id; ?>"
                                           form="bulkApproveForm" onchange="updateBulkCount()"
                                           class="voucher-check w-4 h-4 mr-3 accent-green-600 cursor-pointer" title="Select for bulk approve">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                    <div>
                                        ...
                                        <p class="font-semibold"><?php echo $voucher->expense_date ? date('M d, Y', strtotime($voucher->expense_date)) : '—'; ?></p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500"><?php echo $voucher->created_at ? date('M d, h:i A', strtotime($voucher->created_at)) : '—'; ?></p>
                                    </div>
                                </div>
...
// ── Feature #8: bulk-approve selection ──
function updateBulkCount() {
    const checked = document.querySelectorAll('.voucher-check:checked').length;
    const total   = document.querySelectorAll('.voucher-check').length;
    const btn = document.getElementById('bulkApproveBtn');
    const cnt = document.getElementById('bulkCount');
    if (cnt) cnt.textContent = checked + ' selected';
    if (btn) btn.disabled = checked === 0;
    const all = document.getElementById('selectAllVouchers');
    if (all) all.checked = checked > 0 && checked === total;
}
function toggleAllVouchers(cb) {
    document.querySelectorAll('.voucher-check').forEach(c => c.checked = cb.checked);
    updateBulkCount();
}
</script>
