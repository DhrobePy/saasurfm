...
// Seed default factory branches the first time (when none have been configured yet)
$any_factory = $db->query("SELECT COUNT(*) AS cnt FROM branches WHERE is_factory = 1")->first();
if (!$any_factory || (int)$any_factory->cnt === 0) {
    // Seed by id (1=Sirajganj, 2=Demra) — most reliable, no code typo risk
    try { $db->getPdo()->exec("UPDATE `branches` SET is_factory = 1 WHERE id IN (1, 2)"); } catch (\Throwable $e) {}
}

$branches = $db->query(
    "SELECT * FROM branches WHERE status = 'active' AND is_factory = 1 ORDER BY name ASC"
)->results();

// Fallback: if no factory branches set yet (e.g. status case mismatch), show all active
if (empty($branches)) {
