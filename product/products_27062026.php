...
function grade_palette(string $g): array {
    $u = strtoupper($g);
...
<div class="flex flex-wrap items-center justify-between gap-3 mb-4">
    <div>
        ...
        <p class="text-xs text-gray-400 mt-0.5">Factory price overview — all grades</p>
    </div>
...
<div class="flex flex-wrap items-center gap-3 mb-5">
    ...
    <div class="ml-auto flex items-center gap-2 flex-wrap">
        ...
        <?php endforeach; ?>
        <span class="text-[10px] text-gray-400 italic">(Big Truck · 25MT)</span>
    </div>
    <?php endif; ?>
</div>
...
<div class="flex gap-1 mb-4 border-b border-gray-200">
    ...
    <?php foreach ($by_grade as $g => $gvars):
        $pal = grade_palette($g);
    ?>
    <button class="grade-tab px-3 py-1.5 text-xs font-semibold text-gray-500 hover:text-gray-700 rounded-t-lg -mb-px border border-transparent hover:border-gray-200 transition-colors" data-grade="<?php echo htmlspecialchars($g); ?>">
        <span class="inline-block w-1.5 h-1.5 rounded-full <?php echo $pal['dot']; ?> mr-1 align-middle"></span>
        <?php echo htmlspecialchars($g); ?>
        <span class="text-[10px] text-gray-400 ml-0.5">(<?php echo count($gvars); ?>)</span>
    </button>
    <?php endforeach; ?>
</div>
<?php endif; ?>
...
<div class="space-y-5">
<?php foreach ($by_grade as $grade => $gvars):
    $pal = grade_palette($grade);
    $factory_cols = count($factories);
?>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    ...
                    <th class="px-3 py-2.5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide w-24">Pack</th>
                    <?php foreach ($factories as $fi => $f):
                        $hcolors = ['text-blue-600','text-violet-600','text-teal-600','text-orange-600'];
                        $hc = $hcolors[$fi % count($hcolors)];
                    ?>
                    <th class="px-4 py-2.5 text-right text-[10px] font-semibold uppercase tracking-wide <?php echo $hc; ?>">
            ...
            <tbody>
            <?php
            // Group rows by product for zebra striping
            $prev_pid = null;
            $row_stripe = false;
            foreach ($gvars as $vi => $v):
                if ($v->product_id !== $prev_pid) {
                    $row_stripe = !$row_stripe;
                    $prev_pid = $v->product_id;
                }
                $has_price = false;
                foreach ($factories as $f) {
                    if (isset($price_map[$v->id][$f->id])) { $has_price = true; break; }
                }
            ?>
            <tr class="variant-row border-b border-gray-50 last:border-0 hover:bg-blue-50/30 transition-colors group
                ...
                <!-- Price per factory column -->
                <?php foreach ($factories as $fi => $f):
                    $price = $price_map[$v->id][$f->id] ?? null;
                    $pcolors = ['text-blue-700','text-violet-700','text-teal-700','text-orange-700'];
                    $pc = $pcolors[$fi % count($pcolors)];
                ?>
                <td class="px-4 py-2.5 text-right whitespace-nowrap">
                    <?php if ($price !== null): ?>
                        <span class="text-sm font-extrabold <?php echo $pc; ?>">৳<?php echo number_format($price, 0); ?></span>
                    <?php else: ?>
