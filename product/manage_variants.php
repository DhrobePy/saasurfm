...
function generate_sku($base_sku, $weight, $grade) {
    ...
    $parts = [$sku_prefix, $sku_weight];
    $grade_clean = trim($grade ?? '');
    if ($grade_clean !== '') {
        $sku_grade = strtoupper(preg_replace('/[^a-zA-Z0-9]/', '', $grade_clean));
        if (strlen($sku_grade) > 1) $sku_grade = substr($sku_grade, 0, 1);
        $parts[] = $sku_grade;
    }
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['add_variant'])) {
            ...
            $db->query("INSERT INTO product_variants (product_id, sku, weight_variant, grade, unit_of_measure, status, origin_branch_id) VALUES (?, ?, ?, ?, ?, ?, ?)", [
                $product_id,
                $generated_sku,
                $_POST['weight_variant'],
                $_POST['grade'],
        ...
        if (isset($_POST['update_variant'])) {
            ...
            $db->query("UPDATE product_variants SET sku = ?, weight_variant = ?, grade = ?, unit_of_measure = ?, status = ?, origin_branch_id = ? WHERE id = ? AND product_id = ?", [
                $generated_sku,
                $_POST['weight_variant'],
                $_POST['grade'],
                $_POST['unit_of_measure'],
    ...
    // --- DELETE VARIANT (Feature #3: soft-delete to Recycle Bin) ---
    if (isset($_GET['delete'])) {
        $delete_id = (int)$_GET['delete'];
        // Guard: only delete a variant that belongs to this product
        $owns = $db->query("SELECT id FROM product_variants WHERE id = ? AND product_id = ?", [$delete_id, $product_id])->first();
        if ($owns) {
            // Archive pricing + inventory BEFORE the variant so the CASCADE has
            // nothing left and the restore is complete.
            ensureRecycleBinTables();
            $var = $db->query("SELECT sku FROM product_variants WHERE id = ?", [$delete_id])->first();
            $pdo = $db->getPdo();
            $pdo->beginTransaction();
            $batch = recycleBegin('product_variant', 'Variant — ' . ($var->sku ?? "#$delete_id") . " (#$delete_id)");
            recycleArchiveDelete($batch, 'product_prices',   'variant_id', $delete_id);
            recycleArchiveDelete($batch, 'inventory',        'variant_id', $delete_id);
            recycleArchiveDelete($batch, 'product_variants', 'id',         $delete_id);
            recycleFinalize($batch);
            $pdo->commit();
            $_SESSION['success_flash'] = 'Product Variant moved to Recycle Bin (batch #' . $batch . ') — restorable from Admin → Recycle Bin.';
        } else {
            $_SESSION['error_flash'] = 'Variant not found for this product.';
        }
...
$variants = $db->query(
    "SELECT pv.*, b.name AS origin_branch_name
     FROM product_variants pv
     LEFT JOIN branches b ON b.id = pv.origin_branch_id
     WHERE pv.product_id = ? ORDER BY pv.weight_variant ASC, pv.grade ASC",
    [$product_id]
)->results();
...
<div class="bg-white rounded-lg shadow-md p-6 mb-6">
    ...
    <form action="manage_variants.php?product_id=<?php echo $product_id; ?>" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <!-- Grade / Type -->
        <div>
            <label for="grade" class="block text-sm font-medium text-gray-700 mb-1">Grade / Type <span class="text-gray-400 text-xs font-normal">(optional)</span></label>
            <input type="text" id="grade" name="grade"
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                   value="<?php echo htmlspecialchars($variant_to_edit->grade ?? ''); ?>"
                   placeholder="e.g., A-Grade, B-Grade, Mota, Chikon">
            <p class="mt-1 text-xs text-gray-400">Flour grades: A-Grade, B-Grade, C-Grade. Type variants (e.g. Vushi): Mota, Chikon. Leave blank if not applicable.</p>
        </div>

        <!-- Origin / Factory -->
        <div>
            <label for="origin_branch_id" class="block text-sm font-medium text-gray-700 mb-1">Origin / Factory <span class="text-gray-400 text-xs font-normal">(optional)</span></label>
            <select id="origin_branch_id" name="origin_branch_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                <option value="">— Not set —</option>
                <?php foreach ($branches as $b): ?>
                <option value="<?php echo $b->id; ?>" <?php echo (int)($variant_to_edit->origin_branch_id ?? 0) === (int)$b->id ? 'selected' : ''; ?>><?php echo htmlspecialchars($b->name); ?></option>
                <?php endforeach; ?>
            </select>
            <p class="mt-1 text-xs text-gray-400">Which factory/mill actually produces this variant — separate from which branch sells it.</p>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Origin / Factory</th>
                    ...
                    <?php foreach ($variants as $variant): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <?php echo $variant->origin_branch_name ? htmlspecialchars($variant->origin_branch_name) : '—'; ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
