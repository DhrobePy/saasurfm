...
try {
    ...
    // --- DELETE PRODUCT (Feature #3: soft-delete to Recycle Bin) ---
    if (isset($_GET['delete'])) {
        $delete_id = (int)$_GET['delete'];
        // Archive children (prices, inventory, variants) BEFORE the parent, so the
        // ON DELETE CASCADE has nothing left to erase and a restore is complete.
        ensureRecycleBinTables();
        $prod = $db->query("SELECT base_name FROM products WHERE id = ?", [$delete_id])->first();
        $pdo  = $db->getPdo();
        $pdo->beginTransaction();
        $batch = recycleBegin('product', 'Product — ' . ($prod->base_name ?? "#$delete_id") . " (#$delete_id)");
        foreach ($db->query("SELECT id FROM product_variants WHERE product_id = ?", [$delete_id])->results() as $v) {
            recycleArchiveDelete($batch, 'product_prices', 'variant_id', (int)$v->id);
            recycleArchiveDelete($batch, 'inventory',      'variant_id', (int)$v->id);
        }
        recycleArchiveDelete($batch, 'product_variants', 'product_id', $delete_id);
        recycleArchiveDelete($batch, 'products',         'id',         $delete_id);
        recycleFinalize($batch);
        $pdo->commit();
        $_SESSION['success_flash'] = 'Base Product and its variants moved to Recycle Bin (batch #' . $batch . ') — restorable from Admin → Recycle Bin.';
