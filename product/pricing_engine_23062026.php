    ...
    if ($action === 'apply_prices') {
        ...
        try {
            ...
            foreach ($grades as $grade) {
                ...
                $base74 = round(($base50 / $bag_50) * $bag_74 + $pkg_fee, 2);
                ...
                foreach (['50', '74'] as $wc) {
                    ...
                    foreach ($grade_data[$grade][$wc] ?? [] as $item) {
                        foreach ($branches as $b) {
                            ...
                            $final = round($base_price + $surcharge, 2);
            ...
            auditLog('pricing_engine', 'applied',
                "Smart Pricing updated {$total_updated} price records. " .
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
                <h3 class="font-semibold text-gray-700 mb-3 flex items-center gap-2">
                    ...
                    Branch Surcharges (BDT added to base)
                </h3>
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="text-xs text-gray-500 uppercase bg-gray-50">
                                ...
                                <th class="px-3 py-2 text-center">+ 50 kg</th>
                                <th class="px-3 py-2 text-center">+ 74 kg</th>
                            </tr>
                        </thead>
                            ...
                            <tr>
                                ...
                                <td class="px-3 py-2">
                                    <input type="number"
                                           ...
                                           class="w-full px-2 py-1 border rounded text-center text-sm focus:ring-2 focus:ring-primary-400">
                                </td>
                                <td class="px-3 py-2">
                                    <input type="number"
                ...
                <p class="mt-3 text-xs text-gray-500">
                    ...
                    Surcharges apply to grade-based (50/74 kg) products only. Custom-weight products use a flat price.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3 text-xs text-gray-500">
                <span><span class="inline-block w-2.5 h-2.5 bg-blue-200 border border-blue-400 rounded mr-1"></span>50 kg</span>
                <span><span class="inline-block w-2.5 h-2.5 bg-purple-200 border border-purple-400 rounded mr-1"></span>74 kg</span>
            </div>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm" id="previewTable">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <?php foreach ($branches as $b): ?>
                        <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase bg-blue-50 border-l border-blue-200 min-w-[90px]">
                            <?php echo htmlspecialchars($b->name); ?><br>
                            <span class="text-gray-400 font-normal normal-case">50 kg</span>
                        </th>
                        <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase bg-purple-50 border-r border-purple-200 min-w-[90px]">
                            <?php echo htmlspecialchars($b->name); ?><br>
                            <span class="text-gray-400 font-normal normal-case">74 kg</span>
                        </th>
                        <?php endforeach; ?>
                    </tr>
                </thead>
                <tbody id="previewBody" class="divide-y divide-gray-100 bg-white">
                    ...
                    <tr data-grade="<?php echo htmlspecialchars($grade); ?>" class="hover:bg-gray-50">
                        ...
                        <?php foreach ($branches as $b): ?>
                        <td class="px-3 py-3 text-center bg-blue-50 text-blue-800"
                            id="prev_b<?php echo $b->id; ?>_50_<?php echo htmlspecialchars($grade); ?>">—</td>
                        <td class="px-3 py-3 text-center bg-purple-50 text-purple-800"
                            id="prev_b<?php echo $b->id; ?>_74_<?php echo htmlspecialchars($grade); ?>">—</td>
                        <?php endforeach; ?>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <div class="px-6 py-3 bg-gray-50 border-t border-gray-200 text-xs text-gray-500 flex flex-wrap gap-4">
            ...
            <span><i class="fas fa-building mr-1 text-green-500"></i>Branch prices = base + surcharge</span>
...
function getBranchSurcharge(branchId, wc) {
    const el50 = document.querySelector(`[name="surcharge_50_${branchId}"]`);
    const el74 = document.querySelector(`[name="surcharge_74_${branchId}"]`);
...
function recalcPreview() {
    GRADES.forEach(grade => {
        ...
        if (!valid) {
            ...
            BRANCHES.forEach(b => {
                ['50','74'].forEach(wc => {
                    const c = document.getElementById(`prev_b${b.id}_${wc}_${grade}`);
                    if (c) c.textContent = '—';
                });
            });
            return;
        ...
        const base74 = calcPrice74(base50);
        ...
        BRANCHES.forEach(b => {
            const c50 = document.getElementById(`prev_b${b.id}_50_${grade}`);
            const c74 = document.getElementById(`prev_b${b.id}_74_${grade}`);
            if (c50) c50.textContent = fmt(base50 + getBranchSurcharge(b.id, '50'));
            if (c74) c74.textContent = fmt(base74 + getBranchSurcharge(b.id, '74'));
        });
    });
}
...
function openReviewModal() {
    ...
    GRADES.forEach(grade => {
        ...
        const base74 = (base50 / bag50) * bag74 + pkgFee;

        BRANCHES.forEach(branch => {
            ['50', '74'].forEach(wc => {
                const surcharge = getBranchSurcharge(branch.id, wc);
                const newP  = Math.round(((wc === '50' ? base50 : base74) + surcharge) * 100) / 100;
    ...
    CUSTOM_VARIANTS.forEach(cv => {
        ...
        const newP = parseFloat(el?.value);
    ...
    const html = rows.map(r => {
        ...
        if (r.delta === null) {
            ...
            pctHtml   = `<span class="text-green-700 font-medium">+${r.pct.toFixed(1)}%</span>`;
        } else if (r.delta < -0.005) {
            rowClass  = 'bg-red-50';
            deltaHtml = `<span class="text-red-700 font-semibold">(${fmt(Math.abs(r.delta))})</span>`;
            pctHtml   = `<span class="text-red-700 font-medium">${r.pct.toFixed(1)}%</span>`;
        } else {
            rowClass  = 'bg-gray-50';
